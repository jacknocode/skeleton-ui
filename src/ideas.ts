...
export const ideas: Idea[] = [
  ...
  /* ---------- 「動かしているのは誰か」から出た2種（No.71 / No.72） ----------
     69種を数え直すと、そのすべてが「自分が操作した → システムが応えた」という
     1つの型でできていた。トリガーの種類は数えたことがあったが、行為者を
     数えたことがなく、数えてみたら1種類しかなかった。 */
  {
    id: 'presence-echo',
    title: '他人の操作のこだま',
    motion:
      '自分が触っていない行が、ふいに動く。自分の操作は押した瞬間から動いて跳ねて確定するが、他人の操作は先に気配（薄い輪郭がその行へ寄る）が来て、180msだけ何も起きない間があり、それから跳ねずに滑って入る。目を離している間の変更には気配が来ず、濃い残光が留まって、見た瞬間から薄れ始める',
    /* 芯は「誰がやったかを、色やアバターではなく緩急で語る」こと。
       跳ね＝「自分がやった」の印なので、他人の変更に付けると自分の操作に見える。
       間（ま）は CSS の delay ではなく JS のタイマーで持つ——
       delay で持つと prefers-reduced-motion の一括短縮で3拍が1拍に潰れ、
       標本の意味そのものが消える。 */
    tags: ['共同編集', 'リスト', '他者', '気配'],
    status: 'captured',
    specimenId: 'presence-echo',
  },
  {
    id: 'stale-refresh',
    title: '古くなった数字',
    motion:
      '自分は何もしていないのに、表示中の数字は時間が経つほど線形に退色し、輪郭が緩む。更新は2拍——第1拍で全体が同時に濃さを取り戻し（＝届いた）、140msの沈黙をはさんで、第2拍で変わった数字だけが名乗る。変わらなかった数字は一切動かない',
    /* 鮮度は「3分前」という時刻ラベルではなく濃度で持つ。
       退色にレイアウトを動かすプロパティ（幅・字間）を使わないこと——
       古くなることが「値が変わった」に見えてしまう。使えるのは opacity / blur / scaleY。
       薄さの床は 0.42（消えるのは「古い」ではなく「無い」）。
       「更新（変化なし）」の操作を残すと、第1拍だけ起きて第2拍が起きないことが見え、
       2つを言い分けられている証明になる。 */
    tags: ['アナリティクス', '鮮度', 'リアルタイム', '待ち時間'],
    status: 'captured',
    specimenId: 'stale-refresh',
  },

  /* ---------- 次に育てる2種（No.70〜72 の棚卸しから起こした） ----------
     72種を数え直すと、そのすべてが「そこに何かが在る」前提で動いている。
     在るものが増える・減る・確定する・古くなる語彙は揃ったのに、
     《無い》を語る動きが1つも無い。そして No.71 で他人が画面に入ってきたが、
     その他人と自分が《同じ1つ》を取り合う場面はまだ想定されていない。 */
  {
    id: 'empty-two-kinds',
    title: '「まだ無い」と「無くなった」',
    motion:
      '空っぽの箱が、2通りの空き方で待つ。まだ一度も入っていない箱は、受け入れる形（点線の枠）が先に描かれて静かに呼吸し続ける。中身が出ていって空いた箱は、最後の1件が去った跡が沈み込んで畳まれ、枠が閉じて呼吸しない。同じ「0件」でも、待っているのか終わったのかが動きで分かれる',
    /* 空状態には動かす対象が無い。動かせるのは《地》のほうしかなく、そこが設計の芯になる。
       呼吸する空＝これから来る（受け入れ口が開いている）、静止した空＝もう来ない（閉じた）。
       No.59「動いている＝まだ確定していない」の裏返しで、
       動いていることが期待の表明になる。
       難所は「畳む」ほう。要素が消えたあとに残る高さの始末で読みが変わり、
       ぱたんと閉じると「片付いた」、ゆっくり縮むと「失った」に見える。 */
    tags: ['汎用', '空状態', 'リスト', '不在'],
    status: 'sprout',
  },
  {
    ...
    title: '取り合いの譲り',
    motion:
      '自分が触っている最中の1つに、他人が手を伸ばす。奪い合わせず、先に触っていたほうが持ち続け、あとから来た気配は縁で押し戻されてその場で足踏みする。自分が離した瞬間に、待っていた気配がすっと入って中身が変わる。自分の変更が負けたときだけ、値は跳ねずに滑って上書きされ、押し戻された跡が一拍だけ残る',
    /* No.71 は「他人の操作が自分と重ならない」前提で書いた。重なったときの語彙がまだ無い。
       共同編集で本当に難しいのは衝突のほうで、ふつうは文言（「他の人が編集中です」）で
       処理されるが、それは動きの敗北。
       芯は《待たせていることを、待っている側の動きで見せる》こと。
       足踏み（同じ場所で小さく往復する動き）は図鑑にまだ無い語彙で、
       「進めないが生きている」を1つの動きで言える。
       跳ねを使えないのは No.71 と同じ理由（跳ね＝自分がやった の印）。 */
    tags: ['共同編集', '衝突', '他者', '待ち時間'],
    status: 'sprout',
  },
]
