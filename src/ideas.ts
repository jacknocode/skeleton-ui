...
export const ideas: Idea[] = [
  ...
  /* ---------- 「まだ確定していないもの」から派生した種 ----------
     ...
     残っているのは「放っておくと確定してしまう時間」と「押した時点と効く時点がずれる因果」。 */
  {
    id: 'undo-unravel',
    title: '取り消し猶予のほどけ',
    motion:
  ...
  /* ---------- 次に育てる2種（No.62〜64 の棚卸しから起こした） ----------
     64種のトリガーを数え直すと、まだ2つだけ手つかずの領域が残っていた。
     どちらも「図鑑の基本イージング（ぷるん）がそのままでは使えない」場所で、
  ...
  },
  {
    ...
    title: '先に見せて、あとで戻す',
    motion:
      '押した瞬間から数字が動くが、増えたぶんは浮いて斜線で塗られ、浅く呼吸し続ける。承認されれば浮きが落ちて実塗りに変わり呼吸が止まる。否定されると往路と同じ経路を減速だけで逆再生して戻り、戻り切ってから一度だけ震えて理由が出る',
    /* 図鑑の標本はぜんぶ「起きたことは正しい」前提で動いている。
       ネットワーク越しのUIだけが「先に成功したふりをして、あとで訂正する」を持つ。

       【設計上の論点 — 標本化のときに結論を変えた】
       種の段階では「肝は逆再生にしないこと。同じ道をそのまま戻すと
       『取り消した』ではなく『最初から何も起きていない』に見えてしまい、
       ユーザーが自分の操作が届かなかったことに気づけない」と書いていた。
       標本では逆に、往路と同じ経路の逆再生を採った。別の道で戻すと
       「巻き戻し」ではなく「何か別のことが起きた」に見えたため。
       種が心配していた「気づけない」問題は、経路ではなく
       《戻り切ったあとの震え》と《残り続ける理由の一行》で受けている。
       ——どちらが正しいかは実物を見て決める論点として残す。 */
    tags: ['汎用', '通信', 'エラー', '取り消し'],
    status: 'captured',
    specimenId: 'optimistic-rollback',
  },

  /* ---------- 次に育てる2種（No.67〜69 の棚卸しから起こした） ----------
     69種を数え直すと、その全部が「自分ひとりと、システム」の2者しか想定していない。
     画面を動かしているのが自分以外の誰かであるとき、その動きをどう語るかの語彙が
     図鑑に1つも無い。共同編集・共有ダッシュボードでは、これが芯の語彙になる。 */
  {
    id: 'presence-echo',
    title: '他人の操作のこだま',
    motion:
      '自分が触っていない行が、ふいに動く。自分の操作は押した瞬間から動くが、他人の操作は先に気配（薄い輪郭がその行へ寄る）が来て、一拍おいてから中身が変わる。自分の変更は跳ねて確定し、他人の変更は跳ねずに滑って入る',
    /* 芯は「誰がやったかを、色やアバターではなく緩急で語る」こと。
       自分＝跳ねる／他人＝滑る。他人の操作には必ず予備動作（気配）を置く。
       予備動作なしに変わると「勝手に書き換わった」になる。
       目を離していた間に変わった行だけが残光を持ち、見た瞬間から薄れ始める。 */
    tags: ['共同編集', 'リスト', '他者', '気配'],
    status: 'sprout',
  },
  {
    ...
    title: '古くなった数字',
    motion:
      '自分は何もしていないのに、表示中の数字は時間が経つほどわずかに退色し、輪郭が緩んでいく。新しい値が届くと退色した部分が染め直されるように濃さを取り戻し、変わった数字だけが1拍遅れて名乗る。変わらなかった数字は濃くなるだけで動かない',
    /* 鮮度は「3分前」という時刻ラベルではなく濃度で持つ。古さそのものを見せる。
       芯は「更新された」と「変わった」を別の動きで言い分けること——
       全体が濃さを取り戻す動きと、変わった値が名乗る動きを必ず別の時刻に分ける
       （No.67〜69 の設計則「受領と結果を別の拍にする」と同じ作法）。
       変わらなかった数字を動かさないことが最重要。全部動くとどれが変わったか読めない。 */
    tags: ['アナリティクス', '鮮度', 'リアルタイム', '待ち時間'],
    status: 'sprout',
  },
]
