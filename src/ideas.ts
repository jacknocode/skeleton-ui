...
export const ideas: Idea[] = [
  ...
  /* ---------- No.84〜86「置き換わるとき、何を引き継ぐか」 ----------
     中断の3種（No.81〜83）を作って分かったのは、83種がぜんぶ「同じものが動き続ける」場面だったこと。
     中断はひとつの物体の時間の話で、物体そのものは最初から最後まで同じだった。
     空いていたのは逆側——「別のものに置き換わるとき、何を引き継ぐか」。
     置き換わり方は3つあり、引き継ぐものと、その担保の手段がそれぞれ違う:
       同じものが別の場所へ → 同一性を引き継ぐ  → 担保は「一度も消えないこと」
       仮のものが本物へ     → 枠（寸法）を引き継ぐ → 担保は「骨が実体の型であること」
       同じ場所に別のものが → 何も引き継がない  → 担保は「一度も動かないこと」
     3種を貫くのは、同一性を形でも色でもなく連続性で語ること。
     そしてクロスフェードは、84 では同一性を殺す嘘になり、85／86 では正しい語彙になる——
     同じ技法が置き換わり方によって真偽を変えるのが、この回の主題。 */
  {
  ...
  },
  {
    ...
    title: '動いたか、入れ替わったか',
    motion:
      '同じ見た目のリストに、性質の違う2つの変化を起こす。並べ替えでは行が消えずに移動し、ページ送りでは行が1pxも動かずに中身だけが入れ替わる。同じ画面で「引き継ぐ／引き継がない」を並べて、動きだけでどちらかを言う',
    /* 84 と 85 が「引き継ぐ」側なら、これは引き継がない側。難所は逆向きで、
       「動かしてはいけない」ではなく「同じに見せてはいけない」。
       入れ替えを縦のスライドで書いた瞬間、それは移動の語彙を借りることになり、
       別のデータが来たのに「同じ行が並び替わった」に読める。
       だから同一性の軸（移動＝縦）と入れ替えの軸（不透明度と3pxの微小変位）を直交させる。
       席番号を行ではなく席に持たせるかどうかが、この標本の一番細かくて決定的な分かれ目。 */
    tags: ['汎用', 'リスト', '同一性', '並べ替え'],
    status: 'captured',
    specimenId: 'moved-or-replaced',
  },

  /* ---------- 次に育てる2種（No.84〜86 を作ってみて見えた穴） ----------
     置き換わりの3種を作って見えたのは、86種がぜんぶ
     「静止した舞台の上で・見ている人の目の前で」動いていたこと。
     引き継ぎの是非は物体どうしの関係の話で、物体を載せている舞台のほうは
     ずっと固定されたまま、そして観測者はずっとそこに居ることになっていた。
     残っているのは動きそのものではなく、動きが成立するための前提の側——
     (1) 誰も見ていない間にも変化は起きる (2) 動きが乗っている土台のほうが動いている。
     どちらも「動き単体では意味が決まらない」場面で、図鑑はまだ語彙を1つも持っていない。 */
  {
    id: 'unseen-change',
    title: '見ていない間に起きたこと',
    motion:
      '目を離している間に届いた変化は、戻ってきた瞬間に再生し直さない。動きは見られていなければ情報を運べないので、代わりに「変わった」という跡だけが静止したまま待っている。跡は読まれた時点で引き、二度は説明しない',
    /* 86種はぜんぶ「起きた瞬間に見られている」前提で成立している。
       実際にはタブは裏に回るし、スクロールで画面外に出るし、人は席を立つ。
       素朴な実装は戻ってきたときにアニメーションを走らせるが、それは二重に間違っている——
       起きたのは今ではないのに今起きたように見え、しかも見ていなかった変化の回数は伝わらない。
       難所は「跡の寿命」で、消えるのが早いと見逃し、残り続けると画面が跡だらけになる。
       読まれたこと（可視領域に入る・タブが前面に戻る）をトリガーにするしかない。
       もう1つの難所は、まとめ方。目を離している間に5回変わったものを5回ぶん語るのか、
       最後の1回として語るのか——No.82（同じ返事は束ねる）の、時間軸が伸びた版になる。 */
    tags: ['汎用', '不在', '差分', '通知'],
    status: 'sprout',
  },
  {
    ...
    title: '動いているものの上で動く',
    motion:
      '動いている親の上で、子が自分の動きを始める。親の変形は子の見かけの尺と距離を歪めるので、子は自分の尺をそのまま使えない。土台が動いている間、子の緩急は親の残りに合わせて畳まれる',
    /* 図鑑の86種は全部、動くものが静止した舞台に直接置かれている。
       入れ子は1つも無い。実際のUIでは、開きかけのシートの中でトグルが跳ね、
       スクロールしている行の中で数字が増える。
       難所は座標系で、親が scale していると子の translate は見かけ上その倍率で伸び縮みし、
       同じ「6px 跳ねる」が親次第で 4px にも 9px にも見える。
       もう1つは尺の衝突——親が 420ms で着地する途中に子が 300ms の動きを始めると、
       着地の瞬間に2つの減速が重なって、どちらの動きも読めない濁りになる。
       答えの候補は「子は親の残り尺に収まるまで待つ」か「子は親の間だけ振幅を半分にする」。
       どちらが正しいかは、作って動かすまで決めない。 */
    tags: ['汎用', '入れ子', '座標系', '合成'],
    status: 'sprout',
  },
]
