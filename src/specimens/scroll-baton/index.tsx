...
/* ---- 3パネルのバトンの姿（企画書の表そのまま） ----
   ...
   y はパネル高300前提だった旧レイアウトの値を260/300倍に詰め直すところから始めたが、
   そのまま(91, 104)を使うと WP_PANEL2 と WP_PANEL3 のトラック距離が273pxとなり、
   ビューポート高(260px)を超えてしまう。跡(WP_PANEL2に残る)と受け皿(WP_PANEL3に立つ)は
   企画書の受け入れ条件5「跡と受け皿が同時に読める」を満たす必要があるため、両者の距離が
   ビューポート内に収まるよう、この2つだけ調整幅を持たせた
   （WP_PANEL2: 91→100、WP_PANEL3: 104→70。x・w・h・rは変更していない）。 */
const WP_PANEL1: Waypoint = { x: 46, y: 130, w: 18, h: 18, r: 2 } // 正方形。「48」の行頭マーカー
const WP_PANEL2: Waypoint = { x: 272, y: PANEL_H + 100, w: 14, h: 14, r: 7 } // 円。折れ線の右端の点
        ...
        >
          <div className="mz-scroll-baton-track" style={{ height: TRACK_H }}>
            ...
            <section className="mz-scroll-baton-panel mz-scroll-baton-panel-2" style={{ height: PANEL_H }}>
              ...
              >
                {/* y は旧レイアウト(パネル高300)の値を260/300倍に詰め直した上で、終点だけ
                    WP_PANEL2 の y(100)に合わせて再調整したもの。終点そのものは
                    バトン(円)自身が担うので描かない */}
                <polyline points="28,173 89,139 150,156 211,113 272,100" />
              </svg>
            </section>
