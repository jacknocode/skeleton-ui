...
/* ---- No.104「まだどこにも居ない」----
   ...
   ---- 企画側レビューを受けて追加: 入口の印は「予告」の担体なので、No.59の型を借りる ----
   初版の入口の印は静止した中空の輪だった。企画側の検収で「この標本のいちばん新しい
   担体が動きを持っていない」「480px縮小のGIFでは消えかねない」と指摘を受けた。
   考えてみれば、この印が言っているのは「ここから始まる」という**まだ確定していない
   予告**であって、この図鑑には予告の語彙がすでにある——No.59「予告と発効」の
   「予告の紙は端が揺れ続け、発効の瞬間ぴたりと止まって濃度が入る」。入口の印も
   同じ構造を持っている: 「立つ」までは印は確定しておらず(揺れ続ける=呼吸する)、
   Enter/↓で現在地が立った瞬間、is-entryの条件(hereId===null)が外れて印は消える
   (=発効した瞬間ぴたりと止まる、をここでは"消える"という形で実演している。No.59は
   紙が残って濃度が変わるが、この標本は「無いは担体の不在でしか描けない」という
   本題の主張どおり、確定後は印そのものを画面から消す)。実装は呼吸(opacityの明滅、
   1.8秒ループ)をis-entryのときだけ付け、径は動かさない(隣の静止した行を騒がせない
   ため)。基本イージング(ぷるん)は使わない——跳ねるのは"起きた"出来事の語彙で、
   これは"まだ起きていない・待っている"の語彙なので、素直なease-in-outにした。 */
