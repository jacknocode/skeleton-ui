...
/* ---- No.110「順番のほうが変わる」----
   ...
   現在地の順位が1になると、守るべきscrollTop(=現在地の書類座標 - 目標y)が負になり
   0にクランプされる。クランプが起きたかどうかは実測(wantedとactualの食い違い)から
   毎レンダー導出する派生値(clampedDir)であって、専用のstateフラグを別途持たない——
   持つと「クランプが起きたのに消し忘れる」という規約が警告する第2の状態を生むため。
   クランプ時は無言で0にせず、帯で「先頭になりました」と言う(No.96の教訓の踏襲)。
   ...
   1. 「更新が届く」1クリックがどの行を動かすかは、企画書が決めていない。16idを網羅する
      ...
      し、含めることで(e)のクランプ経路が3クリックという少数の操作で機ozal的に到達できる
   ...
   4. 行の高さが常に一定(絞り込みが無い)なので、filtered-out/place-lostのように
      ...
      実測して初めて違いに気づいた。)
   6. 既定/対照の切り替えは「初期状態への巻き戻し」だが、行は常時transitionを
   ...
   5. 可視判定(画面外かどうか)は毎回 listRef.current.scrollTop を直接読む(値を
      ...
      <div className="mz-place-reordered-frame">
        ...
        {!contrast && clampedDir && (
          <div className="mz-place-reordered-clamped" data-mark="clamped" data-dir={clampedDir} role="status">
            {clampedDir === 'top' ? '先頭になりました。これ以上は守れません' : '末尾になりました。これ以上は守れません'}
          </div>
        )}
      </div>
