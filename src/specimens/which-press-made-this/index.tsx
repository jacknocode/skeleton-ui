...
/* ---- No.173「この粒は、どの押下か」----
   ...
   ---- 踏んだ罠1: 座席とセルの当たり判定を粒/点そのものに付けていた初期実装 ----
   最初、pointerイベントを6×6pxのdot要素に直接付けていたところ、Playwrightの
   `locator.hover()`はdotの中心座標にマウスを動かすため数値上は問題なく
   通ったが、スクリーンショットを見て「地の沈みが粒の輪郭ぎりぎりでしか
   反応しない=指先だと外れる」当たり判定の狭さに気付いた。地(セル/座席)側に
   pointerイベントを移し、担体側からは一切のイベントハンドラを外した
   (担体はイベントを持たない=C1が構造的に保証される、という芯1の形に
   合わせて直した)。

   ---- 踏んだ罠2: 対照の「両側が沈む」をhoverRailWeekとhoverHistoryIdxの
   単純な等値比較だけで実装し、週2の2点が2個とも沈まなかった ----
   対照は履歴が`data-week`を持つ(壊れ方1)。最初、定規の粒を指したときに
   「data-weekが一致する最初の1点だけ」を沈める実装(`indexOf`で最初の1件を
   探す)にしたところ、週2の粒を指しても点が1個しか沈まず、企画のC6
   (週2の粒を指すと沈む点が2個)を満たさなかった。`indexOf`をやめ、
   `HISTORY_WEEKS_CONTRAST`を`every`ではなく`map`で全走査して一致する
   **全indexを配列で**沈める実装に直した。

   ---- 踏んだ罠3: 赤い印(壊れ方2)をhover状態と同じ`is-sunk`クラスに乗せてしまい、
   指していないのに常に地が沈んで見えた ----
   最初、週6の粒・4つめの点の「結べない」印を、担体の`is-sunk`相当のクラス
   (背景色を変えるクラス)に載せて実装しかけたが、これは地(セル/座席)の
   クラスとして書いたコードの使い回しだったため、赤い印を持つ担体の**地**まで
   常時沈んだ色になってしまい、「沈み=今指している場所」という既定の語彙と
   衝突した(指していないのに沈んで見える)。赤い印は担体(dot)自身の
   background-colorを変える別クラス(`is-unmatched`)として実装し直し、
   地(セル/座席)のクラスとは完全に別の要素・別のクラス名に分離した。

   ---- 企画の穴: 「セット/座席の寸法」「操作ボタンの有無」を明記していない ----
   ...
   3. 同じ週の2点を同じ1粒に結ぶ: 週2の粒を指すと、data-week===2の点が
      2個とも(indexが2つとも)沈む(踏んだ罠2で実装)。 */
