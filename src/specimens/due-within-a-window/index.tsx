...
/* ---- No.143「いつ来るかは、幅でしか決まっていない」----
   ...
   だけが別物であることを言う。週5の一点の予定(チップそのもの)は比較のためだけ
   に置き、空き枠を消費しない——占有の主張は`稼働`と`支払い`の対比だけに絞る。
   ...
   言うことになり、芯4の「1個だけ増える」が崩れる)。これは実装しながら気づいた
   弱点で、報告に正直に書く: 実測しない限り「週8が最初から埋まっている」ことに
   踏み込んだ意味は読み取れない可能性が高い。

   ---- 難所2: 週5の一点の予定は、何のために置くか ----
   芯1(帯とチップは同じ語彙)を実測で示すための比較対象。占有の主張(芯2)には
   関与させない——空き枠を消費しない。占有と幅の対比を`稼働` vs `支払い`だけに
   絞ることで、「幅のある帯とチップの見た目が同じ」ことと「幅の帯と継続の帯の
   占有が違う」ことという2つの独立した主張が、1つの帯(週5)に重ねて実装されて
   混線するのを避けた。
