...
// 読みかけ行(id=5)の初期上端Y(文書座標) = 5*52 = 260。枠内座標 = 260 - 156 = 104。
// リスト枠(.list-wrap)の外側の座標系に固定する物差し線はここに引く。
// 行の「中心」ではなく「上端」に合わせてあるのは見た目の都合ではない——中心に引くと
// 破線が行のテキストを横切り、右端の数値（残り 9）が打ち消し線に見える。図鑑では
// 打ち消し線は No.89/No.91 で「消えた旧い値」を意味する語彙として既に使われているので、
// 物差しがその語彙と衝突すると、動かないことを示すための線が「この値は失われた」と
// 読めてしまう。上端に引けば何も横切らず、静止時は行の境界にぴたりと重なって基準に見え、
// ずれた瞬間だけ破線と行の境界が離れる——ずれの量がそのまま2本の線の距離になる
const RULER_TOP = READING_INDEX * ROW_H - INITIAL_SCROLL_TOP
