...
/* ---- No.116「戻せない操作」----
   ...
   **単一の変数**(`'reversible' | 'irreversible' | null`)にする。空席の表示条件は
   ...
   3. 履歴が0個(標本を開いた直後)のとき、空席・締め線のホバー予告が地の色に対して
      薄すぎて読めない、という指摘をレビューで受けた。C2は「両方が1以上のフレーム0枚」
      という個数条件でしか予告の存在を測っておらず、個数が合っていても目視で読めるかは
      別問題だった(No.111/112が踏んだのと同じ「条件は通っているのに通り方が違う」)。
      対処として、点が0個でも「ここが履歴の列だ」を言うレール(.seal/.seatより弱い
      色・場所の提示だけ、事実は言わない)をstrip側に常設し、空席・締め線の線色を
      濃くした(style.css側の追記コメント参照)。レールはNo.95の言う「担体」には
      数えない(C2/C6等の個数条件は従来どおりレールを含まない要素だけで測っている)。 */
  ...
  return (
    ...
    >
      ...
      <div className="mz-irreversible-step-strip" aria-hidden="true">
        <span className="mz-irreversible-step-rail" />
        {history.map((h, i) => (
          <span
            key={h.seq}
            className={
