...
import ScheduledAbsence from './specimens/scheduled-absence'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    nameEn: 'One Outline, Three Meanings',
    category: 'フィードバック',
    trigger: '週を送りながら3つの輪郭（受け付けた／その週には入らなかった／まだ返事が来ていない）を見比べる。返事待ちの帯がどこまで伸びているかを見る（右上で「輪郭を3種類に増やす」対照に切り替え）',
    principles: ['4値目を作らない。割るのは担体ではなく場のほう', '主語は割らないまま、時制だけ割る', '待っている長さは事実。期限は言わない'],
    ecology: 'TODO',
  ...
  },
  {
    ...
    no: 163,
    nameJa: '来ない週が、決まっている',
    nameEn: 'Scheduled Absence',
    category: 'ナビゲーション',
    trigger: '隔週で動く指示の定規を週送りで見る。途中で規則が毎週に変わる（右上で「スキップのバッジと次回予告」対照に切り替え）',
    principles: ['来ない週には担体を1つも置かない', '空きを割るのは、空きの側ではなく定規の側', '規則が変わっても、過去の刻みは遡らない'],
    ecology:
      'No.156 は「まだ来ていない」、No.157 は「できなかった」を撃った。この標本は同じ**空き**の3つめの意味——**そもそも来ない週**である。指示が隔週で動くので、谷の週は最初から起きる予定が無い。No.156 は「空きの意味を割るのは縦線の左右だけ」と決めたが、**規則どおりの空きは縦線の左にも右にも在る**——ここでも割る軸が尽きている。**第1の芯は、谷の週に本当に何も足さないこと。** スキップの札も点線の枠も置かない。実装は `isDueWeek(leavingWeek)` が偽の週について `ticks` にも `ledger` にも書き込まないので、**その週について呼ばれる関数が存在しない**という形で不在が保証されている（実測で谷週の担体は **0 個**。対照はバッジ **1 個**）。**第2の芯が、この回の3種を貫く答えの最初の実例になる——空きを割るのは、空きの側ではなく定規の側である。** No.160 が作った「帯の下の刻み」を借りるが、あちらの刻みは残量という**値**の跡だったのに対し、こちらの刻みは値を持たない——指示が動くはずだった週の**位置**にだけ1本落ちる。原資が足りず粒を置けなかった週にも刻みは残る（`isDueWeek` は資金を見ない）ので、**「刻みが在るのに粒が無い」＝来るはずだったのに来なかった／「刻みが無い」＝そもそも来ない**、の2通りが自動的に生まれる。セルそのものの絵は両方で完全に同一で、実測は `background-color` `rgba(0,0,0,0)` / `border` `0px none` / **30×22px** の **distinct 1 値**——**割っているのは定規側であって、空きの側ではない。** **第3の芯は、規則を量ではなく間隔で言うこと。** 刻みを値ではなく週の位置に置いたので、隔週区間の間隔は **60.000px**、毎週区間は **30.000px**（比 **2 倍**）と、計算式を1つも足さずに規則そのものが読める。**第4は、規則が変わっても過去が遡らないこと**（No.160「跡は遡って作れない」の継承）。週6 で隔週から毎週へ変わった瞬間、既存の刻みの位置差分は**すべて 0px**、個数は 3 → 4（新規 1 本の追加のみ）で、**境目を言っているのは間隔だけ**である。履歴の点は `次の週へ` を8回押しても **±0**（規則はどちらの台帳にも載らない）。対照（スキップのバッジ＋次回予告＋規則変更で過去のバッジを一斉に付け替える実装）は、何も起きていない週に担体が立ち、未来側に予告が **1 個**出て、**バッジが消えた 1800ms 後には「来なかった週」と「そもそも来ない週」の画面差が 0 になる**——閉じた理由が画面のどこにも残らない。**企画の誤りが1つ出た。** 受け入れ条件は「空きのセルの computed style が distinct 1 値」と書いたが、**この語彙圏の先行標本（156/157/160）はどれも空きを独立した DOM 要素として持っていない**（粒が無いことそのものが空きである）。実装はこの標本にだけ全週へ透明・単一クラスのセルを常設して条件を満たしたが、**条件が先行標本の設計と一段ズレていた**ことになる。セルは rail や週番号と同じ常設の下敷きとして扱い、担体の数には含めていない。**実物を見て直った点もこの回に出た。** 対照のバッジは初版が幅 30px 固定・7px で、「来なかった」の5文字が箱からはみ出して**等倍では文字が欠けて読めなかった**。中心合わせの auto 幅に直している。実装が計測側の罠も1つ掘り当てた——要素単体のスクリーンショットは要素の境界でクロップするので、`translateX(-50%)` で左端付近の要素がはみ出すと**ページ内では収まっているのに切れて見える**。',
    Component: ScheduledAbsence,
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: '2本の指示を順に始めて週を送る。片方だけ落ちた週・両方とも起きた週を見比べる（右上で「同じ週に粒を2個置いて色で分ける」対照に切り替え）',
    principles: ['粒は起きたことしか言わない。何回・どちらは場が言う', '同じ週に2個置かない（共通則を破らない）', '読めないことは、代償として隠さない'],
    ecology:
      'No.156〜158 は「1本の指示」を前提に空きの理由・輪郭の意味・履歴の切り分けを決め、No.160 がその上に「同じ指示のまま量が変わる」を足した。この標本は**前提そのものを外す**——実プロジェクト（Startup Sim の、固定費と積立が同時に立っている回）の逆算どおり、**指示が2本、同時に立つ**。ところがこの語彙圏の共通則は「**1週に粒1個＝1回**」で、これは1本の指示を前提にした規則だった。2本になると**粒の個数が回数を意味しなくなる**（1個の粒が2回を意味しうる）。**第1の芯は、共通則を破らないこと。** 定規の粒は週に最大1個のままで、2本とも起きた週も1個しか積まない。実測で全8週の粒は `width` **6px** / `height` **6px** / `background-color` **rgb(61,61,61)** の **distinct 1 値**。**第2の芯が答えで、どちらの指示かは粒ではなく場が言う**——No.160 の刻みを**指示ごとに1段**へ拡張し、その週にどちらが動くはずだったかは段のほうが持つ。刻みは新語彙ではない。No.157 が確立した「塗り＝起きた／輪郭＝起きなかった」の2値を、丸ではなく角の上でそのまま使っているだけで、**値の種類を1つも増やしていない**。**第3は、同居（同心）を作らないこと。** 片方だけ落ちた週（週6）は、定規の粒は塗りのまま **1 個**、同心は **0 個**で、落ちたことは **B の段の輪郭 1 個**だけが言う。塗りと輪郭が同じ週に重なる絵は No.161 が「後から起きていたと分かる」に使い切っているので、ここで再利用すると語彙が割れる。**第4の芯は、読めないことを代償として隠さないこと。** 2本とも起きた週（週2）と1本だけの週（週1）で、定規の粒の6項目は**完全に一致する**——**粒の列だけを見ると、回数は永遠に読めない**。これは欠陥ではなく、共通則を守った帰結であり、実測として出してある。同じことが履歴でも起きる: `指示Aを始める` と `指示Bを始める` の点は **distinct 1 値**で、実装は由来のタグを型としても持たない——**どちらを始めたのかは履歴からも読めない**（No.149/150「点は種類を名乗らない」の継承）。**第5に、行を2段に増やさない。** 増えてよいのは場の側（刻みの段）だけで、粒の列を2段にすると「どちらの段の空きか」という第2の軸が読み手に増え、No.156 が決めた1次元の規則が未検証のまま2次元へ拡張される。対照（同じ週に粒を2個置き、指示ごとに色を変える実装）は、週ごとの粒の個数が **1/2 と distinct 2 値**になり、`background-color` も **2 値**（rgb(58,114,196) / rgb(196,122,42)）——**粒1個＝1回の読み方が、No.151 以降の全標本に遡って壊れる**うえ、この図鑑が自分で封じているはずの色を使っている。**実装が企画の決めていなかったことを3つ決めた。** ひとつ、落ちる理由を資源プールで作らず、No.161 の決め打ち配列と同じやり方（`FAIL_WEEKS_B = [6]`）にした——主題は「粒からは回数が読めない」であって「なぜ落ちるか」ではなく、資源を2本ぶん再現すると複雑さが主張と無関係な場所に増える（落ちた理由を画面が説明しないのは No.147 の継承）。ふたつ、刻みの形を円ではなく**矩形 6×8px** にした——円にすると「新しい粒」に見えて、**担体を足していないという主張が視覚的に崩れる**。みっつ、現在地の縦線を定規・A 段・B 段の3段まとめて貫通させ、3段が同じ時間軸に載っていることを1本の線で保証した。**正直に書いておくべきことが1つある。** C1 は「全週の粒が distinct 1 値」を要求するが、この台本では A が一度も落ちないので、**定規の粒に輪郭が立つ経路は一度も通っていない**（コードの分岐は在るが未使用）。条件は通っているが、通り方が台本に依存している。罠も1つ——縦線の `grid-row` を固定したところ、段を持たない対照でも grid が暗黙に4行ぶんのトラックを作り、**外形が縮まなかった**。行数を減らす修飾は、その行を参照する全要素に伝播させる必要がある。',
