...
import SankeyUnassigned from './specimens/sankey-unassigned'
...
export const specimens: Specimen[] = [
  ...
  {
    id: 'sankey-unassigned',
    no: 84,
    nameJa: '割り振らないサンキー',
    nameEn: 'Unassigned Sankey',
    category: 'アナリティクス',
    trigger: '「すきま: なし」に倒す（帯をなぞると1本だけ残る）',
    principles: ['決まっていない対応は描かない', 'すきまは片側だけ', '一度満ちて止まる'],
    ecology:
      'No.64 と同じサンキーだが、描けない情報がひとつある題材を引き受けた標本。週次収支の赤字の週は、原資が「収入」と「現金の取り崩し」の2つで、行き先が費目4つ——けれど**どの費目を収入が賄って、どれが取り崩しだったのかは決まっていない**。現金に色は付いていないので、その対応はどこにも存在しない。だから原資は1本の柱にまとめ、割るのは柱の中だけにする。No.64 が6本の帯で流入元と結果を結べたのは、その対応が実在したからで、ここでそれを真似ると図が事実でないことを主張しはじめる。問題は、その正直さに副作用があること。対応を持たない帯は左右とも同じ順・同じ積みで並ぶしかなく、始点と終点のyが毎本一致する——ベジエが水平の直線に潰れ、サンキーは1本も曲がらない**灰色の板**になる。積み上げ棒をリボンで描いただけのものが残り、「流れ」だと言っている図が流れていない。「すきま: なし」がその状態で、これは失敗作の再現ではなく、正直に描くと素直にこうなる、という位置にある。効く一手はすきまを**行き先の側にだけ**空けること。右の並びだけが背高になるので帯は必ず外へ広がり、上の帯は上へ、下の帯は下へ弧を描く。ここで肝心なのは縮尺をすきま最大の状態で一度だけ決めて動かさないことで、開け閉めしても帯の太さは1pxも変わらない——量は保存されたまま、形だけが戻る（切り替えると、太さが同じまま端だけが開くのが見える）。濃淡は向きにも使う。同じ墨の薄い側→濃い側でグラデーションを掛けると、原資の内訳を何ひとつ主張せずに「左から右へ」だけが出る。色相を持たない図鑑で帯を4本描き分けるには濃さしか手が無いが、濃さを太さと相関させると濃度が量に見えるので、いちばん太い人件費を中間の濃さに置いてわざと相関を切ってある。粒は持たない。No.64 の粒は「触らなくても生きている」ための常時アニメーションだが、こちらは一度満ちてそこで止まる——常設の観測面で鳴り続ける合図は2度目からノイズで、量を幅と密度で二重に語る必要も無い。残したのは、なぞった流路だけが残ってほかが薄まるほうだけ。',
    Component: SankeyUnassigned,
  },
]
