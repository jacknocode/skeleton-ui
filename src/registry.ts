...
import PlaceReordered from './specimens/place-reordered'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: '親を畳んで、現在地を畳まれた中へ入れる（右上で「親へ移す」の対照に切り替え）',
    principles: ['内側には方角が無い', '代弁するのはいちばん外側の閉じた親', '畳んだのは行であって現在地ではない'],
    ecology:
      'No.90〜107 の18種は、口に出していない前提をもう1つ共有していた——**台帳は据わっている**。行は全部描かれ、集合は変わらず、順番も変わらない。この回はその3つを1つずつ外し、108 が外すのは**「行は全部描かれている」**。No.106 も現在地が見えなくなる場面を扱ったが、あれは**行そのものは描かれていた**（枠を動かせば見える）。ツリーの親を畳むと、行は在るのに描かれない——**担体を置く相手が画面に存在しない**。だから **No.106 の答え（縁に方角の担体）がそのままでは使えない**。畳まれた中は上でも下でもなく「内側」で、方角の指す先が無く、距離も行数で言えない（描かれていない行は、数えられる並びの上に無い）。既定は方角ではなく**入れ物を名指す**——いちばん外側の閉じた親の行に「中に読みかけ」の印を出す。ここは No.95「担体は1つの事実しか言えない」に真っ向から触れる場所で、親の行に現在地の囲みを置くと「親が現在地」と読める。実測で既定は 囲み **1個** / 代弁 **0個**（現在地が描かれているとき）と 囲み **0個** / 代弁 **1個**（畳まれたとき）を撃ち分け、開閉の遷移中の全フレーム（閉じ **23枚**・開き直し **25枚**）で**両方が1のコマは0枚**。第2は**畳んだのは行であって現在地ではない**こと——読み手は畳む操作をしたが、現在地を狙って操作したわけではない（No.96 の構図。ただしあちらは行が消え、こちらは行は在って描かれない）。畳む前後で `data-place` は `taskA1a` のまま不変で、開き直すと同じ行に囲みが戻り、**現在地の行の枠内 y も 60.0px → 60.0px（偏差 0.0px）**。対照（畳んだら現在地を親へ移す。実際によくある実装）は畳んだ瞬間に `data-place` が親 `matterA1` へ書き換わり、**開き直しても戻らない**——読み手は畳んだだけなのに現在地を失う。第3は深さで、畳まれた親のさらに親も畳まれているとき、代弁するのは**いちばん外側の閉じた親だけ**（内側の閉じた親の行は、そもそも DOM 上に **0個**）。**段数は言わない**——「2段内側」と言うと深さが第3の事実として立ち、担体が3つになる。代わりに、外側を開いたとき代弁は**内側の閉じた親へ受け渡される**。受け渡し 160ms の **28フレーム全部で印の個数は 1**（0 になるコマが1枚も無い）で、`data-holder-id` は `clientA → matterA1` の2値のみ。第4は戻り道と時間で、代弁の印を押すと現在地までの閉じた親が**1回で全部**開き（二重に畳んだ状態から1クリックで囲み1個）、3秒放置しても開かない（`data-place` 不変・代弁1個のまま）——**時間では戻らない**（No.93・96 と同じく、行為でしか閉じない）。畳んだ親の行の高さは **30.0px → 30.0px** で変わらない（行が太ると「親が別のものになった」と読める）。企画が定義していなかった穴が1つあった——**`↑` の挙動をどの受け入れ条件も書いていない**。実装は移動の基準点を「いちばん外側の閉じた親自身の行 index」に置き、そこへ ±1 する**1本の式**にして上下を対称にした（実測で `↓` は閉じた親の次の描かれた行 `matterA2`、`↑` は前の描かれた行 `clientA`。どちらも囲み1個・代弁0個＝**中から出たことを移動として言い切る**）。この標本の収穫は実装の側から出た2つで、どちらも「正しさを実測で守る」から**「正しさが構造から出る」**への移動にあたる。ひとつ、**受け渡しは「消えて湧く」より作るのが簡単だった**——A用とB用の担体を出し分けて交差させるのではなく、**同じ1つの DOM 要素を畳んでいるあいだ生かし続け、行 index から出した translateY を毎レンダー書き換えるだけ**でよい。CSS の transition が間を補間するので、「受け渡し中は個数が常に1」は実測で確かめる対象ではなく**そもそも崩しようがない構造**になる。**担体の同一性（DOM 要素の同一性）と事実の同一性（代弁という概念の同一性）を一致させると、動きの正しさが実装の必然として出てくる。** ふたつ、**囲みと代弁の排他は if/else ではなく1つの探索関数の裏表で書く**——根から辿って最初の閉じた祖先を返すか `null` を返すかの二値にし、囲みは `null` のときだけ、代弁は非 `null` のときだけ出す。「両方が1のコマは0枚」は本来 rAF で数えるまでもなく自明になる（それでも実測して裏を取った）。**担体の排他性は、実装者が気をつけて守るものではなく、同じ変数の裏表にして構造で保証するもの。** 測定の側にも教訓が出た。C10（無関係な枝を開閉しても現在地は動かない）の初回測定で「枠内 y が -30px 動いた」と出たが、これは実装のバグではなく **Playwright の `.click()` が可視域の外にあるトグルへ自動スクロールした**ためだった。可視域の中で完結する枝に対象を変えると偏差 **0.0px**。**実測値が条件を満たさないとき、疑うべき先は実装だけではない。**',
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: 'ログが流れるのを眺め、上へスクロールして追従を外す（右上で「末尾の行に印を置く」の対照に切り替え）',
    principles: ['末尾は行ではなく境界', '戻る先は座標ではなく状態', '追いついていないと未読があるは別の事実'],
    ecology:
      '108 が「行は全部描かれている」を外したのに続いて、この標本は**「行の集合は変わらない」**を外す。No.105 も機械が現在地を動かす場面を扱ったが、あれは**尺が既知で逆走できる録音**だった。ライブ（実況・チャット・流れ続けるログ）では**台帳のほうが伸び続ける**。芯は「末尾に居る」が**行ではなく動く境界**であること——行 id で持つと1件増えるたびに古くなり、**No.97「現在地は行の同一性で持て」が持つ先を失う**（No.104 が連続空間で同じ壁に当たったのと同じ形）。だから現在地は `{kind:"row", id}` か `{kind:"edge"}` のどちらかを持ち、`edge` は**行を指さない**。担体もそれに合わせて、末尾の**行**ではなく台帳の**終端の境界線**に置く。実測で追従中に10件到着しても境界の担体の枠内 y の偏差は **0.0px**（180フレーム全部）、`is-cursor` と境界の担体が**両方1のフレームは0枚**。対照（末尾の行に印を置く）は同じ10件で `data-row-id` が **10回とも別の値**に変わる＝**現在地が動いたように見える**（動いていない。増えたのは台帳）。そしてここで、企画が想定していなかった数値が出た。対照の印は**いま湧いている当の行の内側**に乗るので、その行の scale アニメーションが親子関係で合成され、担体の位置が**2.4px 揺れる**。境界の担体はどの行にも属さないので構造的に巻き込まれない——**No.106 の「動いていないものを動きで語らない」が、ここでは「動いているものの子にしない」という一段具体的な実装規則として現れた。** 第2は追従で、自動スクロールは現在地ではなく**枠に貸した権利**（No.105）。上へスクロールすると外れ、3秒放置で `scrollTop` の変化は **0px**（対照は 1300ms で 0→96＝上限へ引き戻す）。外れた瞬間に `data-place-kind` は `edge → row` へ渡り、掴むのは**いちばん上に見えている行**（実測で期待 id と一致）。以後、台帳が5件伸びても読みかけの行の枠内 y は**静止後 0.0px・到着アニメーション中の106フレームでも最大偏差 0.0px**。第3が芯で、**戻る先は座標ではなく状態**。帯を押した瞬間の `maxScrollTop` は **122** だったが、着地するあいだに1件届いて最終的な末尾は **154** になる——`edge` という状態に戻す既定は最終 `scrollTop` が **154** で偏差 **0.0px**、そのとき座標（122）へ飛ぶ実装なら **32px（ちょうど1行）** ずれていた。**押した瞬間の末尾へ飛ぶ実装は、着地するまでに末尾でなくなる。** No.97 の「座標で持つな」の、境界の側での再演になっている。第4は事実の分離で、**「追いついていない」と「未読がある」は別**。帯は追従が外れているあいだだけ 1個（追従中は 0個）、**5秒放置しても消えず件数も変わらない**（時間では閉じない）。件数は外れた時点からの増加分で、押すと帯ごと消え、次に外れた直後は **0** から数え直す。第5に、行の現在地と境界の現在地は**地続き**——最終行で `↓` を押すと `edge` に入り（囲み1→0・境界0→1）、`edge` で `↑` を押すと最終行の `row` に戻る（実測で `edge→row→edge→row` の遷移を確認）。到着する行は**下から湧く**（出発地の無い出現＝No.102 の答え。実測で transform の並進成分は常に 0＝scale と opacity だけ）。対照との差は担体の置き方・引き戻し・帯の3箇所だけで、**追従中の `scrollTop` の進み方は既定 `[32,32,32,32,32]` / 対照 `[32,32,32,32,32]` で完全一致**（主張に関係ない差を作っていないことの担保）。実装が **No.105 の「旗」のバグの第3形**を掘り当てた。モード切替で台帳の行数が変わる瞬間、こちらが1行も書く前に**ブラウザが古い `scrollTop` を新しい上限へ丸めて `scroll` を発火する**。追従の効果は「もう正しい値だ」と判断して代入をスキップするので旗が立たず、**ブラウザ発の補正が読み手の操作と誤認されて追従が外れて**いた。直しは「結果として実際に末尾から離れたか」という第2の検査を足すこと——**旗（意図）だけでなく結果（離れたか）でも判定する**。目視で1つ直した。帯を枠の**上**に置いた初稿は、追従が外れた瞬間の現在地が「いちばん上に見えている行」であるために、**読み手がいま読んでいる行そのものを覆っていた**。下の縁へ移すと、覆う相手が変わるだけでなく、`▾` が指す向きと帯の置かれた縁が揃う——No.106 の「方角の担体は出て行った縁の側に置く」が、そのままここでも効いた。',
  ...
  },
  {
    ...
    no: 110,
    nameJa: '順番のほうが変わる',
    nameEn: 'Place Reordered',
    category: 'ナビゲーション',
    trigger: '更新が届いて台帳が並び替わる（右上で「素直に並べ替える」の対照に切り替え）',
    principles: ['守れるのは現在地か周りかの一方', '動いたものを位置ではなく数で言う', '守れない端では黙らない'],
    ecology:
      '108 が「行は全部描かれている」を、109 が「行の集合は変わらない」を外したので、残るのは**行の順番**。No.100 が「入れ物のほうが変わる」を扱ったその隣で、こちらは**中身の並びのほうが変わる**。この標本には図鑑で初めての事情がある——**守るものが2つあって、両方は守れない**。No.90・96・100 が一貫して「読みかけの行の枠内 y を守る」で答えられたのは、**現在地の行が台帳の中で動かなかった**から。順番が変わると、現在地を据え置けば周りが全部流れ、周りを据え置けば現在地が枠内を移動する。既定は**現在地の行を守る**（読み手が見ているのはその行だから）。実測で並べ替え1回の前後の枠内 y は **68.0px → 68.0px（偏差 0.0px）**、対照（素直に並べ替える）は **68.0px → 102.0px（偏差 34.0px＝ちょうど1行）**。第2は、**それでも順位は伝えなければならない**こと。No.93・96 は「変化を隠す・席で守る」で答えたが、順位そのものが読ませたい事実なので隠せない。答えは No.106 の裏返しになった——あちらは「動いていないものを動きで語るな、距離は数で名乗れ」、こちらは**動いたものを位置で語らず、数で言う**。実測で同じ操作の前後、`data-rank` は **7→8** と変わり、枠内 y は **0.0px**。現在地の同一性（`data-place`）は **"6" のまま不変**で、動いた行は translateY **306px→0px**、順位が変わった行は **10行**、その16行全部の `transition` は `0.42s cubic-bezier(0.34, 1.56, 0.64, 1)` で**完全一致**（周りは一斉に、同じ緩急で流れる）。第3は原因の所在で、可視域の中で完結した並べ替えでは気配は **0個**、画面外で起きた並べ替えでは縁に **1個**「外で1件動いた」「外で2件動いた」と件数を名乗り、静止後は **0個**に戻る（**見えているものについては気配を出さない**——出すと担体が二重になる）。第4は連続で、ライブの並べ替えは数秒ごとに来る。束ね窓 600ms を持つと、200ms 間隔で3件届いても行が動くのは **1回**（対照は **3回** 跳ねる）。ここで企画が気づいていなかった穴を実装が塞いだ——**束ねているあいだ、順位の数字を先に変えてはいけない**。実測で窓の途中（100ms 時点）の `data-rank` は旧値 **"7"** のままで、窓が閉じたときに位置と数字が**同時に** "8" になる。数字だけ先に変えると、まだ動いていない行が**嘘の順位を名乗る**。第5は端で、現在地が台帳の上端に近づくと守るべき `scrollTop` が負になってクランプされ、枠内 y は守れない（実測で **0.0px**＝目標から **-68.0px**）。No.96 の「守れない座標を黙って0にしない」をそのまま踏み、そのとき帯が **1個**（クランプしない並べ替えでは **0個**）。**この帯の文言も1度直している**——初稿は「先頭になりました」と名乗っていたが、クランプが起きるのは順位1のときだけではない（目標 y を引けるだけの行が上に残っていない時点で起きる。実測では**順位2**で起きた）。**順位を名乗ると帯が事実でないことを言う**ので、名乗る先を「台帳の上端」に変えた。3秒放置では順位も位置も `scrollTop` も変わらない（**時間では動かない**）。`▼` で現在地を別の行に移すと、以後は**新しい現在地**の枠内 y が 68.0px で守られる（守る対象は座標ではなく現在地である、の担保）。対照との差は4箇所（補正・束ね・数字・気配と帯）だけで、`transition-duration` も `timing-function` も一致、並べ替え1回の所要は既定 **411.9ms** / 対照 **416.9ms**。この標本の最大の収穫は実装から出た。当初は No.90・93・96 と同じ流儀で **cubic-bezier を JS で数値評価し、独自のクロックで scrollTop を追わせて**いたが、CSS の transition が実際に始まる時刻（次のペイント）と `performance.now()` の起点がずれるので、遷移中に数 px 揺れる。**規約が警告している「同じ仕事をする2系統」を、自分で作ってしまっていた。** 直しは曲線を予測するのをやめ、**現在地の行にいまこのフレームで実際に描かれている transform の行列を毎フレーム読み、そこから scrollTop を書く**こと——読みと書きが同一の rAF コールバックに閉じるので**原理的にずれない**。**別の transition が動かす値は、予測するより、いま描いている値を読むほうが単純で頑健。** 目視と実測でバグを2つ潰した。ひとつ、クランプの帯の高さが行高と一致しておらず下の行の文字と重なっていた（No.106 が同じ直しをしている——**縁の担体は1行ぶんきっかり覆う**）。ふたつ、既定と対照の切り替えが「初期状態への巻き戻し」ではなく**アニメーション**になっていた（全行が常時 transition を持つので、切り替え時の並び替えまで補間されて対照の基準値が 68px ではなく **90.8px** になっていた）。切り替えの瞬間だけ transition を外し、ペイント後の次フレームで戻して解いた。**モードの切り替えは動きではない**——対照を「見せる」ための操作が、対照の測定値そのものを汚していた。',
    Component: PlaceReordered,
  },
]
