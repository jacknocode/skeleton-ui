...
import ScheduleSlips from './specimens/schedule-slips'
import SameWeekManyOrigins from './specimens/same-week-many-origins'
import DueWithinAWindow from './specimens/due-within-a-window'
import OccupancyWithoutAnOwner from './specimens/occupancy-without-an-owner'
import OrderWithinAWeek from './specimens/order-within-a-week'
import RunwayEndsBeforeThePlan from './specimens/runway-ends-before-the-plan'
...
import AttemptLeavesNothing from './specimens/attempt-leaves-nothing'
import PaidButNothingMoved from './specimens/paid-but-nothing-moved'
import RepeatsWithoutMe from './specimens/repeats-without-me'
...
import OutlineMeansThreeThings from './specimens/outline-means-three-things'
import TwoStandingOrders from './specimens/two-standing-orders'
import UndoneAfterTheFact from './specimens/undone-after-the-fact'
import PressMeansTwoThings from './specimens/press-means-two-things'
import OrderDecidesWhoFails from './specimens/order-decides-who-fails'
import RuleSkipsThisWeek from './specimens/rule-skips-this-week'
...
import GrainMadeByTheReading from './specimens/grain-made-by-the-reading'
import BothHandsAtOnce from './specimens/both-hands-at-once'
import UndoWithoutReturn from './specimens/undo-without-return'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger:
      '`交換する` を10回ほど押して捨て置き場を育てる。溢れたらトレイの中をホイールで送って続きを読む。`片付ける` で詳しさを落とす（右上で「弱めて描き、時間で消し、閾値で束ねる」対照に切り替え）',
    principles: ['薄めず・小さくせず、位置と詳しさだけで畳む', '溜まったことは占有面積が言う', '片付けは読み手の操作。画面は自分から片付けない'],
    ecology:
      'No.132〜134 は「捨てたほうを消さず・薄めず・動かさずに残す」と決めたが、あれは**1回きりの交換**を前提にしていた。この標本はその続きで、**10回繰り返したときに何が起きるか**を扱う。最初に答えを出さなければならない問いが1つあった。**「捨てた」という関係を運んでいるのはどれか**——捨てたほうの値そのものか、いつ捨てたかか、何と引き換えにか。試すと**値そのものは運んでいない**（「3回目の交換で、何かと引き換えにこれが捨てられた」は、値を知らなくても読める）。だから捨て置き場の1マスは `data-seq`（いつ）と `data-kept-value`（何と引き換えに）を要素が在る限り手放さず、畳むときに落とすのは `data-value` を**表示する子要素だけ**にした。実測で畳みの前後、マスの個数は **5 → 5** で不変、値ラベルの要素数は **5 → 0**、`seq` / `kept-value` は5件とも一致——**落ちたのは詳しさだけで、関係は要素と属性という測れる形で残る**。No.91 が「跡は束ねずに畳む」と決めたのをそのまま持ってくると、この標本では**関係のほうが落ちる**（捨てられたものは、単体では「ただの古い値」と見分けが付かない）。担体そのもの（点。**9×9px** / `rgb(61,61,61)` / `opacity 1`）は本線（取った値の履歴）と**同じCSSクラス**を共有させ、値を揃えて一致させたのではなく**構造で一致を保証**した（実測で本線と捨て置き場から採った6個の computed style は、背景色・不透明度・幅・高さの4値とも **distinct 1 通り**）。**この標本のいちばんの主張は、捨てたものが実際に場所を取ることである。** 捨て置き場は幅 306px のまま、交換1回で高さ **34px・面積 10,404px²**、5回で **74px・22,644px²**、10回で **114px・34,884px²** と、触った回数に比例して育つ。トレイは高さ 100px で打ち止めてあるので、10回に届くころには**本当に溢れる**（実測で `scrollHeight` **126px** > `clientHeight` **98px**）。**溢れをどう扱うかが最大の分かれ道だった。** 器を痩せさせる案は点が重なって読めなくなる失敗（No.116・118 が踏んだ「担体が読めない」）を呼び込み、本線を押す案は本線の点が最大10個しか無く圧迫されるほどの余地が無い。採ったのは**器がスクロールする**——外形は交換回数によらず1pxも変わらない（全モード・全回数で幅 **320px** が不変、高さは既定で最大 **263px**）。ここで計測の罠が1つ出た。**`overflow-y: auto` を持つ容器自身の `getBoundingClientRect()` は、溢れても一定値のまま変化しない**（クリップされた表示上のサイズしか返さない）ので、C3 の「占有面積が増える」はスクロールコンテナではなく**その中身の要素**を測らないと必ず落ちる。**溢れの扱いを選ぶことは、測る対象を選ぶことでもある。** もう1つの分かれ道は、**場所を取る不便さを、いつ・誰の判断で軽くするか**である。前回の縛り「交換レートを画面が決めない」の延長で、**片付けることも、画面がレートを安く見せる介入になり得る**。だから既定は自分からは何も減らさない——実測で10回のあいだマス数は **1 → 10** まで一度も後戻りせず単調に増え、**5秒放置しても 10 のまま**。減らせるのは `片付ける` を押した読み手だけで、押すと値ラベルを持つマス（**70×34px**）が点だけのマス（**20×20px**）へ縮む。ただしこの変化は**不可逆**で、一度付いた `is-folded` は二度と外れない（3秒待っても畳んだ4件は **4 件**のまま）。**片付けた事実そのものは消さない。** 対照は4つのナイーブな壊れ方を1つに同居させる。捨てた点は最初から **4×4px** / `opacity 0.4` という、取った点（**9×9px** / `opacity 1`）より**弱い姿**で右詰めに現れ、2.5秒放置すると **1 → 0 個**まで消え、累計が6件に達した瞬間、画面が自分の判断で個々の点を全部引っ込めて `捨てた: 6` の**バッジ 1 個**に束ねる（実測でチップ **0 個**）——**この一度の遷移が「関係を束ねて消す」と「画面が自分で片付ける」を同時に体現している。** 正直に留保も書いておく。**「溜まった」ことを言っているのは、専有面積が育つことと、溢れて縦スクロールが要ることだけ**である。触れば読めるが、**静止画1枚では「箱が小さいだけ」に見える可能性は消えていない**。',
  ...
  },
  {
    ...
    category: 'ゲーム',
    trigger:
      '`現金を払う（¥40）` を数回押して `次の週へ` を進める。払った3週後に確定する（右上で「確認ダイアログ＋推定＋帯＋完了トースト」対照に切り替え）',
    principles: ['「分からない」を幅にしない', '空の場所は実線の箱で言う', '確定は完了ではない'],
    ecology:
      '現金を払って時間を買う。**押した時点では、いくらで買えたのかが決まっていない。** No.132〜134 が「交換レートは画面が決めない。読み手が決める」と決めたことの**裏側**で、**読み手が決めるには、レートが見えていなければならない**——見えないときはどうするのかを撃つ。最初に封じたのは No.74 の帯（確からしさを幅で言う）である。**帯で描くとレートが確率変数になる。** 実際にはレートは確率的なのではなく、**まだ観測されていないだけ**であり（No.110「まだ手元に無い」の交換版）、**「分からない」と「ばらつく」は別の事実**である。既定の答えは、受け取り枠を交換が生成された瞬間から**幅 62px 固定の実線の箱**にし、中の塗りだけが幅 0 で待つこと。**No.114 の破線の輪郭をここに流用してはいけない**——あれは「まだ起きていない」の語彙なので、借りると「まだ起きていない未来の推定」に読める。実線にすることで**箱＝交換は起きた（事実）／塗り＝レートが分かった（知識）**を別の担体に分けている。実測で払った直後、塗りの幅は **0px**、数字を持つ要素は **0 個**、帯は **0 個**。払ったほうは動く（`transform: scaleX` で **18.036px → 46px**）が、受け取り枠自体は確定前の20フレームで最大差分 **0px**——**払ったことは事実なので動き、受け取ったことはまだ事実でないので動かない。** 確定の瞬間に動くのは**受け取る担体だけ**で、払った額・他の未確定の行・レールの差分はすべて **(0,0,0,0)**、当該行の塗りだけが **0 → 44.1875px** へ伸びる。第2の芯は**対応関係**である。3回続けて払うと未確定が3つ並ぶので、どれがどれの確定なのかを読み手が追えなければならない。各交換に支払い順の `seq` を持たせ、確定量を `RETURN_TABLE[seq]`（**26 / 9 / 33 / 17h**、決め打ちで乱数なし）だけで決めた——**週の進み方にも他の交換の有無にも依存しない**ので、pay → advance を3回繰り返しても確定順（**26h → 9h → 33h**）は一度も入れ替わらない。第3は**過去のレートを、これからの側に置かない**ことで、置くと**画面が予告していないのに読み手が勝手に予告として読む**（**画面が黙っていても、並べた時点で主張になる**）。実測で操作行のテキストは `現金を払う（¥40）` / `次の週へ` / `週 N` のみで、過去の確定値から作った数値は **0 個**。第4は、**値札が無いことを確認では言えない**こと。払ったあとに取り消せない以上、素直に書くと確認ダイアログに倒れるが、**No.116 がそれを明確に否定している**（読み手を止めて文章を読ませるのは動きの敗北）。既定は `role="dialog"` **0 個**、「レートは変動します」等の文言 **0 箇所**。そして**確定は完了ではない**——チェックマーク・トースト・`確定しました` を1つも出さず、確定した行は消さずに残る（3回払って3回確定した後も行数は **3 のまま**）。**レートが分かったことは、次の交換の判断材料になる。だから終わっていない。** 対照は No.116 が否定した確認ダイアログ（**1 個**）、No.74 の誤用そのもの（推定値 **1 個** ＋ 斜線の帯 **1 個**。これまでの確定値の平均から作る）、そして完了の演出（`確定しました ✓` のトースト **1 個** が出たのち **1.3 秒**で行そのものが配列から消え、履歴が **0** まで落ちる）を同居させる。**対照では、レートを学ぶための材料が毎回きれいに片付けられていく。** 実物を見て直った点も1つある。**確定した数字を塗りの内側に置くと、塗りが伸びるほどラベルの背景を侵食してコントラストが死ぬ**（値が大きい交換ほど読めなくなる、という主張と正反対の壊れ方）。枠の外の読み取り列へ移して解いた——**C1 の「文字列が空」のような数値条件は、このコントラスト崩壊を1つも検出できない。**',
  ...
  },
  {
    ...
    category: 'フィードバック',
    trigger:
      '`買う` と `見送る` を押し分ける。どちらを押しても単価はその場では動かず、`次の回へ` で初めて次の単価が姿を見せる。買う・買う・買う・見送る・見送る・買うと回すと、自分の分だけが戻る回が見える（右上で「文言で名乗り、赤くなり、1つの数に混ざり、次の値札を先に出す」対照に切り替え）',
    principles: ['原因が読み手にあることを、画面は名乗らない', '止められる分と止められない分は別の担体に置く', '自分の分は、台帳の点と同じ形で並ぶ'],
    ecology:
  ...
  },
  {
    ...
    category: 'ゲーム',
    trigger:
      '`発注する` を押すたびに1行増える。自分の分（チップ）は押す前から個数が決まっているが、外の分（箱）は空のまま生まれ、次に `発注する` を押した瞬間だけ、直前の行の塗りが実測ぶんに伸びる（`見送る` は箱を確定させない）。発注3回・見送り・発注と回すと、自分の分のチップが `0,1,2` → 見送りで `0` に戻る回が見える（右上で「先出しの推定・薄い延長・`でした ✓`・注記の小文字」対照に切り替え）',
    principles: [
      '「観測待ち」の担体は再発明せず、No.136 の実線の箱をそのまま借りる',
      '自分の分は台帳の数え上げでしかない。だから未来の側に No.137 のチップを置ける',
      '合計は数として合成しない。チップと箱を同じ1本の並びに置き、占有した長さで語る',
    ],
    ecology:
      '外注の単価は `自分の分 + 外の分` で出来ている。**自分の分は規則で決まる**（これまで連続して発注した回数 × 40円。台帳の行を数え上げるだけなので**押す前から確定している**）が、**外の分は買ってからしか分からない**。1つの数字の**内側**で、分かっている部分と分かっていない部分が混ざる。第1の芯は**新しい担体を発明しないこと**——「観測待ち」という事実に図鑑はすでに答えを持っている（No.136 の**実線の箱＋幅0の塗り**）ので、外の分はそれをそのまま借り、**この標本が新しく決めるのは「規則で決まる部分」の担体だけ**にした。第2の芯がその担体で、自分の分は**過去からの予測ではなく、既に台帳に在る行の算術**なので **No.137 のチップ**（`10×10px` / `rgb(61,61,61)` / `border-radius: 2px`）を継ぐ——**同じ形を未来の側に置いてよい根拠は、それが過去の行の数え上げでしかないこと**である。実測でチップと台帳の点は `width` / `height` / `border-radius` / `background-color` / `border-style` が一致し、**x の差は 0.00px**（見送りを挟んだ後の行でも同じ）。自分の分の計算に確定値が混ざっていないことは、外の分の台本を**全部10倍**にして同じ操作列を再実行しても結果が**バイト単位で一致**することで確かめた（`{"countsPerRow":[0,1,2,0],"chipX":[511,511,525]}`）——**No.136 が禁じた「これからの側に値を置く」に触れないのは、自分の分がレートを一度も参照しないからである**。第3の芯は**合計を数として合成しないこと**で、チップの並びと箱を**同じ1本の並びの上に左から順に**置き、合計は横方向の**占有長**として在る（`+` も合計欄も区切り線も無い）。第4の芯が**分かっている部分だけで判断させない**ことで、空の箱は生まれた瞬間から**過去の外の分の最大値ぶんの幅**を持つ——幅は確からしさではなく**器の大きさ**（No.136 の箱と同じ意味）。既定に `推定`／`約`／`見込み` 相当の文字列 **0 件**、警告色 **0 件**、箱の `animation-name` は **4/4 とも `none`**、箱の幅は pending→resolved で**一度も変わらない**（`150/90/120/120px`、高さは全箱 `16px`）。確定の瞬間に動くのは**その行の塗りだけ**で、既存のチップは確定を挟む20フレーム（16ms 間隔）で **0.000px** も動かない——自分の分の更新は**次の行が生まれるときに初めて現れる**（1手ずれ。No.137 と同じ構造）。4回発注しても **4 行のまま**残り、完了の印は **0 件**。**企画の式が実装で破綻した。** 芯4 の「箱の幅＝過去の外の分の最大値」は、共有した台本 `[180,240,...]` の**2件目で既に破れる**（1件目の 180 ぶんしか予約しないのに実際は 240 で 30px はみ出す）。初版は `overflow: hidden` でこれを黙って切り落としており、**予約不足の箱が「ぴったり埋まった箱」に見えていた**——芯4 自身の主張と正反対である。答えは**はみ出しをクリップせず、塗りが箱の枠から正直に出ること**にしたが、**それでもまだ壊れていた**——塗りが枠線を塗りつぶすので、画面上では**箱がどこで終わっていたのかが読めず、ただの長い棒**に見えた（No.122「担体が読めるか」の再演）。最終形は塗りと箱を**兄弟要素**にして塗りを下（`z-index: 0`）・枠線を上（`z-index: 1`）に置き、枠線に `box-shadow: 0 0 0 1px #fff` の1pxのハローを敷いた（No.104 で使った手の再利用。新色ではない）——**器の右端と塗りの先端が、別々に読める**。**器がどこまでだったかが読めないと、はみ出しは何も語らない。** もう1つ、**No.137 のコメントが警告している罠に、その警告を読んだ上でまた落ちた**——発注行を包む `.rail` にだけ `padding: 7px` があり、絶対配置の子の座標系が台帳とずれて **x が 7px 食い違っていた**。**ラベル列の幅を揃えるだけでは足りず、その行を包む容器のパディングも揃える必要がある**（最終的には行内の座標を全部**行ローカル**に付け替えて、履歴の長さに依存しない形にした）。実装が企画の空白も1つ埋めている——**外の分がいつ確定するか**を企画が決めていなかったので、確定は**次の発注**に紐づけ、見送りでは確定させないと解釈した。外形は既定が空 **340×138px**、4件で **340×234px**、満杯で **340×260px**。**「既定が収まるから対照も収まる」は偽だった**——対照の1行は見出し＋注記＋バーの2〜3行ブロックで、同じ件数でも **473px** まで伸びたので、`MAX_HISTORY` を 6 に落として **340×302px** に収めている。',
  ...
  },
  {
    ...
    category: 'ゲーム',
    trigger:
      '`借りる` を押すと、翌週から5週ぶんの空き枠が消えてチップに変わる。もう一度 `借りる` と、重なった週にチップが縦に積む。`次の週へ` で現在地を進めると、追いついた週から順に枠が空きに戻るが、履歴のチップは1個も消えない（右上で「未来の枠が破線・返済中の文言と進捗バー・60ms刻みで順に現れる・完済で履歴も消える」対照に切り替え）',
    principles: [
      '確定した未来は、空きが無いことだけで言う',
      '同じ形のチップが、定規に載ると時制を、履歴に載ると読み手の操作を言う',
      '消える縛りと、消えない跡は、別の担体に分ける',
    ],
    ecology:
      '前借り——現金がいま増える代わりに、**これから5週ぶんの枠が埋まる**。まだ来ていないのに、もう空いていない。図鑑はここまで「これから」を **1種類**としてしか持っていなかった——No.114 の破線（不確か）と、No.136 の実線の箱（もう起きたが、まだ知らない）の2つである。**返済は不確かではない**ので破線は使えず、**まだ起きていない**ので実線の箱も使えない。第1の芯は、そこで**新しい線種を足さない**と決めたこと——**埋まっていることは、空き枠の担体が存在しないことだけが言う**（No.121「先に取られていた」の「在るはずのものが無い」を、**未来の側**で再演する）。実測で、既定の未来側の `border-style` は distinct **`[none]`** の1値のみ（**dashed 0 件**。対照は `[none, dashed]`）、`返済`／`あと N 週`／`残り` 相当の文字列 **0 件**、警告色 **0 件**、進捗バー **0 個**。借りると空き枠が **10 → 5 個**へ減り、チップが **0 → 5 個**へ増える（**−5 / +5 ちょうど**）。第2の芯が**台帳の割れ方**である。No.119 は「読み手がやったことは履歴に、時間がやったことは定規に」と決めたが、返済は**時間が来れば自動で起きるのに、起こしたのは読み手**なので、そのままでは台帳が割れる。答えは**担体の形が主語を言い、担体の位置が時制を言う**こと——チップは `10×10px` / `rgb(61,61,61)` / `border-radius: 2px` / `border-style: none` で No.137 の履歴の点と**同一クラス**（実装は分岐ではなく同じクラスを両所で使っている）、それが定規のどの目盛りに載っているかだけが時制を運ぶ。第3の芯は**消える縛りと消えない跡の分離**で、実装は `loans`（いま効いている縛り）と `history`（読み手の過去の操作）を最初から**別の state・別の DOM** に置いた——構造で分けたので、タイミングの調整では壊れない。2回借りて（3〜7週・4〜8週）6週進めて完済すると、定規の占有は週1〜8すべて **0**（空きに復帰）に戻り、**履歴は 10 → 10 で1個も減らず**、10個の座標差も **0**。借りた瞬間の5つは `animation-name` が **5/5 とも `none`**・`transition-duration: 0s` で、最初のフレームと次フレームの `getBoundingClientRect` 差は **0.00px**——**1回の操作は1つの出来事**なので、5つを順に湧かせない（対照は 60ms 刻みで5回起きたように見せる）。定規そのものは借りる前後で **280×54px → 280×54px（0.00px 差）**、目盛り10個の x も差 **0**——**変わるのは中に何が居るかだけ**。重なった週は No.130 の「深さは本数」を継いで**縦に積み**、積みの y 間隔は重なる4週すべてで **最小 12px・最大 12px（一定）**。外形は既定 **340×212px** で全状態を通して一定。実装が企画の空白を2つ埋めている。**枠が戻る単位**（企画は「5週まとめて」か「週ごと」かを決めていなかった）は**週ごと・独立**とし、現在地が追いついた週から順に解ける——おかげで「片方の借りだけが先に解ける」中間状態が実在する。**空き枠を過去の週にも一様に置く**ことも実装の判断で、借りる対象に過去週は構造的になりえないので誤読しない。この標本は No.140 と分界線を持つ——**埋まった枠が現在を通過する瞬間は 140 の主題**なので、ここでは週送りに何の演出も足していない。',
  ...
  },
  {
    ...
    category: 'ゲーム',
    trigger:
      '`次の週へ` を押すだけ。押すたびに現在地の縦線が右へ進み、すでに置かれている予定を追い越していく。週7には何も来ていない（右上で「通過の瞬間に跳ねて光る・赤いトーストが名乗る・済んだ印が付いて薄くなる・予定が現在の列へ飛ぶ」対照に切り替え）',
    principles: [
      '予定の担体は1pxも動かない。動くのは現在地の縦線だけ',
      '済んだかどうかは、現在地との位置関係だけが言う',
      '由来は、履歴の列と予定の列が同じ定規を共有していることだけで読める',
    ],
    ecology:
      '**確定した未来は、いつか「いま」になる。** 図鑑は「未来をどう描くか」を No.114 以降で何度もやったが、**未来が現在になる瞬間**を一度も扱っていなかった。この標本には `借りる` が無い——予定は**読み手が来る前から入っている**（週1に借りた跡が1つ、週5・6・8 に予定。**週7はわざと空**）。操作は `次の週へ` の1つだけで、読み手は「その予定を」押していないが「時間を」押している。第1の芯は、**予定どおりに来たものは、何も起きていないように見える**という難所への答え——動かすと「起きた」が強すぎ、動かさないと通過に気づかない。答えは**動くのを1つに絞る**ことで、実測で1クリックにつき `getBoundingClientRect` が変化した要素は**ちょうど 1 個**（現在地の縦線、差 **30.00px** = 目盛りの間隔）、チップ4個・目盛り9個・ラベル・レールを含む他のすべての子孫は **0.00px**。**動いたのは時間のほうで、予定は最初からそこに在った**（No.119「時間がやったことは定規に」の継承）。しかも**通過は出来事ではない**ので、通過数が変わっても動く要素は増えない——0クリック **0 個** / 1クリック **1 個** / 3連打（予定2件を通過）**1 個**（差 **90.00px** = 3×30）。第2の芯が**由来の言い方**である。「3週前の自分の操作」を再掲すると No.112 の履歴の列へ担体を引き渡すことになり、No.94 の「出発地へ消えない印を置く」は**空間**の答えなので時間には使えない。答えは**履歴の列と予定の列が同じ x 定規を共有する**こと——実装は両行を1つの CSS Grid の列（`chipX(week)=(week-0.5)*PITCH`）に載せたので、x の一致は手計算ではなく**構造**で保証される。実測でチップ中心は `借りた:週1=145` / `予定:週5=265, 週6=295, 週8=355`（viewport px）で、いずれも `chipX(week)+130.00` ちょうど——**全週・両行を通して系統誤差 0.00px**。**説明を1文字も書かずに由来が読めるのは、2つが同じ座標系に載っているときだけ。** 第3の芯は**済んだ印を置かない**ことで、通過したチップは現在地より左に来ただけ——`background-color` / `width` / `height` / `opacity` が通過の前後で**文字列一致**し、`opacity` は全チップ・全状態を通して **distinct 1 値 `1`**、`animation-name` は distinct **`[none]`**、チェックマーク **0 個**、`返済日`／`済`／`完了`／`今週は` 相当の文字列 **0 件**。週7に何も来ていないことも画面は言わない（**「何も来なかった」を言う必要は無い**——No.119「起きなかったことは動かせない」）。現在地の担体は**面ではなく 2px の縦線**で、x が目盛りからちょうど半セルずれるので**チップとの重なり面積は 0px²**（静止時だけでなく週4→9 の各遷移で実測）、下地には `rgb(214,214,211)` のレールを常設した（No.122 の教訓）。乱数は無く、3クリック後の DOM スナップショットは**独立した2回のリロードで文字列一致**する。外形 **340×165px**。**数値条件を全部通したまま絵が壊れる**例がこの標本でも出た——Grid のラッパに `align-items: center` が効いていたため、3行にまたがる縦線の列が **stretch されず縮み、線がほぼ消えていた**。`left` の値自体は正しかったので **C1〜C7 の数値条件は全部通っており、目視だけが捕まえた**（`align-self: stretch` で解決）。企画の誤りも1つ出た——受け入れ条件が禁止文字列に「済」を**部分一致**で入れていたため、**主題の言葉である「返済」そのものが使えなくなっていた**（行ラベルを `借りた` / `予定` にして解いた）。もう1つ、初版のキャプションが `動くのは現在地の線だけ` と**この標本の主張を文章で言ってしまっていた**（PR #37 No.118 と同じ失敗）ので、`「次の週へ」で現在地が進む` という操作の説明だけに落とした。計測側の落とし穴も見つかっている——図鑑は全標本を1つの DOM に同時にマウントするので、`data-role="marker"` が No.119 のものと衝突し、スコープを切らない `querySelector` が**別の標本の要素を掴んで「重なり 0」という偽の合格**を返していた。',
  ...
  },
  {
    ...
    no: 141,
    nameJa: '予定のほうがずれる',
    nameEn: 'Schedule Slips',
    category: 'ゲーム',
    trigger:
      '`次の週へ` で現在地が進む／予定チップを押すと、その予定だけが1週先へ動く（右上で「跳ねて光る＋元の週にゴースト＋警告トースト＋時間が進むときも予定がわずかに動く」対照に切り替え）',
    principles: ['動いた側にだけ中割りを与える', '来ていない日に跡を残さない', '主語は動き方ではなく台帳が言う'],
    ecology:
      'No.140 は「**予定は1pxも動かない。動くのは現在地の縦線だけ**」と決めることで「動いたのは時間のほうだ」を言い切った。だが**予定のほうがずれる場合を決めていない**。ここで初めて**相対運動が曖昧になる**——定規と予定の距離が変わる絵は、時間が進んだときとずれたときで**同じ**である。140 は片方を固定して曖昧さを消したが、両方が動きうるなら固定では解けない。芯は、**動いた側にだけ中割りを与える**ことだった。時間が進むとき動くのは現在地の線だけ（実測 **30.00px**、予定チップ3個・履歴・目盛り9個・行ラベル3個はすべて **0.00px**）で、しかも線は**尺ゼロで飛ぶ**。予定がずれるとき動くのはそのチップだけ（**30.00px**、現在地の線は **0.00px**）で、こちらは `transition: left 0.12s linear` の**等速**で運ばれる。16ms 間隔のサンプルで、ずれ中のチップは distinct **9値**（中割りが出る）、時間進行中の同じチップは distinct **1値**、時間進行中の線は distinct **2値**（中間値 **0個**）——**中割りが出た側が、動いた側である。** 等速にしたのも縛りで、`cubic-bezier(0.34, 1.56, 0.64, 1)` で跳ねさせるとずれが「祝うべき出来事」になる（対照はそれをやる）。第2に、**元の予定日に跡を残さない**。ゴーストも点線も置かない（ずれ後も予定行のチップ数は **3 のまま**、`opacity` distinct **[1]**、`border-style` distinct **[none]**、ゴースト要素 **0**）——**元の予定日は一度も来ていない**ので、跡を置くと No.113 の「事実でないものの跡が事実に見える」に触れる。跡は**履歴の行**に残す。第3が主語で、**読み手が繰り延べたずれと、外の都合で動いたずれを、まったく同じ動きで描く**（両者の `transition-duration` / `timing-function` は **0.12s / linear** で文字列一致）。違うのは**履歴に点が増えるかどうか**だけ——読み手のずれは **+1**、外のずれは **+0**。**同じ形の動きのままで、主語は台帳の側が言う。** ここで No.142 と食い違って見えるが、142 が行で分けているのは**由来**であって、141 が分けているのは**主語**である——**行は由来を分け、主語は「台帳に載るかどうか」が分ける。** 第4に、ずれると**元の週の空きが1つ戻り、先の週の空きが1つ消える**が、それは1つの出来事なので順に見せない（空き枠の総数はずれ前 **6**、0/21/41ms…の全11サンプルでも **6** で不変）。**実装が企画の穴を2つ実測で見つけている。** ひとつ、台本の予定が週5・6 と隣接しているため、週5をずらすと週6と同じ週に重なり、占有が Set で潰れて空きの総数が **6 → 7 に増えた**——空きの行は週ごとに0/1の二値でしか占有を言えないので、2件が同じ週へ重なると**この不変式は構造的に破れる**。「週9より先へは動かせない」という既存の前例を延長し、**移動先の週が埋まっているときも、そのクリックは何も起きない**という規則を1本足した（No.142 の実装も**独立に同じ規則へ辿り着いている**）。ふたつ、既定のキャプションに書いた「1週ぶん**ずれる**」が、企画自身の禁止語リストに引っかかった——**この標本の主題語が、可視テキストから禁じられていた**（PR #44 で「済」を部分一致で禁じて「返済」が使えなくなったのと同じ失敗の再発。「動く」に置き換えて解消）。禁止語 **0件**、警告色 **0要素**、既定側の全担体で `animation-name` distinct **[none]**（移動は transition であって animation ではない）。外形は初期・週送り後・ずれ後とも **340×192px** で一定（対照はトースト表示中のみ 340×232px）。',
    Component: ScheduleSlips,
  },
  {
    id: 'same-week-many-origins',
    no: 142,
    nameJa: '同じ日に、別々の理由で来る',
    nameEn: 'Many Origins, One Week',
    category: 'ゲーム',
    trigger:
      '`次の週へ` で現在地が進む／チップを押すと、その1件だけが1週先へ動く（右上で「由来ごとに形と濃さを変える＋同じ週を束ねて ×N＋引き出し線＋合計行」対照に切り替え）',
    principles: ['形は分けない。分けるのは行', '重なりは数ではなく縦の並びが言う', '由来を線で結ばない'],
    ecology:
      'No.137 は「**気づきは、同じ形をしていることでしか起こらない**」と決め、No.140 はそれを継いで「**由来は、同じ定規を共有していることだけで読める**」と答えた。ところが**由来が3つあると、同じ形であることが逆に由来を消す**——今週に返済・給与・家賃が来ていて、どれも同じ形の点である。形を分ければ 137 の答えが使えなくなり、分けなければどれがどれの続きか読めない。No.139 の縦積みは「同じ週に何個あるか」しか言っておらず、あのときは**由来が1種類しか無かったので破綻しなかっただけ**である。答えは、**形は分けず、行を分ける**こと。3つの由来のチップは同一クラスで、既定の10個全部について `width: 10px` / `height: 10px` / `border-radius: 3px` / `background-color: rgb(61,61,61)` / `border-style: none` / `opacity: 1` が distinct **1値**（完全一致）。分かれているのは行だけで、**3行が1つのトラック列（同じ x 定規）を共有する**——チップ中心 x と `chipX(week)` の差は10個すべてで **0.0000px**。行ごとに座標を手計算せず、CSS Grid の列を単一のコンテナで持たせたので、No.138 で踏んだ「その行を包む容器の padding が座標系をずらす」罠が**構造的に起こりえない**（3行が同じ1個の要素を共有するため `padding-left` は自明に同値）。**由来は縦（どの行に居るか）が言い、時期は横（どの列に居るか）が言う。** 第2に、同じ週の3つを「3つ」として数えない。合計も件数も出さない（禁止語 `件`/`合計`/`今週`/`¥` **0ヒット**、引き出し線 **0本**、バッジ **0個**）——今週の重さは、**現在地の線の上に縦に並んだチップの数**が言う（週5の3チップの y 間隔は **27.000px / 27.000px** で最小＝最大、現在地の線との重なり面積は3チップとも **0.000px²**）。No.138 の「合計は数ではなく長さで在る」の、縦の版。第3に、**行に開いてあるので、同じ週の3件が別々の行に居て、束ねなくても個別に触れる**——1件を押すと動くのは **1個だけ**（**dx 30.00px / dy 0.00px**）で、他の2件・行ラベル・目盛り・現在地の線・他の行のチップはすべて **0.00px**。ずれの緩急は No.141 とまったく同じ語彙（`transition-property: left` / `duration: 0.12s` / `timing-function: linear`）で、**この回の3種が1つの主題であることを、値の一致として持っている**。既定側の `animation-name` は distinct **[none]**、警告色 **0要素**、外形は「次の週へ」を4回連打しても **340.00×192.00px** で不変（対照は 340.00×216.00px）。**企画が決めていなかったことを実装が2つ埋めている。** 現在地とちょうど同じ週のチップは「もう来た」側として動かせないこと、そして**移動先の週に別の予定が居るときは動かさない**こと——後者は No.141 の実装が**独立に同じ規則へ辿り着いた**もので、2つの標本が別々に同じ穴に落ちて同じ答えを出している。対照では**企画のほうが壊れた**。「由来ごとに形と濃さを変える」と「同じ週を束ねる」を同居させると、家賃の週（5・9）が常に他の由来に取られて、**家賃の三角形が一度も画面に出ない**——壊れ方1 を見せるための対照が、壊れ方2 によって消えていた。束ねのアンカーを下の行へ移し、束ねであることを**大きさだけ**で言うようにして、3つの形（●／■／▲）と3段階の `opacity`（1.0 / 0.7 / 0.45）が同時に出るようにしている。触り方も No.141 と同じ語彙で、**見た目の 10×10 は1pxも変えずに、透明な 24×24 の当たり判定を同じ中心へ重ねている**（チップ自身に padding を足すと C1 の distinct 1値が崩れる）。',
    Component: SameWeekManyOrigins,
  },
  {
    id: 'due-within-a-window',
    no: 143,
    nameJa: 'いつ来るかは、幅でしか決まっていない',
    nameEn: 'Due Somewhere in the Window',
    category: 'ゲーム',
    trigger:
      '`次の週へ` で現在地が進む／窓の中で `いま来た` を押すと支払いの週が決まる（右上で「破線の帯＋未定バッジ＋左端が縮む＋確定で収束＋幅ぶんの空きを3つ消す」対照に切り替え）',
    principles: ['幅と長さは、空きの減り方だけが分ける', '幅は縮まない。動くのは時間だけ', '確定しても担体は1つも動かない'],
    ecology:
      'No.138〜140 は「**確定した未来は、位置で言うしかない**」で揃い、No.141 は位置が**動く**場合、No.142 は位置が**共有される**場合を撃った。残っていたのが**位置が一点に定まらない**場合である——来ることは確定しているのに、来る週は「6〜8週のどこか」でしか決まっていない。破線は借りられない（No.114 の予告＝**不確か**であり、ここでは意味が反転する）。既定の `dashed` は **0件**で、幅は**同じチップを幅ぶんに伸ばした1本の帯**で言う——帯とチップの `border-style: none` / `background-color: rgb(61,61,61)` / `height: 10px` / `border-radius: 3px` は**文字列一致**、違うのは `width` だけ（チップ **10px** / 帯 **90px**）。**この標本の芯は、幅（いつか分からない）と長さ（3週間続く）が同じ担体になってしまうこと**である。帯を見た読み手は「3週間続く出来事」と読む。凡例を置かずにこれを分ける手が要り、答えは No.139 の**占有**だった——**続くもの（稼働）は3週すべての空きを消し、幅のあるもの（支払い）は1つしか消さない**。実測で、稼働（週2〜4）と支払いの窓（週6〜8）は**同じ 90px の帯**でありながら、空き週は `[1,6,7,9]` の **4個**（占有は稼働3＋一点の予定1＋支払いの押さえ1＝5）で、支払いが消しているのは **1つだけ**。対照（幅ぶんの空きを3つ消す壊れ方）では同じ台本で空き週が `[1,9]` の **2個**に落ちる。**同じ長さの帯が、空きの減り方だけで別物になっている。** 第2に、**幅は縮まない**。週が進んで窓の左端を通過しても帯は動かず（週送り1クリックで動いた要素は現在地の線 **1個・30.00px** のみ、帯の `left`/`width`・チップ・空き枠4個・目盛り9個・行ラベル3個はすべて **0.00px**）、線が帯を貫き、線より左は「来なかった週」、右は「まだ来うる週」として同じ帯のまま読ませる（週7で線 x=**389**、帯 **330〜420** の内部。線には No.104 の 1px の白ハロー `rgb(255,255,255) 0px 0px 0px 1px` を敷いて、暗い帯の上でも読めるようにした）。**位置が一点でないときでも、動くのは時間だけ。** 第3に、**確定しても担体は1つも動かない**——帯は消えず、縮まず、来た週にチップが1つ増えるだけ（確定前後で帯の `left: 150px` / `width: 90px` / `opacity: 1` / `background-color` が文字列一致、担体総数 **3 → 4**、増えたのはチップ1個、`animation-name: none`）。**帯は「どこかで来る」という事実、チップは「来た」という事実で、別の事実だから片方が他方を消さない。** 実装が実物で2つ見つけている。ひとつ、**確定チップが帯に埋もれた**——同じ色・同じ高さで、確定週が帯の内側に入るので、チップが増えたことがスクリーンショットでまったく読めなかった。No.139 の「重ねず縦に積む」を借りて支払いの行を2段に割り、x を1pxも変えずに y だけ分けている。**この壊れ方は、数値条件を1つも破らずに起きた。** ふたつ、**「仮の押さえ」がどの週に居るのかは、読み手には読めない**。実装は「確定前は窓の右端（週8）を押さえ、確定と同時に週7へ移す」を同一フレームで行い、空き枠の総数は **4個で不変**だが、空き枠はただの無地の点なので「なぜ最初は週8なのか」は文言なしでは伝わらない。区別を作ると芯3の「増えるのはチップ1個だけ」が崩れるため、**読めないと分かった上で、文言で説明しない側を採った**——**占有の語彙は「何個ぶんか」は言えても「どの週か」を言えない。** これはこの回の答えの限界であり、次の種にしている。禁止語15語すべて **0件**、バッジ **0個**、警告色 **0要素**、既定側の帯・チップは `animation-name: none` / `transition-duration: 0s`。外形は週4・週6・週7・確定後・週9のどの状態でも **340×191px** で一定。',
    Component: DueWithinAWindow,
  },
  {
    id: 'occupancy-without-an-owner',
    no: 144,
    nameJa: 'その空きは、誰のものか',
    nameEn: 'Vacancy Without an Owner',
    category: 'ゲーム',
    trigger:
      '`次の週へ` で現在地が進む／予定チップを押すと、その1件だけが1週先へ動く／窓の中で `いま来た` を押すと来る週が決まる（右上で「空きを由来ごとに色分け＋確定前から窓の3週を薄い点で押さえる＋戻った空きにゴースト＋占有と空きを引き出し線で結ぶ＋`空き 7/9` のカウンタ」対照に切り替え）',
    principles: [
      '空きは二値のまま。理由は空きの側に書けない',
      '列が決まらない占有は、空きを消さない',
      '主語は台帳が言う。空きの見た目は、誰のものでも同じ',
    ],
    ecology:
      'No.139 が「確定した未来は、**空きが無いこと**だけで言う」と決めて以来、この図鑑は占有という語彙を持っている。だがその語彙は**二値**（点が有る／無い）でしかなく、No.143 はそこに幅のある予定を持ち込んで「幅のあるものは空きを1つだけ消す」と決め、その1つを窓の右端（週8）に置いた。数値条件は全部通ったが、**実物を見た企画者に「なぜ週8が消えているのか」が読めなかった**。この標本はその続きで、**空きは、自分が誰のものかを言えない**を主題にしている。答えは 143 の決定を**この回が撤回する**ことだった——**列が決まらない占有は、空きを消さない**。実測で、窓（週6〜8のどこかで1回来る）の確定**前**、週6・7・8の空きは **3個とも在り**、`いま来た` を押した瞬間に消えるのは **1個だけ**、しかも**確定した週（週7）**である。同じ瞬間に帯は `left`／`width` とも **0.00px** 差で動かず、確定チップが週7に1つ生える。**「帯そのものは何も消費していなかった」が、操作の前後2枚を比べるだけで読める。**「読めないものは、置かない」という決めのほうが、置いてから説明を足すより短い。第2に、**空きの点は二値のまま**である——7個の空きの computed style は distinct **1通り**（`rgba(0,0,0,0)` / `1px solid` / `border-radius: 50%` / `6×6px` / `opacity: 1`）で、既定のコードに「由来で見た目を分ける」分岐がそもそも存在しない。対照（由来ごとに色分けする壊れ方）では同じ7個が distinct **3通り**に割れる。第3に、**戻った空きに跡が残らない**。予定を1週ずらすと元の週の空きが戻り移動先の空きが消えるが、これは同一フレームで起こり、0 / 16 / 33 / 50 / 120ms の**全サンプルで空きの総数が 7 のまま不変**。戻った空きの点と、一度も占有されていない空きの点は computed style が完全一致し、**片側にしか無い DOM 属性が 0 個**（ゴースト要素 0個）——**その週には一度も何も来ていない**のだから、跡を残せば No.113 の「事実でないものの跡が、跡のほうが事実に見える」に触れる。第4に、**主語は台帳だけが言う**。読み手がずらしたときは履歴が **+1**、規則で確定したときは **+0**。そのとき動いた／戻った空きの点の見た目は**両者で完全に同じ**である——**「誰のものか」は空きの側には書かれておらず、載る台帳が違うことだけが主語を言う。** 実装が企画の穴を1つ実測で塞いだ。芯2により窓は確定前は「ただの空き週」なので、**予定チップを週7まで動かしてから確定を押せる**——既に無い空きをもう一度消そうとして、空きの総数不変が壊れる。実装は新しい印や警告を足さず、`occupiedWeeks()` という1つの関数を「空きの描画」「チップ移動の衝突判定」「確定の衝突判定」の3箇所すべてで使い、**確定も「移動先が埋まっていたら何も起きない」という既存の無反応規則の対象にした**（実測で `confirmed` は false のまま、履歴も 2 → 2 で不変）。もう1つ、**CSS のコメントの中に書いた `is-cat-*/is-dim` の `*/` がコメントを途中で閉じ、巻き添えで空きの点の基底ルールが CSSOM から丸ごと消えていた**——初回のスクリーンショットで「空き」行がレールだけの空白になり、`getComputedStyle().width` が `auto` を返す異常値から特定した。**数値条件を全部通したまま絵が壊れる**の6回目で、原因は初めて見る形だった。禁止語 **0件**、バッジ **0個**、引き出し線 **0本**、警告色 **0要素**、全要素の `animation-name` は **`none`**、予定チップの `transition` は `left` / `0.12s` / `linear`、現在地の線は中割り **0**（30サンプルで中間値 0件）。外形は週3・週6・確定後・週9・移動失敗後のどれでも **340×228.44px** で不変。',
    Component: OccupancyWithoutAnOwner,
  },
  {
    id: 'order-within-a-week',
    no: 145,
    nameJa: '同じ週の、どちらが先か',
    nameEn: 'Which of Them Came First',
    category: 'ゲーム',
    trigger:
      '`次の週へ` で現在地が進むだけ。週6と週8には `入る` と `出る` が同じ列に載っている（右上で「①②の番号＋週を2列に割る＋先着を大きく濃く＋決まっていない週にも順序を付ける＋底を一点で描く」対照に切り替え）',
    principles: [
      '位置は同率としか言えない。順序は、履歴に載る順が言う',
      '順序が無いことは、同時に載ることが言う',
      '一点に決まらない値は、No.143 の幅をそのまま借りる',
    ],
    ecology:
      '定規のいちばん細かい目盛りは**週**である。同じ週に2件が載ると、**位置は同率としか言えない**——どれだけ位置を精密にしても、そこから順序は出てこない。だが世界の側には順序がある。**これは位置の語彙が原理的に届かない場所である。** 手はほとんど残っていない。縦（y）は No.142 が「形は分けない。分けるのは**行**」と決めて**由来**に割り当てたので、順序を行で言えば同じ担体が2つの意味を持つ（PR #37 の「担体の空きは、1つの意味しか持てない」に正面から当たる）。週の中を2列に割るのは**定規を作り直すこと**で、`chipX` を共有する語彙圏の全部と食い違う。番号・矢印・大小・濃淡は No.118 が封じている。**残っていたのは時間だった。** この図鑑はすでに No.141 で「中割りが出た側が、動いた側である」と時間を語彙として使っており、その延長で**順序は、履歴に載る順だけが言う**。実測で、週6（順序が決まっている）は履歴の2点が **140.20ms** の差で増え、週8（順序が決まっていない）は **0.00ms**——同一コミットで2点が同時に載る。**順序が無いことを、順序を書かないことではなく、同時に載ることで言う。** 位置は同率のまま置いてある：週6・週8とも2件のチップの中心 x の差は **0.0000px**、`chipX(week)` との系統誤差も **0.0000px**。履歴の2点の computed style は distinct **1通り**で、**先に載ったほうが大きい・濃いといった差は 0**——順序は**載った順にしか出ていない**。第2の芯は、順序に依存する値の描き方である。その週のあいだにいちばん下がった値（`底`）は、順序が決まっていれば一点、決まっていなければ一点に定まらない。ここで新しい語彙を足さず、**No.143 の幅をそのまま借りた**——`底` の担体は週3・4・6で **10px**（チップと同値）、週8で **30px**（＝1週ぶんの幅）で、`border-style` / `background-color` / `height` / `border-radius` は4週とも**文字列一致**（違うのは `width` だけ、`dashed` は **0件**）。幅がちょうど1週ぶんなのは偶然ではない。**この標本の不確かさは週8の内側にあるので、幅は定規の上で「その週の中のどこか」を正確に覆う**——143 の帯が「来る週が一点でない」を週の並びの上で覆ったのと、同じ軸の上での同じ言い方になっている。実装が企画の決め落としを2つ塞いだ。ひとつ、**`底` がどの週に生えるのかが未定義だった**——「その週に予定が在るときにだけ生える」という規則を足した（事象が無ければ残高は動かないので、「いちばん下がった値」という出来事自体が無い）。ふたつ、**同じ週に2点重なったときに横へ並べるか縦へ積むかが未定義だった**——横に並べると x がずれて芯1と衝突するので、縦に積む。目視の前に実測が壊れ方を1つ捕まえている：**履歴の点に `width` の指定漏れがあり、幅 0px で画面に存在しなかった**（共有クラスは `height` しか持たず `width` は個別指定が要る作りだった）。スクリーンショットでも履歴の点は1つも見えておらず、`width: 0px` という computed style が先に見つけた。週送りで動いたのは現在地の線 **1個だけ**（30.000px）で、生えた `底` と履歴の点はその後2回の週送りを経ても **0.000px**。全子孫要素の `animation-name` は **`none`**、線は中割り **0**（16ms サンプルで distinct 2値・中間値 0件）。外形は週1〜9・両モードで **340×222px** 不変。',
    Component: OrderWithinAWeek,
  },
  {
    id: 'runway-ends-before-the-plan',
    no: 146,
    nameJa: '予定より先に、続けられなくなる',
    nameEn: 'Runway Ends Before the Plan',
    category: 'ゲーム',
    trigger:
      '`使う` / `足す` で原資の右端が動き、`次の週へ` で現在地が進む。週7の予定は、原資が届いていないあいだ空きの無い列に載っている（右上で「届かない予定を薄くする＋尽きる週に赤い旗＋残り週数の文言＋旗が0.4秒かけて滑る＋届かない予定を定規から落とす」対照に切り替え）',
    principles: [
      '成立しないことを、予定の担体には書かない',
      '3つめの担体を足さない。可否は空きが言う',
      '動くものを増やさない。増減するのは個数だけ',
    ],
    ecology:
      '予定は**確定している**。位置も正しい。定規の上の週7に、間違いなく置かれている。ところが**そこへ届く前に原資が尽きる**——位置は何ひとつ間違っていないのに、その予定は成立しない。**位置の語彙が足りない場面は、No.138 以降のこの語彙圏を通して初めてである。** 書ける場所がない。薄くする（No.114 の「やめた」）・破線にする（No.114 の「不確か」）・消す・赤くする——**全部すでに別の意味に予約済み**で、しかも尽きる週を新しい形で名指すと、それは**予定とまったく同じ形の3つめの担体**になる（No.142 が撃った問題の再来）。答えは**担体を足さないこと**だった——**尽きる週から先は、空き枠が無い**。もう何も置けないからである。占有が「予定で埋まっている」を言うのと**同じ形**で、「原資が無いので置けない」を言う。**そしてこれは No.144 の答えの帰結として正しい。** 空きが無い理由が予定なのか原資なのかは**見分けが付かない**が、144 が「空きは理由を持たない」と決めたのだから、これは矛盾ではなく**語彙どおり**である——実測でも空きの点の `className` は distinct **1種**、属性セットも distinct **1種**で、区別する属性・クラス・色は **0件**。**限界を承知で使い切るところまでが、語彙の設計である。** 第2の芯は、3つめの**動くもの**をどう描くかだった。尽きる週は読み手が操作するたびに動く——予定でも定規でもないのに動く。中割りを与えれば「誰かが動かした」に読め、与えなければいつ動いたか分からない。実装は**そもそも動かさなかった**：空きの点の `left` は生成時から不変（`使う` / `足す` の前後で残存点の差は全て **0.00px**）で、変わるのは**個数だけ**（初期 **5個** → `使う` で **4個** → `足す`×2 で **6個**）。増減は**端からのみ**起き、残った週番号は常に連続した範囲になる。**「動いた」のではなく「在る範囲が変わった」**——中割りを与える相手がそもそも居ないので、No.141 の非対称（動いた側にだけ中割り）を破らずに3つめの動くものが描ける。予定チップは、原資を最大まで減らしても増やしても週9まで送っても `left` distinct **1値**（週4 は 330.00px、週7 は 420.00px）、`opacity` distinct **1値**、`background-color` / `border-style` も各1値で、**成立するときとしないときで完全に一致**する。`足す` で原資の右端が週7に届いた瞬間に起きるのは、**空きの点が1個増えることだけ**（予定は 0.00px、`opacity` も不変）。**実装が企画の誤りを実測で1つ訂正した。** 台本は「予定が居る週の空きは No.139 の占有規則どおり予定に消される」と明記していたが、それを実装すると**台本自身の数値例（週1〜5に5個）・C2（残った週番号が連続）・C4（週7に届いた瞬間に +1個）の3つすべてと矛盾する**ことが分かった。3つの独立した根拠が一致して「予定週による追加控除をしない」モデルを指しており、**除外モデルでは週7の点が原資到達後も永久に現れず、成立と不成立が原理的に見分けられなくなる**ところだった。**受け入れ条件どうしの整合が、企画の誤りを外から捕まえた例である。** 禁止語8語すべて **0件**（原資を0まで枯渇させた状態でも）、バッジ **0個**、警告色 **0要素**、旗 **0個**、`animation-name` distinct **`none`**、marker / 空き / 予定チップの `transitionDuration` は全て **0s**（＝空きの増減は滑らない。移動ではなく増減だから）。現在地の線は原資が 0 でも止まらず週9まで到達する（**時間は原資と関係なく進む**）。外形は既定 **340×194px**、対照 **340×219px** で、同一モード内は全状態で不変。**正直に書いておくと、「空きの無い列に予定が載っている」という関係だけで成立しないことが読めるのは、この図鑑の語彙にすでに触れている読み手である。** 「空きの点＝その週まで原資が届いている」という約束を知らない初見の読み手には、予定チップの真下に点があるかどうかを意識して見比べる必要がある——No.143 の「仮の押さえ」と同種の限界が、**答えの側に回っても残った**。',
    Component: RunwayEndsBeforeThePlan,
  },
  {
    id: 'promise-not-yet-known',
    no: 147,
    nameJa: 'その約束を、まだ知らない',
    nameEn: 'Promise Not Yet Known',
    category: 'ナビゲーション',
    trigger: '空きの点が無い週に `置く` を押す。何も起きない（右上で「凡例＋初回だけの案内」対照に切り替え）',
    principles: ['画面は説明しない', '無反応が定義を教える', '読み手が学習しても、画面は学習しない'],
    ecology:
      'No.146 は最後に**自分の限界を正直に書いた**——「空きの無い列に予定が載っている」で成立しないことが読めるのは、**この図鑑の語彙にすでに触れている読み手だけ**である、と。この標本はその続きであり、**146 種ぶん積んだ語彙そのものへの問い**でもある。**語彙は積むほど強くなるが、積むほど初見には読めなくなる。** 素直な答えは「教える」だが、それは4方向から封じられている——凡例・注記・ツールチップ・初回だけの案内は**全部「文章で言う」に当たり**、動きで教えれば**教えたことが出来事になり**（No.114）、2回目以降に同じ動きを出せば**説明ではなく予告として読まれ**（No.117〜119 の語彙に混ざる）、そもそも**画面は読み手が知っているかを観測できない**。既定の答えは**教えないこと**である。代わりに、**知らなくても間違えない形**にする——読み手が置こうとした瞬間、置けた／置けなかったという**結果そのもの**が空きの点の定義を教える。**無反応は、それ自体が定義の提示である。** 実測で、空きの点が無い週7に `置く` を押した前後、ページ内**全 200 要素**の `left` / `top` / `width` / `height` の最大差分は **0.000px**、`opacity` / `background-color` の差は **0 件**、要素数は **200 = 200**、履歴は **0 → 0**。これは**出来事ではあるが、読み手が起こした出来事**なので、画面が勝手に動いて「いま何かが起きた」と読ませることがない。第2の芯は**説明モードを持たないこと**である。「初回かどうか」を判定する箇所がコードのどこにも無い（`data-first-visit` の類を持たない）ので、**1回だけの案内を持たないことで、1回だけの問題そのものが消える**。実測で、同じ操作を 1回目と5回目に行ったときのスナップショットは**互いに完全一致**（最大差分 **0.000px**・スタイル差 **0 件**）——内部の `data-attempts` は **1 → 5** と増えているのに、画面には 1px の差も出ない。**読み手が学習しても、画面は学習しない。** 第3は**先回りして無効化しない**ことで、`置く` は置けない週でも `disabled` にならない（全9週で **0 回**）——押せるかどうかを操作の前に画面へ出せば、それは「教える」ことになる（No.122 の「押せるうちは押させる」の継承）。既定に説明の担体は **0 個**（`legend` **0** / `tip` **0**、禁止語 4 語すべて **0 件**）、`animation-name` が付いた要素 **0 個**、警告色 **0 箇所**。対照は凡例 **1 個**・初回の吹き出し **1 個**・読み込み直後に光る要素 **5 個**（初期の空きの点の数と一致）で、外形も **340×182px → 340×236px** と縦に伸びる——**説明は場所を取る**。そして規則は、画面が一度も口を開かないまま実演される: `足す` で空きの点が **5 → 6 個**に増えたとき、既にある5個の `left` 差分は**すべて 0.000px**（増えたのは週6の1個・`left` 322px だけ）で、**さっき何も起きなかった週6に、今度は置ける**（履歴 **0 → 1**、空き **6 → 5**、予定チップ **0 → 1 個**）。**同じ操作の結果だけが変わることが、規則の説明になっている。** 実装が新しい罠を1つ掘り当てた。**ボタンの `hover: translateY(-1px)` が `getBoundingClientRect` に 1.000px のニセの差分を作り、C2（同一操作の前後が完全一致）が偽陽性で落ちる。** 手本にした No.146 や No.143 のボタンは同じ transform を持っているが、あちらには「操作前後で座標不変」を測る条件が無かったので問題化していなかった。**測る条件を足すと、それまで無害だった演出が壊れる側に回る。**',
    Component: PromiseNotYetKnown,
  },
  {
    id: 'plan-too-large-for-a-slot',
    no: 148,
    nameJa: '1週ぶんでは足りない予定',
    nameEn: 'Plan Too Large for a Slot',
    category: 'ゲーム',
    trigger: '粒の数が違う予定を選んで `置く`。足りなければ何も起きない（右上で「足りないと予告＋一部だけ置く」対照に切り替え）',
    principles: ['量は幅ではなく個数で言う', '続くことと重いことは消し方が分ける', '一部だけ置かない'],
    ecology:
      'No.144 は「空きは**誰の**かを言えない」で終わったが、実は**いくつぶんか**も言えていない。空きは週ごとに **0/1 の二値**でしかなく、No.146 はその上に「原資が届く範囲」を載せた。**146 種を支えてきた二値の担体が、量に当たって初めて足りなくなる。** 素直な答えは「大きい予定を幅で描く」だが、**横幅はこの語彙圏で既に三重に埋まっている**——No.138 が占有長＝合計に、No.143 が幅＝いつ来るかが一点に定まっていないことに、No.145 が幅＝値が一点に定まっていないことに使った。**四重目は取らない。** 既定の答えは No.88「量は個数へ翻訳する」を借りることで、**予定チップは、必要な空きの個数ぶんの「粒」を持つ**。実測で、粒と空きの点は **6×6px**・`border-radius` **50%**・`background-color` **rgba(0,0,0,0)**・`border-color` **rgb(179,179,179)** まで一致し、**隣との間隔も 24px / 24px で完全に同値**——実装は数値を揃えたのではなく、**同じ 1 つの CSS クラスを `data-role` で出し分けている**ので、構造として揃う。**必要な数と在る数が、同じ物差しで読める。** 第2の芯は**続くことと重いことの撃ち分け**である。No.143 の稼働（続くもの）は**隣り合う空きしか消せない**が、重いものは**どの空きでも消せる**——実測で粒3個の `B` は週 **2 / 4 / 6** に**飛び飛びで**載り（空きの点 **4 → 1 個**）、それでも残った空きの `left` 差分は **0.000px**（点は動かず、詰め直しもしない）。**同じ「3個消す」でも、連続を要求するかどうかが、続くことと重いことを分ける。** 新しい担体は1つも足していない。第3は**一部だけ置かないこと**である。量が入ると「一部だけ置ける」が生まれ、No.141〜144 の「移動先が埋まっていたら何も起きない」が初めて足りなくなる——だが**一部だけ置くのは画面がやってよいことではない**（読み手が頼んだのは「この予定を置く」であって「置けるだけ置く」ではない。No.69 の楽観の訂正と同じ穴）。実測で、空きが1個しかない状態で粒2個の `C` を置こうとすると、ページ内**全 204 要素**の差分が **0.000px**・スタイル差 **0 件**・履歴 **±0**。`置く` は `disabled` にならない（全操作で **0 回**）。第4は**足りないことをいつ言うか**で、答えは**言わない**——触る前でも後でもなく、**必要な個数と在る個数が、最初から同じ画面に、同じ大きさで並んでいる**。読み手が数えれば分かる。**画面は数えない。** 既定は `animation-name` **0 個**・警告色 **0 箇所**（対照は **1 個 / 3 箇所**）。そして規則は説明されないまま実演される: `足す` で空きが **1 → 2 個**になると、さっき何も起きなかった `C` が**置ける**（空き **2 → 0**、履歴 **+1**、手元 **1 → 0 枚**）。**配線側（企画）が実物を見て1つ直した。** 初版のキャプションは「空きの点が足りなければ、何も起きない」と**規則そのものを文章で名乗っていた**。数値条件 C1〜C8 は全項目通っていたのに、である。**動きで言うと決めた主張は、文章を1行足すだけで無効になる**（No.118 でまったく同じ直しをしている）。実装が報告した罠も1つ: **「同じ物差し」を値の一致で作ろうとするとサブピクセルで揺れる。同じクラスを共有させると、揃うことが構造で保証される。**',
    Component: PlanTooLargeForASlot,
  },
  {
    id: 'partly-done',
    no: 149,
    ...
    nameEn: 'Partly Done',
    category: 'ゲーム',
    trigger: '原資が足りないまま `払う` を押す。一部だけ済む（右上で「一部成功バッジ＋全部戻す」対照に切り替え）',
    principles: ['1回の操作は1つの点', '済んだかは残りが言う', '部分は宣言ではなく状態'],
    ecology:
  ...
  },
  {
    ...
    ecology:
      'No.147 は「無反応は、それ自体が定義の提示である」まで押し切った。だがそこまで来ると**無反応が二義的になる**——規則による無反応（この週には置けない）と、操作が届いていない無反応（回線が落ちている）が、**画面上まったく同じ絵**になる。しかも読み手の次の一手は正反対で、規則なら**諦める**、届いていないなら**待つ／押し直す**。素直な答え（分けるために何かを出す）は No.147 の縛りが自分に返ってきて封じられ、薄さ（＝確度）・消滅（＝やめた）・色（＝警告）は**すべて別の意味に予約済み**である。既定の答えは、**片方を不在から取り出すこと**だった——**届いていないことは、不在ではない。** 受け取っていないものを「何も起きなかった」と描くのが誤りで、届いていない操作は消えたのではなく**まだ渡っていない**。実測で、規則の無反応は本当に何も起こさない（空きの無い週で `置く` → ページ内**全要素**の `left`/`top`/`width`/`height` の最大差分 **0.00px**、要素数 **42 = 42**、履歴 **±0**、粒 **0 個**）のに対し、回線が落ちているときは**「操作」の行に粒が 1 個**増え、それ以外の全ロール（目盛り・予定・空き・現在地・履歴・ボタン群）の差分は **0.00px**、空きの点 **3 → 3**、履歴 **0 → 0**。**2 つの無反応の差は、要素が 1 個あるかどうかだけ**である（既定にこの 2 つを見分ける色・線種・クラスは **0 件**）。**新しい担体は足していない**——未達の粒は No.148 / No.149 の粒とまったく同じ 6×6 の輪郭の点で、computed style の distinct は **1 通り**（`6px|6px|rgba(0,0,0,0)|rgb(179,179,179)|solid`）、届けばそのまま履歴の塗り点になる（回線を戻すと粒 **3 → 0**・履歴 **+3**、履歴点の distinct も **1 通り**）。**説明ではなく、操作そのものが在る。** 「まだ」と「ずっと」は閾値で分けない（この図鑑は閾値を持たない）——粒は**押した週に留まり**、現在地だけが離れていく（`次の週へ` ×2 で縦線 **639.00 → 699.00px**、粒は **592 / 622 / 652.00px** のまま差分 **0.00px**）。**待っている時間は、定規が既に言っている。** **企画の誤りを実測が1つ訂正した。** 台本は「2回押せば2つとも届く（＝二重に届く）」と書いていたが、**空きは週ごとに 1 個**なので、同じ週への3連打は粒 **3 個**になった上で、届くのは **1 個だけ**である（実測: 粒 **3 → 0**、空き **3 → 2**、履歴 **+1**、予定 **+1**）。**束ねないことの意味は「2回届く」ではなく「2回とも判定される」ことだった**——2回目以降は届いた上で何も起きないので、履歴に書くべき出来事がそもそも無い。実装が踏んだ罠も1つ: **中割りの決済を待たずに測ると、測定そのものが壊れる**（`次の週へ` の直後は縦線が 0.28s の遷移中で、本当は 0.00px のはずの差分が **30.14px** と出る。280ms 以上待つ必要がある）。既定は説明の担体 **0 個**（凡例・吹き出し・トースト・バッジ・スピナー各 0）、禁止語 10 語すべて **0 件**、警告色 **0 箇所**、`置く` の `disabled` **0 回**、外形 **340×213px**。対照（無反応を説明で分ける）は最大 **340×240px** に伸び、**連打を束ねた結果、回線復帰後に 1 件しか届かない**（読み手が何回押したかが消える）。そして対照にだけ `cRuleBlocked = !cSlot.includes(cMarker)` という**規則を描画時に問い合わせる依存**が生まれた——既定ではこの式はイベントハンドラの中にしか無い。**説明で分けるとは、画面が規則を知っていることにする、ということである。**',
  ...
  },
  {
    ...
    ecology:
      'No.147〜149 は「量は幅ではなく個数で言う」（No.88 の継承）で3種を通した。だが**個数は整数しか持てない**——1粒に満たない量は **0 個**としか言えず、**何も無いことと、少しあることが、同じ絵になる**。ここでも読み手の次の一手は正反対で、本当に 0 なら**諦める**、端数が在るなら**待てば貯まる**。逃げ道は全部塞がっている: 粒を半分描くのは No.149 が「塗りの量を担体にしない」で封じたばかり、小さく描くのは No.148 の「粒と空きの点が同じ大きさであること」を壊す、単位を割れば**全標本の粒の意味が変わり**、数字や％は「文章で言う」に当たる。既定の答えは、**満たない量を粒にせず、場所として置くこと**だった。この図鑑は既に **輪郭＝これから／場所**、**塗り＝満ちた／済んだ** の2値を持っている（No.122 / No.148 / No.149）ので、端数は**次の 1 粒が満ちる場所**としてなら、いま在る語彙だけで置ける。実測で、0 のときは点が **0 個**、1粒に満たない量が入ると**輪郭の点が 1 個**現れ（塗り ±0、他の全静的要素の差分 **0.000px**）、**さらに入れても絵は 1px も変わらない**（内部の `data-amount` は **0.4 → 0.8** と動くのに、輪郭の `left` は **160.000px → 160.000px**、静的要素の差分 **0.000px**、個数 ±0）。**どれだけ溜まったかは言わない。言うのは、溜まりはじめたことだけ。** 端数の量が違っても輪郭の computed style は distinct **1 通り**である。そして**粒は湧かない**——1 粒を超える瞬間、**それまで輪郭だった点がその場で塗りに変わり**（同じ `left` **160.000px**、差分 **0.000px**）、新しい輪郭が右隣の **170.000px** に 1 個現れる。**場所が先に在るので、読み手が何も足していないのに増えた、という読み違えが起きない**（No.114 の「動かした時点で起きたことになる」を、位置を先に置くことで回避している）。塗りの点と輪郭の点は `width` **6px** / `height` **6px** / `border-radius` **50%** / `border-width` **1px** / 隣との間隔 **10.000px** まで一致し、違うのは `background-color` と `border-color` **だけ**。**企画の誤りを実装が2つ捕まえた。** ひとつ、受け入れ条件が挙げた内部値 **0.9 は原理的に到達できない**（+0.4 刻みなので端数は `{0, 4, 8, 2, 6}` の巡回しか取らない）——**条件に書いた数値が、その標本の規則では起こらないことがある**。ふたつ、**「場所の位置をどう決めるか」を企画が書いていなかった**ため、素直に「輪郭の index = いま払える粒数」と実装すると、`払う` のたびに場所が**左へ動いてしまい**、条件（払っても端数の場所は動かない）に落ちる。解は `formed`（満ちた総数・減らない）と `spent`（払った総数・減らない）に分け、場所を常に `index = formed` に固定すること——結果として**払われるのはいちばん古い粒**になり、**減るのは左端から**になった（先行標本の「減るのは右端から」と向きが逆になったが、位置の不変はこちらでのみ構造的に保証される）。実装からの発見がもう2つ: **浮動小数点の誤差は、ちょうど「1粒になる瞬間」の真上で起きる**（`0.4+0.4+0.4 = 1.2000000000000002`）ので内部は10倍した整数で持つ必要があること、そして **`払う` が `formed` を減らさない設計にすると上限を置かない限り点がレールと外形を突破する**こと（`FORMED_CAP = 9` で頭打ちにし、40 連打でも外形 **340×190px** のまま）。既定は `animation-name` **0 個**・警告色 **0 箇所**・担体の `transition-duration` **0s**・3 ボタンとも `disabled` **0 回**・禁止語 **0 件**・画面上の小数点 **検出なし**、外形 **340×190px**。対照（端数を量として描く）はバッジ表示時 **340×215px** で、**物差しが 2 つになる**（点の個数と塗りの量）うえ、**1 粒になる瞬間に大きさが変わって「湧いた」ように見える**（端数の点は 0.8 で幅 **≈5.6px** まで育ち、1.0 を跨ぐと **6px** の塗り点に切り替わって右に **≈2.8px** の小さな点が現れる）。',
  ...
  },
  {
    ...
    ecology:
      'No.144 は「**空きは理由を持たない**」と決め、No.146 はその限界を承知で使い切った。この標本は、**その限界が実際に噛む場所**を撃つ。定規の上で空きの点が無い週は **2 か所**にある——**現在地より手前**（もう過ぎた。二度と置けない）と、**原資が届いていない先**（まだ届かない。待てば置ける）。どちらも「点が 0 個」で、**絵は同じ、次の一手は正反対**である。過ぎた週を塗る・破線にする・薄くするは**全部すでに別の意味に予約済み**で、しかも空きの点が 3 値になると No.148 の「同じ物差しで数える」が壊れる。`置く` を `disabled` にするのは No.122 / No.147 が封じている。既定の答えは、**不在に理由は書けない。書けるのは、不在の「場所」の側である**——定規には既に**現在地の縦線**が在る（No.119 が「時間がやったことは定規に」と決めたときの担体）ので、「もう」は縦線の**左**、「まだ」は**右**。**新しい担体は 0 個。** 実測で、週2（手前）と週7（原資の先）で `置く` を押すと、**どちらもページ内全要素の差分 0.000px**・履歴 **±0**・要素数 **37 = 37** であり、さらに**2 つの試行の事後スナップショットが互いに完全一致**する。空きの点の `className` は distinct **1 種**、属性セットも distinct **1 種**、computed style も **1 種**——**左右を見分ける手がかりは 0 件**である。第2の芯は、**同じ列の両端が、別の主語に動かされる**ことだった。`足す` は**右端**に 1 個足し（**3 → 4 個**、既存の点の `left` 差分すべて **0.000px**、縦線も **0.000px**）、`次の週へ` は**左端**を 1 個削る（**3 → 2 個**、残った点の差分すべて **0.000px**、**動いた要素は現在地の縦線だけ・30.00px**）。**右は原資が伸ばし、左は時間が削る。** そして削られた側は戻らない——`次の週へ` ×2 のあと `足す` ×3 を押しても、点が在る週は **[5, 6, 7, 8]** で**最小値は現在地のまま**、増えるのは右端だけである（No.116 の不可逆を、履歴ではなく定規の上で再演している）。第3の芯は、**意味が変わっても、絵は変わらない**こと。原資が届く前に現在地がある週を越えると、その週は「まだ」から「もう」に変わるが、**その瞬間に動いたのは縦線の 30.00px だけ**で、まわりの要素の差分は **0.000px**（点はもともと無いので、動かしようがない）。**不在は、意味が変わっても不在のままである。** **企画の誤りを実装が1つ名指しした**——受け入れ条件の数値例が、仕様自身が定めた「縦線は週の左端に置く（現在地の週は『これから』側）」と噛み合っておらず、週が 1 つずれていた（現在地 5 → 6 では週6 はまだ「これから」側で、「もう」に変わるのは 6 → 7）。**実装は条件どうしの整合を取って、正しい週で実測している。** 実装からの発見が2つ: **選択の解除タイミングが、条件の成立そのものだった**（`置く` の成否によらず選択を解除しないと、週2 と週7 で選択の印の位置が違うので「2 試行が完全一致」が成立しない）。そして**選択の印を背景色だけで作ると、収録ステージの地色と同じ `#eaeae8` なので画面から消える**（数値は全部通るのに絵が壊れる典型。枠線を足して解いた）。**配線側（企画）が目視で1つ外した。** 操作行に `週 N` の読み取り値が置かれていた——C4 は**位置と大きさだけ**を見ているので、**中身だけが変わるテキストは 0.000px で素通りする**。この標本の芯は「変わるのは現在地だけ」であって「現在地を 2 か所で言う」ではない。既定は禁止語 **0 件**・警告色 **0 箇所**・`animation-name` **0 個**・`置く` の `disabled` **0 回**・縦線以外の `transition-duration` **0s**、外形 **340×188px**。対照（不在に理由を書く）は最大 **340×209px** で、**時間が進むほど画面が過去で埋まる**——斜線セルは **2 個 → 6 個**に増え、定規 270px に対する占有は **22% → 67%** になる。',
    Component: NotYetOrNoLonger,
  },
  {
    id: 'attempt-leaves-nothing',
    no: 153,
    nameJa: 'やってみたことが、残らない',
    nameEn: 'The Attempt Leaves Nothing',
    category: 'フィードバック',
    trigger: '同じ週で `置く` を3回押す。空きの無い週でも押す（右上で「試行を履歴に残す」対照に切り替え）',
    principles: [
      '試行は、起きたことの台帳には載らない',
      '「残らない」は、担体が消えることではなく、列に入らないことで言う',
      'やってみて何も起きなかったことは、失敗ではなく保留である',
    ],
    ecology:
      'No.150 の**実測が開けた穴**。あの標本は「届いていない操作」を粒として不在から取り出したが、実測すると同じ週への3連打は**粒が 3 個になった上で届くのは 1 個だけ**だった。残る 2 個は**届いた上で規則が何も起こさなかった操作**で、粒が消えるだけで跡を持たない——**3回押したのに、画面には1回ぶんしか無い。** 素直な答えは全部塞がっている: 全部履歴に残すと**履歴が試行で埋まり**（No.91）連打を咎める画面になる。失敗として残すと No.122 が**外の出来事**に割り当てた担体（輪郭だけの点）を規則の側に流用することになり、しかも「失敗の履歴」は**画面が規則を知っている**状態（No.147 / No.150 が封じた）に半歩戻る。第1の芯は、**試行は起きたことの台帳には載らない**。空きのある週で `置く` を3回押すと、履歴は **+1**（1個）・予定チップ **1 個**・粒 **0 個**で、**2回目と3回目の前後はページ内全要素の差分 0.000px・要素数 206 = 206**（配線側の独立実測でも同じ）。第2の芯は、**やってみたことは押した場所に残る。ただし個数は数えない**。空きの無い週で押すと**粒が1個**立ち、そこへ何回押しても**粒は 1 個のまま**（連打の回数は個数でも振幅でも語らない）。第3の芯が主題で、**「残らない」は担体が消えることではなく、列に入らないことで言う**。起きた操作の粒はそのまま履歴の列へ移り、起きなかった操作の粒は**押した週に留まる**——実測で、現在地が2週進んでも粒の `left` は **202.000px / 352.000px** のまま **0.000px 不変**、動いたのは縦線だけ（**220 → 280px**）。第4の芯は、**やってみて何も起きなかったことは、失敗ではなく保留である**。原資の外（週7）に留まった粒は、`足す` で原資が週7に届いた瞬間に**列に入る**（粒 **2 → 1**、履歴 **0 → 1**、予定チップ **+1**、既存の履歴点は **0.000px**）。**そしてこの標本は、No.150 が割ったものを意図的に混ぜている。** 未達の粒（回線）と空振りの粒（規則）は computed style が **distinct 1 通り**（`6px｜6px｜rgba(0,0,0,0)｜rgb(179,179,179)｜solid｜50%`）で、空きの点とも一致する。根拠は No.150 自身の収穫の逆向きの系——**ゼロの二義性は次の一手を分けるところで初めて問題になるので、次の一手が同じなら分けなくてよい**。ただし**留まる粒には、次の一手が違う2種類がある**: 現在地より手前（過ぎた週）の粒は**もう列に入れず**、原資の先の粒は**待てば入る**。ここで効いているのが No.152 の答えで、**その2つを分けているのは粒の見た目ではなく、現在地の縦線に対して左に在るか右に在るか**である（実測で縦線 280px に対し **202px（左） / 352px（右）**）。**担体を増やさずに済んだのは、152 が置いた縦線が、153 の二義性まで割ってくれたから**である。既定は `disabled` **0 回**・禁止語 **0 件**・警告色 **0 箇所**・`marker` 以外の `transition` / `animation` **0 件**、外形 **340 × 209px**（対照も同じ 340 × 209px）。対照（試行を履歴に残す）は履歴が **塗り点 ＋ 輪郭点 ＋ `×2` バッジ**に割れ、`この週には置けません` の文言が出る——**履歴が試行で埋まり、画面が規則を知っていることになる。** **この標本は、企画の誤りを5件あぶり出した**（詳細は README）。いちばん大きいのは、**既定の手順の字面と C1 が両立しないこと**（「粒が在れば何もしない」だけでは、1回目で予定が置かれた週への2回目の押下が新しい粒を生んでしまう）で、実装はガードを**予定が置かれた週にも広げて**解いている。実装が踏んだ罠が2つ: **`置く` のあとに選択を解除する先例（No.152）をそのまま踏襲すると、同じ週への連打が測れない**（2回目以降の対象が現在地の週へ落ちる）ので、この標本では選択は目盛りの再クリックでしか動かない。そして**対照の `×N` バッジを点の真上に置くと、トラックの `overflow-y: hidden` でほぼ全部クリップされる**——数値では通るが絵は空になる罠が、また別の形で出ている。',
    Component: AttemptLeavesNothing,
  },
  {
    id: 'paid-but-nothing-moved',
    no: 154,
    nameJa: '払ったのに、減らない',
    nameEn: 'Paid, But Nothing Moved',
    category: 'ゲーム',
    trigger: '1粒に満たない額を `払う` を2回続けて押す（右上で「端数を量として描く」対照に切り替え）',
    principles: [
      '減ったことは、原資の側ではなく、払った側が言う',
      '1粒に満たない量が在る場所は、向きを持たない',
      '効いた無反応と、効かない無反応は、履歴の点の個数だけで分かれる',
    ],
    ecology:
      'No.151「1粒に満たない」の**裏面**であり、実プロジェクト（1粒に満たない小口の支払い）からの逆算。151 は**入る側**の端数を「場所」として置き、**0.4 と 0.8 の絵は完全に一致する**（量を絵で言わない）と決めた。**その決定によって、出ていく側の端数を絵で言う手はもう塞がっている**——塗りを削る・小さくする・数字で書く・トーストで名乗る・`払う` を `disabled` にする、はすべて別の原則が封じている。しかも出る側の絵は No.150 の**規則による無反応とまったく同じ**になる。**三度目のゼロの二義性**だが、今度は向きが逆で、**何も起きていないのではなく、起きているのに見えない**。第1の芯は、**減ったことは、原資の側ではなく払った側が言う**である。1粒に満たない額を払うと、原資の右端が**塗りから輪郭に変わるだけ**で、点の総数は **3 = 3** のまま、他の点の `left` は **160.000px / 170.000px** と**差 0.000px**、輪郭の `left` **180.000px** は払う前に3個目の塗りが在った位置と**差 0.000px**。**動いたものは1つも無く、増えたのは履歴の点1個だけ**である。第2の芯は、**1粒に満たない量が在る場所は、向きを持たない**。151 と同じ絵の規則（塗り `floor(R)` 個 ＋ 端数があれば輪郭1個）をそのまま使うので、輪郭の点は「満ちかけ」でも「欠けかけ」でもない。実測で、塗りと輪郭は `6px`／`6px`／`border-radius 50%`／`border-width 1px`／ピッチ **10.000px** まで一致し、違うのは `background-color`（`rgb(61,61,61)` / `rgba(0,0,0,0)`）と `border-color`（`rgb(61,61,61)` / `rgb(179,179,179)`）**だけ**——No.151 の実値と完全に同じである。そして**払いと入りは、同じ絵の上を行き来する**: 1粒ぶん払い切って点が **3 → 2** に減った状態から `入る` を1回押すと、2回目の払いの直後の絵と**各点の `left` が差 0.000px で完全に一致**する。第3の芯が主題そのもので、**払っても絵が変わらないことがある**。2回目の払いの前後で、ページ内**全要素の差分 0.000px・要素数も同数**、変わったのは履歴の点が **1 → 2** になったことだけ（内部の残額は 2.6 → 2.2 と動いている）。第4の芯は、**二度払っても咎めない**こと。見えないので読み手はもう一度払うが、画面はそれを止めない——**防ぐのは規則の仕事で、画面の仕事は起きたことを残すこと**。そして**効いた無反応と、効かない無反応は履歴の点の個数だけで分かれる**: 残額が端数だけになった状態で `払う` を押すと絵は **0.000px** のまま履歴 **+1**、さらに押すと（足りないので）**全要素 0.000px・要素数 191 = 191・履歴 ±0**。**配線側が独立に測り直しても同じで**、7回払ったあとの8回目は点が `place@160.000px` のまま履歴 **7 → 7**、`disabled` は **0 回**である。既定は禁止語 **0 件**・数字を表示する要素 **0 個**・警告色 **0 箇所**・`animation-name` **0 個**・`transition-duration` は**全要素 0s**（この標本には現在地の縦線が無いので例外なし）、外形 **340 × 140px**。`入る` を 40 連打しても上限 9 点で頭打ちになり外形は **340 × 140px** のまま（No.151 の実装が点でレールを突破した罠の予防）。対照（端数を量として描く）は **340 × 167px** で、払った量は目で読めるようになる代わりに、**削れた粒を1個と数えるのか 0.6 個と数えるのかが決まらない**——No.148 で確立した「量は個数で言う」が壊れる。実装が2つ報告している: **パレットの逸脱は目視では気づけず、CSS を機械的に洗って見つかった**（ボタンの hover / active に規定外の中間色 `#545454` / `#262626` を作っていた）。そして**先例から継承すべきは点の実値であって、ボタンの動き方ではない**（No.151 のボタンは `transform` の transition を持っていたが、この回の共通則では例外なしで禁止）。',
    Component: PaidButNothingMoved,
  },
  {
    id: 'repeats-without-me',
    no: 155,
    nameJa: 'もう押していないのに、続いている',
    nameEn: 'It Repeats Without Me',
    category: 'ナビゲーション',
    trigger: '`始める` を1回押してから `次の週へ` を押し続ける（右上で「繰り返しを履歴に載せる」対照に切り替え）',
    principles: [
      '履歴に載るのは、読み手が押した回だけ。繰り返して起きたことは定規に載る',
      '続いていることは、未来側に何も置かないで言う',
      '続きが無いことは、担体が増えないことだけが言う',
    ],
    ecology:
      '実プロジェクト（毎週出ていく固定費）からの逆算で、企画の側でこの回の3つめとして起こした標本。No.153 と No.154 が「押したのに跡が足りない」方向だったのに対し、**残っているのは逆向き**——**押していないのに跡が増える**。読み手が一度「毎週払う」と指示すると、以後は週が進むたびに支払いが起きるので、**起きた回数は押した回数より多い**。No.119 はこの図鑑の台帳を2つに分けた（**読み手がやったことは履歴に、時間がやったことは定規に**）が、ここに来るのは**3つめの主語**である——**読み手が一度やった指示を、時間が代行して繰り返す。** どちらに載せても壊れる: 履歴に毎週の点を増やすと**読み手がやっていないことが読み手の台帳に混ざり**（No.98）、どちらにも載せないと本当に起きたことが画面のどこにも残らない（No.154 の失敗と同型）、未来側に「次はここ」と予告するのは No.114 が封じている。既定の答えは**主語で台帳を分けること**だった。第1の芯は、**履歴に載るのは読み手が押した回だけ**。`始める` で履歴 **+1**、以後 `次の週へ` を4回押しても履歴は **1 個のまま**である。第2の芯は、**繰り返して起きたことは定規に載る**。週送りのたびに通り過ぎた週へ塗りの粒が1個置かれ、粒は **1 → 5 個**（週2〜6）になる。このとき**動いたのは現在地の縦線だけ**で、各週送りの前後で**非マーカー要素の差分は 0 件（0.000px）**、縦線は毎回 **+30.000px**（合計 **+120.000px**）。既に置かれた粒は**増えるだけで1pxも動かない**。第3の芯は、**続いていることを未来側に何も置かないで言う**こと。現在地より先の週に属する担体は **0 個**（破線・影・輪郭・予告はすべて対照側にしか無い）で、禁止語 **0 件**（`毎週` `自動` `次回` `継続` `定期` `予定` ほか10語）・警告色 **0 箇所**・バッジ **0 個**。**続いているかを言っているのは、今週の粒が在るかどうかだけ**である。第4の芯は、**止めたことも読み手の操作なので履歴に載る**こと。`止める` で履歴 **+1**（2個）になり、既に置かれた粒は **5 個のまま・全数 0.000px**（1個も消えない）。そして**続きが無いことは、担体が増えないことだけが言う**——止めたあと同じ `次の週へ` を3回押しても、粒 **+0**・履歴 **±0**・**要素総数 36 = 36**・非マーカー差分 **0 件**で、動いたのは縦線の **+90.000px** だけ。**配線側が独立に測り直しても、動いた要素は縦線1個だけである。** 正直に書いておくと、履歴の点は distinct **1 通り**（`6px`／`6px`／`rgb(61,61,61)`／`border-style none`／`50%`、定規の粒と完全一致）なので、`始める` の点と `止める` の点は**見分けが付かない**。**台帳が2つあることの代償は、1つの問いに1つの台帳しか答えられないこと**で、「いま続いているか」は履歴ではなく定規の仕事である（`始める` を2回押しても2回目は履歴 **±0**・全要素 **0.000px** で、押した回数は履歴に出ない）。外形は既定 **340 × 161px**、対照（`毎週` バッジ・「次回: 週N」の文言・未来の破線セルが出た状態）**340 × 186px**。対照の代償は実測で出ている——`始める` → 週送り6回 → `止める` で履歴は **8 点**になり、**読み手が実際にやった操作はそのうち 2 点（25%）**しかない。**繰り返しを読み手の台帳に載せると、読み手がやったことが埋没する。** **配線側の目視で1件直した**: 履歴の点が履歴のラベルと**1行ずれて定規の側に寄っていた**（中心差 **-15.500px**。定規の粒との距離のほうが近く、点が定規のトラックに属して見えた）。原因は、行ラベルの共通クラスに書いた `grid-row: 2` が**別のグリッドの中のラベルにも効いていた**こと。**この標本の芯は「どちらの台帳に載っているか」なので、担体が載っている行が読めないと主張そのものが壊れる**——そして C1〜C8 はこれを1つも検出しなかった（どの条件も「行が合っているか」を見ていない）。直したあとの中心差は **0.000px**（途中、トラックとインナーの縦中央がずれた **-2.000px** の残差も潰している）。',
    Component: RepeatsWithoutMe,
  },

  /* ---------- No.156〜158「空いている週の理由は、ひとつではない」の回 ----------
     No.155 は代行（読み手が1回だけ指示した繰り返し）を**その週が始まった瞬間**に起こしたので、
     「いま続いているか」は今週の粒が在るかどうかだけで読めた。この回はその前提を外す——
     代行は**週が終わるときに**起きる。結果として現在地の週のセルは常に空になり、
     「空いた週」が同じ絵のまま複数の意味を持ちはじめる。 */
  {
    id: 'this-week-not-arrived',
    no: 156,
    nameJa: '今週ぶんは、まだ来ていない',
    nameEn: 'This Week Has Not Arrived',
    category: 'ナビゲーション',
    trigger: '`次の週へ` を2回押してから `始める` を押し、そのまま週を送り続ける（右上で「今週のセルに予告を置く」対照に切り替え）',
    principles: [
      'まだ来ていないことには、担体を1つも置かない',
      '空きの意味は、その週が縦線の左か現在地かだけが割る',
      '始めた直後の1週間、定規は「続いている」を言えない',
    ],
    ecology:
      'No.155 は毎週の代行を**その週が始まった瞬間**に起こしたので、「いま続いているか」は**今週の粒が在るかどうか**だけで読めた。この回はその前提を外す——実プロジェクトの週送りは**週が終わるときに**走る。粒が置かれるのは `次の週へ` を押した瞬間で、置かれる週は**出て行く週**（押す前の現在地）になる。帰結として**現在地の週のセルは構造上つねに空**になり、155 の読み方はそのまま使えない。今週のセルが空なのは (a) まだ今週ぶんが来ていない (b) そもそも動いていない、の2通りで、読み手の次の一手が違う（待つ／始める）。**四度目のゼロの二義性**である。しかもこの出来事は**どちらの台帳にも載らない**——読み手は押していないので履歴に無く、まだ起きていないので定規にも置けない（No.119 の2つの台帳に、初めて「どちらでもない出来事」が来た）。既定の答えは**担体を1つも置かないこと**だった。第1の芯は、**まだ来ていないことに担体を足さない**。`始める` の直後、現在地の週の担体は **0 個**、未来側（週5..9）も **0 個**で、増えたのは履歴の点 **1 個**だけ（`0 → 1`）。足した瞬間それは「これから来る」の予告になり、No.114 が封じた未来側の担体を現在地の空きに置いたのと同じことになる。第2の芯は、**空きの意味を割るのは縦線の左右だけ**であること。過ぎた空きの週（週2・週3）と現在地の空きの週（週7）で、目盛りの `background` / `animation-name` / `opacity` は**完全一致**し、粒はどちらも **0 個**——**UI は区別を1つも名乗っていない**。区別を作っているのは縦線の `left` **180.000px**（= `(7-1)*30`）が、週2の右端 **30.000px**・週3の右端 **60.000px** より大きいという**位置関係だけ**である（No.153 が置いた「留まる粒は縦線より左なら、もう列に入れない」の規則を、**不在**に広げた形）。第3は置かれる週で、粒は**出て行く週**に置かれる——現在地が週4のときに押すと、粒は **週4 に 1 個・週5 に 0 個**。`paid` へ書き込む経路が `handleNext` の1箇所しか無いので、C1・C3・C6 は分岐ではなく**構造で**保証されている。週送りのたびに動くのは縦線だけで、非マーカー要素の差分は **0.000px**・要素数 **21 = 21**・縦線 **+30.000px**。**この標本のいちばん正直な条件が C6 である。** 「週4 で `始める` を押す直前」と「押した直後」を突き合わせると、定規側は**要素数 14 = 14・全要素の差分 0.000px・外形差 0×0**で、**違いは履歴の点が 1 個増えたことだけ**だった。つまり**始めた直後の1週間、定規は「続いている」を言えない**——そのあいだ続きを言っているのは履歴の点 1 個で、しかも履歴の点は種類を名乗らない（No.149 / No.150）。**台帳を2つに分けたことの代償が、そのまま実測に出ている。** 共通則の点検は、禁止語 **0 件（18語）**・数字 **0 個**（週の目盛りを除く）・警告色 **0 箇所**・`disabled` **0 回**・`animation-name` が縦線以外すべて **`none`**・`transition-duration` が縦線以外すべて **`0s`**。`始める` を2回押しても履歴・粒・現在地は**すべて不変**（押した回数は履歴に出ない。No.155 の既存の決定）。外形は既定 **340 × 161px**、対照 **340 × 182px**。対照は「今週のセルに破線セルと `次回: 週7` を置く」「現在地に薄い粒（`opacity 0.35`）を先出しし、出て行った週にも薄い跡を残す」「`毎週の支払いは継続中です` と名乗る」の3つで壊れ、薄い跡は **6 個**まで積もる——**`standing` が生きていなかった週にも薄い粒が残るので、起きたことと起きなかったことが同じ見た目に潰れる**。**実装が2つ報告している。** ひとつは企画の誤りで、**C2（週送りの前後で要素総数が同数）と C3（同じ操作で粒が 1 個増える）は字面が衝突している**——実装は台本を辿って「C2 が指すのは `始める` より前の週送り」と読み、standing 中の遷移も別途測って**既存の粒はどちらの場合も 1px も動かない**ことを示した。もうひとつは**先例からの継承の罠**で、No.155 をなぞって `次の週へ` に `disabled` を付けてしまい、共通則（`disabled` 0 回）に反したので削除してハンドラ内のガードに変えている（**先例から継承すべきは実値であって、ボタンの振る舞いではない**——No.154 が同じことを別の形で報告している）。',
    Component: ThisWeekNotArrived,
  },
  {
    id: 'proxy-attempt-failed',
    no: 157,
    nameJa: '代わりにやったことが、できなかった',
    nameEn: 'The Proxy Could Not',
    category: 'ゲーム',
    trigger: '`足す` で原資を入れて `始める`、原資が尽きるまで `次の週へ` を押し続ける（右上で「落ちた週をスキップして警告を出す」対照に切り替え）',
    principles: [
      '起きなかったことは、起きたことと同じ場所に輪郭で置く',
      '代行の失敗は、読み手の台帳には載らない',
      '失敗は停止ではない。続きは、次の1回が起きることが事後に言う',
    ],
    ecology:
      '実プロジェクト（原資が足りなくて固定費が払えない週）からの逆算であり、No.155 の**3つめの主語の続き**。あの標本は「読み手が一度やった指示を、時間が代行して繰り返す」を台帳の分け方で解いたが、**代行が失敗する場合**を決めていなかった。この回の前提（代行は週が終わるときに起きる）を足すと、同じ「空いた週」が **3 通り**の意味を持つ——続いていて起きた／続いていたができなかった／そもそも動いていなかった。**二義ではなく三義**である。素直な答えは全部塞がっている: 失敗を読み手の履歴に載せると**読み手の失敗**に見え（No.98 / No.153）、定規に「起きたこと」として載せると嘘になり（起きていない）、色や点滅やトーストで演出すると伝わるのは「事故が起きた」という**評価**になる（No.118 の決定）。既定の答えは、**起きなかったことを、起きたことと同じ場所に輪郭で置くこと**だった。第1の芯は場所で、輪郭の粒と塗りの粒は**同じ週に置かれたときの `left` / `top` / `width` / `height` の差がすべて 0.000px**、`border-radius` も **`50%`** で一致し、**違うのは `background-color`（`rgb(61,61,61)` / `rgba(0,0,0,0)`）と `border-color`（`rgb(61,61,61)` / `rgb(179,179,179)`）だけ**である（塗りにも同じ 1px の border を持たせて、**箱として完全に同一**にしてある）。原資 0 での週送りは、その週に**輪郭 1 個・塗り 0 個**を置き、**履歴 ±0**（代行は読み手の操作ではない）・**原資 ±0**（払えていないので減らない）。第2の芯は、**その輪郭が「もう列に入らない」と読める理由**である。No.153 は「留まる粒は縦線より左なら、もう列に入れない／現在地なら、待てば入る」と決めた。この標本の輪郭は**必ず出て行く週に置かれる**ので、生まれた瞬間から過去にある——実測で輪郭の中心 **185.000px** に対し縦線 **200.000px**、次の週も **215.000px** に対し **230.000px**。**新しい担体も、新しい規則も要らなかった**（`ledger` へ書き込む関数が `handleNext` しか無いので、現在地の週に輪郭が生まれる経路が**コードに存在しない**）。第3は、**失敗は停止ではない**こと。原資を `足す` と、次の週送りで週7 に**塗りの粒**が戻る——**続いていたことは、次の1回が起きることが事後に言う**。このとき過去の輪郭は**塗りに変わらない**（`background-color` 不変・`left` 差 **0.000px**）。跡も動かない: 週5・週6 の輪郭は3手進めても **0.000px**・要素数同数。第4に、3つの絵が実際に別物であることを1つの定規の上で測っている——週2（塗り）**1 個** / 週5（輪郭）**1 個** / 週1（空）**0 個**。共通則の点検は、禁止語 **0 件（18語）**・数字 **0 個**（画面の文字列は週の目盛りとラベルとボタンだけ）・警告色 **0 箇所**・`disabled` **0 回**（**原資が 0 でも `次の週へ` は押せる**——押せない理由を `disabled` で語らない）・`animation-name` が縦線以外すべて **`none`**・`transition-duration` が縦線以外すべて **`0s`**。外形は既定・対照とも **340 × 206px**（台本の全状態で不変）。対照は「落ちた週をスキップして何も置かない」「赤い点滅と `支払いに失敗しました` のトースト（1800ms で消える）」「失敗を履歴に足す」の3つで壊れ、**トーストが消えたあとの絵は、最初から何も無かった週と同じ**に戻る（No.89 / No.119 の再演）。**この標本は、この回の共通則の表に穴を1つ見つけた。** 表は「現在地の輪郭＝受け付けた、まだ効いていない」という枠を持つが、**この標本ではそのマスへ至るコード経路が存在しない**（輪郭は必ず出て行く週にしか生まれない）——**同じ表の 6 マスを、1つの標本が全部は埋められない**。そのマスは No.158 が埋めることになった。計測側の罠も1つ出ている: **縦線の transition（0.28s）が終わる前に座標を取ると C3 の数値が破綻する**（実装の不具合ではなく待機不足。各操作後に 350ms 待って測り直した）。',
    Component: ProxyAttemptFailed,
  },
  {
    id: 'stop-takes-effect-next',
    no: 158,
    nameJa: '止めたのに、もう一度だけ来る',
    nameEn: 'Stop Takes Effect Next Week',
    category: 'ナビゲーション',
    trigger: '`始める` → `次の週へ` を数回 → `止める`。そのまま週を送る（右上で「即座に止まる」対照に切り替え）',
    principles: [
      '最後の1回は実際に起きるので、嘘をつかずに粒を置く',
      '受け付けたが効いていないことは、現在地の週の輪郭が言う',
      '履歴は「押した時点」ではなく「効いた時点」の台帳である',
    ],
    ecology:
  ...
  },
  {
    ...
    category: 'フィードバック',
    trigger: '`足す` → `置く` → `次の週へ`（原資が足りる）→ `置く` → `次の週へ`（原資が足りない）。そのまま週を送る（右上で「消して赤く知らせる」対照に切り替え）',
    principles: ['効いた週と取り残された週で、押すボタンも絵も同じ', '意味が変わったことは、担体ではなく縦線との位置関係が言う', '押したのに効かなかった操作は、履歴に載らない'],
    ecology:
      'No.156 が「空きの意味を割るのは縦線の左右だけ」と決め、No.158 が「現在地の輪郭＝受け付けた、まだ効いていない」と決めた。**両方を認めると、縦線が通り過ぎた瞬間、同じ輪郭が受理から「もう入らない」へ変わる**。この標本はその瞬間を撃つ。芯は3つ。ひとつ、**担体は1pxも動かない**。取り残しの瞬間、輪郭の `left` / `top` / `width` / `height` の差はすべて **0.000px**、`background-color`（`rgba(0,0,0,0)`）も `border-color`（`rgb(179,179,179)`）も不変で、**同じ窓で動いた要素は現在地の縦線 1 個だけ**（**+30.000px**）。要素の増減は **0**（`data-role` ＋ 週 ＋ 種別でキー化して突き合わせ）。**起きた出来事は「週が進んだ」だけで、保留それ自体には何も起きていない**（No.119 の継承）。ふたつ、**押したのに、読み手の台帳には何も残らない**。効いた週の `次の週へ` は履歴を **1 → 2（+1）** にするが、取り残された週の `次の週へ` は **2 → 2（±0）**——**押したボタンは同じ**である。No.158 の「履歴は効いた時点の台帳」の帰結であり、No.153「試行は起きたことの台帳には載らない」の系でもある。**この代償は隠さない。** みっつ、**割れないものを、割ったふりをしない**。取り残された輪郭（読み手の操作が効かないまま過ぎた）と、代行が試みて落ちた輪郭（No.157）は、`background-color` / `border-color` / `border-width`（`1px`）/ `border-radius`（`50%`）/ `width`（`6px`）/ `height`（`6px`）の6項目が **distinct 1 値で完全一致**し、中心の `y` も **0.000px** で一致する。**画面が言えるのは「その週には入らなかった」までで、主語（読み手か代行か）は言わない**（No.98 / No.153 の継承）。意味を割っているのは位置関係だけで、現在地に在るあいだは縦線 **360.000px** ≤ 輪郭中心 **375.000px** < **390px**、取り残された後は輪郭中心 **405.000px** < 縦線 **480.000px** になる。そして**取り残しの前後で輪郭は同一の DOM ノード**（印を付けて別操作を挟んでも `sameNode: true`）——**新しく現れたのではなく、そのまま在る**。過ぎた週は埋まらない（`足す` で原資が戻っても輪郭の差 **0.000px**・色不変）が、**新しい週には塗りが立つ**（週6 に `rgb(61,61,61)`）＝**失敗は停止ではない**（No.157 の継承）。対照（間に合わなかった保留を消して赤く知らせる）は同じ操作で輪郭が**削除**され、点滅セル（`rgba(179,58,58,0.16)`）とトースト（`rgb(179,58,58)`・文言は `間に合いませんでした`）が増える。壊れ方は3つとも実測に出た——消えるので**押していない週とまったく同じ絵**になり、赤い点滅は「事故が起きた」という**評価**になり（読み手は自分の操作が失敗したと読むが、実際には効かせる条件が揃わなかっただけである）、そして **1800ms 後には既定と同じ「禁止語 0 件・警告色 0 箇所」に戻る**——**壊れ方そのものが時間で消えるので、跡は何も残らない**。**実装が計測側の罠を1つ掘り当てた。**「未来側に担体を置かない」をピクセル比較（輪郭の `left` > 縦線の `left`）で測ると、**現在地ちょうどに在る輪郭を誤って未来側と判定しかねない**——縦線はセルの左端に、輪郭はセルの中央に立つので、現在地の輪郭は常に縦線より右にある。`data-week > data-current-week` という**意味の比較**に直して **0 個**を確認した。**この標本は「現在地＝許容」という規則を持つので、幾何で測ると規則のほうが落ちる。** 実装の判断も3つ効いている。代行の失敗は常設にせず**台本で1回だけ落とす**（常設にすると `置く` と代行が同じ原資を奪い合い、企画が1手ずつ明示した原資の推移が実装の内部順序に依存してしまう——企画の暗黙の前提どうしの矛盾で、実装コストの話ではない）。`置く` の二度押しは **`disabled` にせず、何も起きない**（共通則「同じ週に2個置かない」の帰結。実測でも粒は 1 個のまま）。`足す` だけは**即時に効く操作なので履歴 +1**（`置く` は解決待ちなので履歴に触れない）。外形は既定 **340×204px**、対照はトースト表示中で **340×235px**。塗りの粒が立つ週は既定・対照とも **[2, 6]** で完全一致＝主張が余計な差を作っていないことの担保である。',
  ...
  },
  {
    ...
    category: 'ゲーム',
    trigger: '`足す` で原資を満たして `始める`、`次の週へ` を押し続ける（週5から1週あたりの消費が静かに2倍になる。右上で「トーストで知らせて粒を大きくする」対照に切り替え）',
    principles: ['規則が変わったことは、どちらの台帳にも載らない', '量は場所で言い、変化は間隔で言う', '跡は遡って作れない。並べて残すだけで、比べるのは読み手がする'],
    ecology:
      '実プロジェクト（Startup Sim の、固定費の額が途中で変わる回）からの逆算であり、No.155「押した回数と、起きた回数は合わない」の**量の版**である。**1回の指示が、途中から別の意味になる。** 指示は出し直していない。粒も毎週1個・同じ大きさのまま（8週ぶんの粒の `width` **distinct [6]** / `height` **distinct [6]** / `background-color` **distinct [`rgb(61,61,61)`]**、個数の増分は毎回 **+1**）。変わったのは **1粒の重さ** で、画面のどこにもそれを言う担体が無い。逃げ道は全部塞がっている——量を粒の大きさで言うと No.151 / No.154 の「1粒＝1単位」が壊れ（大きい粒は「2回起きた」とも読める）、規則は読み手が押していないので履歴に載らず、起きた回数は変わっていないので定規にも載らない（No.153 が名指しした**3つめの主語**は台帳を持たない）。そして「速くなった」と言うには基準が要るが、この図鑑は方針の閾値を持たない（No.106）。既定の答えは、**画面は「変わった」と言わない。並べて残すだけ。比べるのは読み手がする**（No.115「良し悪しは読み手が決める」の系）。原資は**場所**で言い（No.151）、週送りのたびに**その回に動いたぶんの跡を、刻みとして帯の下に落とす**。実測で、帯の右端の1週あたりの移動は規則変更前が **distinct [-20px]**、変更後が **distinct [-40px]** で、帯そのものの `height`（**8px**）と `background-color`（**`rgb(140,140,140)`**）は**不変**。刻みのローカル座標は置かれた順に **200 / 180 / 160 / 140 / 100 / 60 / 20**、間隔は **distinct [20, 40]** で、**境目は規則が変わった週と一致する**。規則が変わった瞬間、履歴は **2 → 2（±0）**、既存の刻み4本の `left` 差分はすべて **0.000px**、**画面上のテキストは1文字も変わらない**——**跡は遡って作れない**ので、刻みは最初から1本ずつ置かれている。そして原資が尽きた週の粒は **輪郭 1 個**（`rgba(0,0,0,0)` / `1px solid rgb(179,179,179)` ＝ No.157 と**同一実値**。新しい担体は足していない）。**原因の担体が、結果の担体より後に読まれる**——読み手が帯を見に行くのは、たいていこの輪郭が出た週である。既定が「並べて残すだけ」で足りるのは、そのときに読めるものが最初から在るからで、**出来事として1回だけ語る実装は、この順序に間に合わない**。対照（トーストで知らせ、以後の粒を大きくする）はそこが3つとも壊れる——粒を大きくすると量を粒で言うことになり（**11px** の粒が **6px** の粒の隣に並ぶ）、トーストは **1800ms** で消えるので原資が尽きた週に見に行っても何も残っておらず、「変わった」を**出来事**として1回語るだけなので以後の週に何も残らない。実際に変わったのは**恒常的な規則**のほうである（No.118「外れは1回の事故ではなく性質」の系）。**実装が企画の誤りを3つ名指しした。** ひとつ、**C1 と C5 は企画の推奨実値のまま同一の台本では両立しない**——帯 220px・消費 20/40px・週5 で倍を8回の週送りに当てると必然的に週8 で原資が尽きて輪郭になり、C1 が要求する「8週ぶんの粒の `background-color` が distinct 1 値」は成り立たない。使い果たさない台本（C1 用）と企画推奨の台本（C2〜C5 用）に分けて解いた。ふたつ、**C2 の「新規要素 0 個」は字義どおりには成立しない**——毎週必ず粒1個と刻み1本が増える設計なので、規則変更の週も通常週と同じだけ要素が増える。字義どおり 0 個にすると芯4（跡は遡って作れない）と矛盾する。「規則変更の週に**専用の**新規要素が現れないこと」と読み替え、**通常週と同じ増分（ともに +2）**で裏を取った。みっつ、**C5 の「刻みの個数が週送りの回数と一致」も曖昧**——週送りは 8 回だが刻みは **7 本**（実際に原資が動いた回数）である。刻みは「動いた跡」という芯の定義に忠実な後者を採った。実装の判断も効いている。**刻みは帯の内側ではなく帯の下に落とす**——内側だと残量に応じて刻みが乗る背景（塗り／地）が変わり、「刻みが帯の地色に溶ける」事故が構造的に起きる。**規則変更は定数 1 個（`RULE_CHANGE_WEEK = 5`）と純関数 1 個だけで表し、`handleNext` に週5 専用の分岐を作らない**——毎週まったく同じコード経路を通ることが、そのまま「規則変更の週に専用の要素が現れない」ことの裏付けになる。**`足す` は +1 刻みではなく満杯まで埋める**（1回の押下で台本が組めるうえ、同じ実装のまま台本だけ変えて2つの条件を測り分けられる）。**実物を見て直した点が、この標本にも出た。** 数値条件（間隔 distinct 2 値）を全部通していたのに、**`width: 1px` の刻みは等倍ではアンチエイリアスでぼやけ、帯の下に何も無いように見えた**。3倍に拡大すれば綺麗に見えるので一度は見送りかけたが、**実際に読まれる解像度で確認し直して `1px → 2px`・高さ `6px → 7px` に上げた**（色と位置の計算は無変更）。**「拡大すれば読める」は、読めることの証明にならない。** 外形は既定 **340×191px**、対照はトースト表示中で **340×212px**。同じ台本を通したときの粒の週と種類は既定・対照で完全一致（週1〜7 が塗り、週8 が輪郭）。',
  ...
  },
  {
    ...
    category: 'フィードバック',
    trigger: '`始める` → `次の週へ` を数回 → `返事が届く`。週3の輪郭に塗りが重なる（右上で「光らせて差し替える」対照に切り替え）',
    principles: ['後から起きることと、後から分かることは別', '輪郭は消さない。塗りが、その中に入る', '置くのは「分かった週」ではなく「起きた週」'],
    ecology:
      'No.150 は「届いていないことは、不在ではない」と決め、No.157 は「起きなかったことは、起きたことと同じ場所に輪郭で置く」と決めた。両方を認めると、**返事が遅れている週は輪郭として置かれ、読み手はそれを「起きなかった」と読む**。そして後になって、実は起きていたという返事が届く。この標本は**過去の読み直し**を描く。まず先行標本との関係を正確にしておく必要がある。**No.157 が禁じたのは「後から起きること」**（過ぎた週は埋まらない＝時間の不可逆性）**であって、「後から分かること」ではない**。起きたのは週3、分かったのは週6以降——この標本は輪郭を書き換えず、**同じ週に塗りを追記するだけ**なので、157 の決定と衝突しない。芯は3つ。ひとつ、**増えるだけで、既存の粒は1pxも動かない**。届いた瞬間、既存 15 要素（目盛り 9・定規の粒 5・履歴の点 1）の座標差分は**すべて 0.000px**、消えた要素 **0 件**、新規要素 **1 件**、要素総数 **15 → 16**。**縦線も動かない**（`left` 差 **0.000px**）——**今週には何も起きていない**。履歴も **±0**（読み手は押していない）。ふたつ、**輪郭は消さない。塗りが、その中に入る**。中心の `x` / `y` の差は **0.000px**、塗りの外接矩形が輪郭からはみ出す量は上下左右すべて **0.000px**。輪郭 **12×12px**（枠 1px・内径 10px）の中に塗り **6×6px** が入り、内壁との隙間が各辺 **2px** 残る。**「起きていた」と「そう読めていなかった」が同居する**——輪郭は推測の跡ではなく**その時点の画面の状態**という事実なので、No.113「事実でないものの跡を残さない」には触れない。みっつ、**緩急を付けない**。現れた塗りを含め、縦線以外で `transition-duration` が `0s` でない要素 **0 件**、`animation-name` が `none` でない要素 **0 件**。**遅れて届いたのであって、いま起きたのではない**（No.121「出発点が画面の外にあるものは、動きで描けない」の継承）。最終状態では3つの絵が同じ定規に並ぶ——通常の週（塗り **1**・輪郭 **0**）、**返事が来ないまま終わった週**（塗り **0**・輪郭 **1**）、**遅れて届いた週**（塗り **1**・輪郭 **1**）。週3 が過去になってから届かせても同心は成立し（中心差 **0.000px**）、届いたあと週を2つ進めても座標差分は **0.000px** のまま（**時間では消えない**。No.89 に耐える）。対照（光らせて差し替え、トーストで知らせる）は同じ場面で週3 が **塗り 1・輪郭 0** になり、**最初から確定していた週1 とまったく同じ絵**に潰れる——**遅れて確定した事実が、画面から見分けられなくなる**。トーストは `rgb(179,58,58)` で **1800ms** で消えるので、**見ていなければ何も残らない**。**実装が企画の穴を2つ埋め、うち1つは企画の誤りを名指しした。** ひとつめは**同心の寸法**——6px 同士を重ねると枠が塗りにほぼ隠れて「輪郭が立ったままだ」が読めない。この標本にかぎり輪郭を **12px** に拡げ、**標本内のすべての輪郭（遅れて届いた週も、来なかった週も）を同じ 12px に統一**した。ここで実装が正直に指摘している——**共通則は輪郭の実値（6px）を「完全一致」と定めており、「新しい担体を足していない」と「この標本だけ実値を変える」は字面でぶつかる**。担体の**種類**は増えていないが、**共通則が固定した実値はこの標本だけ変わっている**。企画書に例外として明記すべきだった。ふたつめ、`返事が届く` は **`disabled` にせず何度でも押せる**——返事待ちを FIFO のキューに積み、押すたび先頭を 1 件だけ解決し、空なら何も起きない。そして **`返事が届く` は履歴を動かさない**。読み手の指示ではあるが、動かすのは過去週の記録であって**今週の出来事ではない**。**「いつ分かったか」ではなく「いつ起きたか」の場所に置く**、がこの標本の決定である。目視でも1つ分かった。**同心は等倍のスクリーンショットでは差が読み取りにくく、4倍に拡大して初めて確証が持てた**——塗り 6px・輪郭 12px・枠 1px・隙間 2px の組み合わせが、通常の塗りともただの輪郭とも一目で別物に読めることを、拡大画像で確認している。外形は既定 **340×161px**、対照 **340×192px**。塗りが立つ週の集合は既定・対照とも **[1,2,3,4,5,6]** で完全一致。',
    Component: LateArrivalInThePast,
  },
  {
    id: 'outline-means-three-things',
    no: 162,
    nameJa: '同じ輪郭が、三つのことを言っている',
    nameEn: 'Outline Means Three Things',
    category: 'フィードバック',
    trigger: '原資を入れて `始める`、週を送る。返事待ちの週・入らなかった週・現在地の受理が同じ絵で3つ並ぶ（右上で「輪郭を3種類に割って凡例を出す」対照に切り替え）',
    principles: ['割らない。輪郭は1種類のまま置く', '区別は時間だけがする', '待てば分かる、とも言わない'],
    ecology:
      'この語彙圏は輪郭に**三つの意味**を割り当ててしまった——**受け付けたがまだ効いていない**（No.158、現在地の週）、**その週には入らなかった**（No.157 / No.159、縦線の左）、**返事がまだ来ていない**（No.161、これも縦線の左）。No.156 が「空きの意味を割るのは縦線の左右だけ」と決めているので、**後ろの2つは割る軸を持たない**。輪郭を2種類にするのは共通則（担体は塗り・輪郭・無の3値）を壊し、No.151 以降の13種の読み方に遡って効く。理由を文章で書くのは No.152（不在に理由は書けない）と No.150（待てとも諦めろとも言わない）が封じている。**画面はこの区別を、いま言えない。** 既定の答えは**割らないこと**である。週3（返事待ち）・週4（入らなかった）・週5（現在地の受理）の3つの輪郭は、`background-color` **rgba(0,0,0,0)** / `border-color` **rgb(179,179,179)** / `border-width` **1px** / `border-radius` **50%** / **12×12px** が **distinct 1 値で完全一致**し、中心 `y` は3つとも **139.500px**（差 **0.000px**）——**同じ絵が3つ並ぶ**。凡例 **0 個**・ラベル **0 個**・トースト **0 個**で、画面テキストに `受理` `入らなかった` `返事待ち` `遅れ` は **0 回**出ない（内部の `data-reason` は持つが、スタイルシートがこの属性を選択する規則は **0 件**）。第2の芯は**区別を時間だけがする**こと。`返事が届く` を押すと週3 に塗りが **1 件**追記され（消えた要素 **0 件**）、週4 の輪郭は `left` **259.000px → 259.000px**・色不変（差 **0.000px**）。`次の週へ` を押すと週5 の輪郭（12×12・枠 rgb(179,179,179)）が塗り（6×6・rgb(61,61,61)）に変わり、週3・週4 の差分は **0.000px**。**同じ絵から3つの別の結末が出てくる。** 第3は**待てば分かるとも言わない**ことで、最終フレームの週6（返事が来なかった）と週4（原理的に解決しない）の6項目は**完全一致**——**最後まで割れない**。既定は縦線以外に `transition-duration` ≠ `0s` / `animation-name` ≠ `none` が **0 件**、未来側の担体は全13フェーズで **0 個**（幾何ではなく `data-week > data-current-week` の意味で測っている。No.159 が掘り当てた罠の継承）。対照は逆に**割る**——輪郭が黄の点線（返事待ち）・赤の破線（入らなかった）・青灰の実線（受理）の **distinct 3 値**になり、凡例 **3 行**とラベル **3 個**が要る。担体が3値から5値に増え、**読み手は凡例を覚えないと定規が読めなくなる**。さらに「まだ返事が来ていません」のトーストが **1 個** → 1800ms 後 **0 個**。**実装が「数値条件を全部通したまま絵が壊れる」を実際に踏んだ。** 対照のラベルを素直に輪郭の横へ直付けしたところ、週3〜週5 は **30px ピッチ**しか離れていないため文字列が重なり、`返事待入...カ受理` と判読不能になった——**C1〜C6 はこの状態でも全項目通っていた**。スクリーンショットを目視して発見し、週番号付きの縦積みリストに設計変更している。企画の側の誤りも2つ出た。ひとつ、**返事待ちの週が原資を消費するかを企画が決めていなかった**（「原資 60・1週 20・週4 で尽きる」の3つは、待ちの週が消費しない解釈だと帳尻が合わない）。ふたつ、**(a) 受理の輪郭を見せるには、この標本だけ「落ちたら立ち消える」に変える必要がある**（No.157 は落ちても指示が生き続けるので、`始める` の再押下が潰れる）——この依存関係は企画書に書かれておらず、実装が自力で見つけて決めている。そして実装が皮肉な副作用を1つ名指しした: **`始める` というボタンのラベルが、この標本では「受理してまだ効かない指示」の意味になり、No.157 / 159 / 161 の「即座に効く指示」と違う効き方をする**。担体の多義を暴く標本が、**操作の側に新しい多義を1つ作っている。**',
    Component: OutlineMeansThreeThings,
  },
  {
    id: 'two-standing-orders',
    no: 163,
    nameJa: 'どちらの指示の粒か、言えない',
    nameEn: 'Two Standing Orders',
    category: 'ゲーム',
    trigger: '行の見出しを押して2本の指示を別々の週に始め、原資が細るまで週を送る。片方だけ落ちる週と両方落ちる週を見る（右上で「1行に粒を2個並べて種類を付ける」対照に切り替え）',
    principles: ['行を複製する。担体の種類は増やさない', '同じ行に塗りと輪郭を同居させない', '軸は増やさない。履歴は行を名乗らない'],
    ecology:
      '共通則「**1週1個**」は、指示が**1本**であることを前提にしていた。指示が2本立つと粒の個数が「起きた回数」を意味しなくなり（1個の粒が2回を意味しうる）、片方だけ落ちた週に塗りと輪郭を同じ場所へ置くと、**No.161 がその絵に「後から起きていたと分かった」を割り当てている**ので衝突する。既定の答えは**行を複製すること**——指示1本＝1行、「1週1個」は**1行あたりの規則**として読み替える。担体の種類は増やさない: 塗り（**6×6px / rgb(61,61,61)**）は2行で **distinct 1 値**、輪郭（**6×6px / rgba(0,0,0,0) / 1px solid rgb(179,179,179)**）も6項目が2行で完全一致、行の `height` 差 **0.000px**・`width` 差 **0.000px**・レールの地色は2行とも **rgb(214,214,211)**。**指示を名乗るのは行の見出しだけで、粒は名乗らない。** 全週・全行を通じて**1つのセルに担体が2個の週は 0 件**。第2の芯は**同じ行に塗りと輪郭を同居させない**ことで、片方だけ落ちた週（残 30・固定費 20 が通り積立 30 が落ちる）は A 行が塗り **1**・輪郭 **0**、B 行が塗り **0**・輪郭 **1**——**同居した週は 0 件**で、No.161 の「同居＝後から分かった」と撃ち分かれている。両方落ちた週（残 10）は両行に輪郭 **1** ずつ。実装は分岐でこれを作らず、**共有の原資プールを行A→行Bの順に直列で引く**ことで「先に引いたほうだけが通る」を素直に導いた。第3は**軸を増やさない**こと: 縦線は2行を貫く **1 個**（行ごとの縦線は 0 個）、週の目盛りも両行で共有する1組、履歴も **1 本**のまま。点は **2 個**（`始める` を押して効いた回数と一致）で、落ちた週の前後で **±0**、点の見た目は **distinct 1 値**＝**どちらの指示の点かを名乗らない**。この代償は隠していない——**どちらを始めたかは、履歴ではなく定規の側（粒が立ちはじめた行）が言う。** 既定は禁止語 **0 件**・赤 **0 箇所**・指示名（行見出しを除く）**0 回**、縦線以外に `transition-duration` ≠ `0s` / `animation-name` ≠ `none` が **0 件**、未来側の担体 **0 個**。外形 **340 × 207px**。対照は3つの壊れ方が同時に出る: **1行に粒を2個並べる**（週2〜7 の **6 週**で2個＝No.155 の語彙では「2回起きた」に読める）、**粒に種類を付ける**（`background-color` が **distinct 2 値**、凡例 **2 行**）、**履歴と文章で名乗る**（履歴ラベル **2 個**、`⚠ 積立が実行できませんでした` の赤いトーストが 1 個 → 1800ms 後 **0 個**。禁止語 1 → 0・赤 2 → 0 箇所）。**壊れ方3だけは時間で消える**ので、見ていなければ落ちたことが画面に残らない。実装が計測側の罠を1つ掘り当てた。**縦線の 0.28s の transition の途中で測ると、`lineX(週)` の理論値と 40px 以上ずれる**（週5 到達直後の実測 **244.45px** に対し理論値 **286px**。400ms 待つと一致）。**動く担体が1つだけ残っている標本では、その1つが測定の起点にも終点にもなる。** 企画の側の不足も2つ名指しされた。ひとつ、**2本の指示をどこから始めるか**を企画が決めていなかった——`始める` ボタンをテキストで区別すると指示名が画面に増えて C6 に抵触するので、実装は**行見出しそのものをボタンにする**ことで辻褄を合わせている。ふたつ、**「片方だけ落ちる残高」に至る原資の台本を企画が示していなかった**ので、`足す` を +20 刻みにして回数で帳尻を合わせる試行錯誤が要った。',
    Component: TwoStandingOrders,
  },
  {
    id: 'undone-after-the-fact',
    no: 164,
    nameJa: '起きたことが、起きていなかったことになる',
    nameEn: 'Undone After the Fact',
    category: 'フィードバック',
    trigger: '`始める` で週を送り、塗りが並んだところで `取り消しが届く` を押す。過去の週は動かず、現在地に粒が1個立って帯だけが逆向きに戻る（右上で「過去の塗りを輪郭に差し替えて赤く知らせる」対照に切り替え）',
    principles: ['取り消された週は書き換えない。取り消しは現在地に置く', '取り消しを名乗らない。違うのは向きだけ', '取り消しは定規に載り、履歴には載らない'],
    ecology:
      'No.161 は「**後から分かること**」を描いた——過去の週に輪郭が立っていて、遅れて届いた返事が同じ週に塗りを同心させる。**その時間反転**を、この図鑑は一度も描いていない。すでに塗りが立っている週について、後から「あれは起きていなかった」と分かる場合である。逃げ道は全部塞がっている: 塗りを輪郭に書き換えるのは輪郭の**四義目**（No.162 が三義で限界だと測った直後）であり、No.157（過ぎた週は書き換わらない）と追記オンリーの台帳を破る。斜線・×・薄さは担体の値を増やし（薄さは No.74 / No.114 で確度に使用済み）、消すのは No.160「跡は遡って作れない」に触れる。既定の答えは、**取り消しを訂正ではなく出来事として扱う**こと——**画面は「なかったこと」にしない。「なかったことになった」という出来事を足す。** 取り消しが届いた瞬間、週3 の塗りは矩形差分 `dx` / `dy` / `dw` / `dh` が**すべて 0.000px**、`background-color` は **rgb(61,61,61)** のまま不変、既存 **7 要素**の座標差分も**すべて 0.000px**、消えた要素 **0 件**・新規 **1 件**。その1件は**現在地に立つ**（`data-week` **6** ＝ `data-current-week` **6**）——**同じ「後から」でも、事実の追加は過去の座標へ、事実の否定は現在の座標へ。** 過去週に増えた要素は **0 件**、未来側の担体も **0 個**。第2の芯は**名乗らないこと**で、取り消しの粒と普通の粒は `width` **6px** / `height` **6px** / `background-color` **rgb(61,61,61)** / `border-width` **0px** / `border-color` / `border-radius` **50%** が **distinct 1 値で完全一致**する（内部の `data-undo-of` を CSS は参照しない）。**違うのは向きだけ**である: 原資の帯は普段 20px ずつ縮むが、取り消しの週だけ **0.000px → 20.000px** と**伸びる**。刻みは **2px 幅 / 7px 高 / rgb(61,61,61)** の distinct 1 値、間隔も **20.000px** で同値で、値の列は **80 / 60 / 40 / 20 / 0 / 20** ——最後の 20 が戻りである。**向き**という軸は、この語彙圏（No.149〜163）が一度も使っていなかった最後の余地で、担体を1つも増やさずに意味を1つ足せた（No.55「向き＝事実、濃さ＝評価」の、13種ぶり後の再利用）。第3は台帳で、取り消しは**定規に載り、履歴には載らない**——読み手は押していないので履歴は **1 → 1（±0）**。既定は禁止語 **0 回**・赤 **0 箇所**で、縦線（0.28s）以外に `transition-duration` ≠ `0s` / `animation-name` ≠ `none` の要素は **0 件**。そして**2つの出来事として残る**: 週を2つ進めても週3 の塗り **1 個**と取り消しの粒 **1 個**はどちらも在り、戻った 20px は実際に次の週送りを動かして塗りを **1 個**立てる（**合計は合うのに、跡は2つ残る**）。対照は逆に**過去を書き換える**——週3 の塗りが **1 → 0 個**・輪郭が **0 → 1 個**に置換され、その上に斜線が **1 個**（`opacity` **0.4**）重なり、`⚠ 週3の計上が取り消されました` の赤いトースト（**rgb(179,58,58)**）が **1 個** → 1800ms 後 **0 個**、履歴は **1 → 2（+1）** で赤い点が残る。**書き換えた側では「最初から起きていなかった」ことになり、取り消しという出来事そのものが画面から消える**（週3 が No.157 の「原資が尽きて落ちた週」と同じ絵に潰れる）。一方で**読み手が押していない操作が、読み手の台帳には残る**——**消す場所と残す場所が、両方とも逆になっている。** 実装が新しい罠を1つ報告した。**ボタンが5個に増えると、先行標本（最大3個）の寸法では1行に収まらず、カードの高さが 223px まで膨らむ**（共通則の 160〜210px を超える）。padding と font-size を詰めて **186px** に収めている——**語彙が増えると操作が増え、操作が増えると寸法の約束が先に壊れる。** 企画の誤りも1つ名指しされた。**手順の週番号がオフバイワンだった**——企画は「取り消しの次の `次の週へ` で週7 の塗りが立つ」と書いたが、この語彙圏の先行4種（157 / 159 / 160 / 161）は一貫して「`次の週へ` は**押す前の現在地**について定規へ書き込む」規約なので、正しくは**週6**である。実装は先行標本の規約を維持して企画の記述ミスと判断し、主張（戻った原資が次の週送りを動かす）は週6 の塗りで裏取りした。**企画が規約を1つ読み落とすと、受け入れ条件のほうが先に嘘になる。**',
    Component: UndoneAfterTheFact,
  },
  {
    id: 'press-means-two-things',
    no: 165,
    nameJa: '同じボタンが、二つのことをする',
    nameEn: 'Press Means Two Things',
    category: 'フィードバック',
    trigger: '原資が足りる週と足りない週で、同じ `始める` を押す。押す前は同じ絵で、押した後に立つ担体が違う（右上で「押す前に予告してボタンを2つに割る」対照に切り替え）',
    principles: ['押す前に予告しない', '規則は書かない。規則の入力を見せる', '違いは、あとから立つ担体だけが言う'],
    ecology:
      'No.162 の実装が名指しした副作用そのものである——**担体の多義を暴いた標本が、操作の側に新しい多義を1つ作った**。No.157 / 159 / 161 の `始める` は即座に効く指示だが、No.162 の `始める` は「受け付けたがまだ効かない」指示になっていた。図鑑は担体の多義（162）と主語の不足（163）を測ったが、**操作そのものの多義**を一度も測っていない。この標本の `始める` は、原資が足りていれば**押した瞬間に効き**（現在地に塗り）、足りなければ**受け付けられて次の週に持ち越される**（現在地に輪郭）。**押す前に、どちらになるかは画面のどこにも書かれていない。** 逃げ道は塞がっている: 押す前に言えば予告になり（No.114 の担体を操作の側へ持ち込むことになり、しかも読み手の状態に依存するので毎フレーム更新される＝No.117 の鮮度）、ラベルを2つに割るのは No.162 が担体について却下した解（種類を増やす）を操作の側でやることであり、**この操作は試せない**（押せば効いてしまう。No.153「試行は台帳に載らない」は**何も起きなかった試行**の話である）。既定の答えは**予告しないこと**で、`始める` ボタンは①押す前②押した直後③原資が足りる週④足りない週の4状態で矩形差分が**すべて 0.000px**、`background-color` **rgb(61,61,61)** / `color` **rgb(245,245,243)** / `border-width` **0px** / `opacity` **1** が **distinct 1 値**、`disabled` 属性 **0 件**、予告要素 **0 個**、禁止語（`受け付` `持ち越` `足りません` `いま押すと`）**0 回**。第2の芯は**規則を書かない代わりに、規則の入力を残さず見せる**こと。帯の実測幅は `data-funds` と**差 0px**（初期 **100** / 最終 **10**）、1週ぶんの費用を表す刻みの間隔は **distinct 1 値（30px）**、閾値の線・数字・単位の要素は **0 個**。**画面は規則を書かないが、規則の入力は全部見えている**ので、読み手は「残りが刻み1つぶんに足りているか」を見比べて予測できる——台本の全6回の押下（即座 **3回** / 受理 **1回** / 何も起きない **2回**）で、**予測と実際が 6/6 一致**した。**ただし実装が、企画が入力を数え間違えていたことを実測で示した**: 「帯の残り ≥ 刻み間隔」の1変数だけでは、即座に効いた直後の2度押し（残り **70** ≥ **30** なのに結果は**無**）を誤予測する。**規則の入力は2つではなく3つ**で、3つめは「今週すでに担体が立っているか」——これも画面に在るので主張は保たれたが、**入力を全部見せると決めた企画が、入力の数を数え落としていた**。第3の芯は**違いをあとから立つ担体だけが言う**こと。受理の輪郭（週4）と、持ち越したがまだ足りなかった輪郭（週5）は `width` **6px** / `height` **6px** / `background-color` **rgba(0,0,0,0)** / `border-width` **1px** / `border-color` **rgb(179,179,179)** / `border-radius` **50%** が **distinct 1 値で完全一致**し、CSS が `data-week` / `data-kind` を選択する規則は **0 件**。週4 の輪郭はその後の週送りの前後で矩形差分が**すべて 0.000px**・色不変・消えた要素 **0 件**（追記オンリー＝No.157）で、新規は次の週に **1 個**。`足す` を挟んで持ち越しが**塗り**として立ったとき、はじめて「あの輪郭は受理だった」と読める——**現在地の輪郭は、次の週送りを待って初めて意味が確定する。** 代償は台帳に出る: 履歴の点は**即座に効いた押下の回数**（実測 **4**）と一致し、受理・持ち越しの解決・何も起きなかった押下では **±0**。**読み手が押して起こしたのに、読み手の台帳に残らない出来事が生まれる**（No.164 の「取り消しは定規に載り、履歴には載らない」と同型の非対称）。実装は輪郭に `reason` 型を持たせなかった——この標本の輪郭は分岐が1種類しかないので、**作り分けようがない設計の帰結として C4 が満たされる**。あわせて家風の割れを1つ名指しした: **`足す` を履歴に計上するかが No.163（しない）と No.162 / 164（する）で割れていた**ので、「即座に効いた押下」という定義に沿って計上する立場を採っている。既定は縦線以外に `transition-duration` ≠ `0s` / `animation-name` ≠ `none` の要素 **0 件**（要素総数 46 中）、赤 **0 箇所**、未来側の担体 **0 個**、外形 **340 × 177px**。対照は3つ同時に壊す——**押す前に予告する**（予告要素が常時 **1 個**。しかも同じ週の2度押しでは何も起きないのに「今週に入ります」と言い続けるので**予告が嘘になる**）、**ボタンを2つに割る**（`今週に入れる` / `受け付けだけする`。原資が足りない週は `disabled` を**実際に持つ**ので、共通則「disabled にしない」を対照が実演して破る）、**受理を名乗る**（青い輪郭＋トースト＋履歴のラベル付きチップ）。対照の外形は定常 **340 × 197px** で、**トーストが出ている瞬間だけ 227px** まで伸びる——実装が指摘したとおり、**寸法の約束が既定だけのものか対照にも効くのかを、企画も共通則も決めていなかった。**',
    Component: PressMeansTwoThings,
  },
  {
    id: 'order-decides-who-fails',
    no: 166,
    nameJa: '上の行が先に取る、とは書いていない',
    nameEn: 'Order Decides Who Fails',
    category: 'ゲーム',
    trigger: '2本の指示を始めて原資が細るまで週を送り、落ちるのが下の行だと分かったところで行を入れ替える。定規は動かず、次に足りない週で落ちる行が変わる（右上で「順位を書いて理由を知らせ、遡って作り直す」対照に切り替え）',
    principles: ['順番を名乗らない。縦の並びが順番を持つ', '規則の変更は定規に載らない。操作だけが履歴に載る', '順番は、落ち方の反復だけが言う'],
    ecology:
      'No.163 は2行を作り「行を複製する／担体は増やさない／履歴は行を名乗らない」と答えたが、**自分の実装で規則を1つ作ってしまっていた**——共有の原資を行A→行Bの順に直列で引くことで「片方だけ落ちる」を分岐なしに導いた、その**順番には担体が無い**。この標本は 163 の実装（固定費 20/週・積立 30/週・原資 100・`足す` +20・直列消費）を**1つも変えず**、変えたのは「どちらを先に引くか」を `order` 配列から読む1点だけである。既定の答えは**順番に担体を与えないこと**——**縦の並びそのものが順番を持つ**。No.156 が「空きの意味を割るのは縦線の左右だけ」と決めて以来、**縦の位置はこの語彙圏で一度も意味を持っていなかった**（横＝時間、縦＝優先）。実測で、順位番号・矢印・凡例は既定で **0 個**、禁止語（`順` `優先` `先` `1番` `2番` `番目`）は**全9状態で 0 回**（ボタン・行見出し・キャプション込みの `innerText` で数えた）、落ちた週に理由を書く要素 **0 個**、赤 **0 箇所**。2行は完全に対称で、`height` **20.000px** / `width` **320.000px** が差 **0.000px**、レールの地色は両行とも **rgb(214,214,211)**、塗り（**6×6px / rgb(61,61,61)**）と輪郭（**6×6px / rgba(0,0,0,0) / 1px rgb(179,179,179)**）が2行で **distinct 1 値**——**入れ替えても、行の中身の見た目は1つも変わらない。** 第2の芯は**入れ替えが定規を動かさない**こと。ハンドルを押した瞬間、目盛り **8個**・担体 **8個**・現在地の縦線・帯の矩形差分が**すべて 0.000px**、消えた要素 **0 件**・新規 **0 件**、外形も **{340, 190}** で完全一致。動いたのは**行コンテナの `y` だけ**で、行A **145 → 170px**（**+25.000px**）・行B **170 → 145px**（**−25.000px**）＝行の高さ **20px** ＋ 行間 **5px** と一致し、`x` は両行とも **145.000px** のまま（左右には 1px も動かない）。**規則そのものは台帳に載らないが、規則を変えた操作は履歴に載る**——履歴は入れ替え1回で **+1**（`始める`×2 と合わせて **3 点**）、点は **distinct 1 値**で何を入れ替えたかを名乗らず、`次の週へ` では **±0**。入れ替えに transition は付けていない（CSS 全体で transition 宣言は `.marker` の **1 箇所だけ**）——動かすと「入れ替えが出来事である」と読めてしまうが、入れ替えは**規則の変更**であって定規に載る出来事ではない（No.160）。第3の芯は**順番を、落ち方の反復だけが言う**こと。落ちた行の記録3回ぶんは **B・B・A**——入れ替え前は2回連続で下の行（積立）が落ち、**入れ替え後の最初の足りない週では上に来た積立が通り、下に落ちた固定費が輪郭になる**。**1週だけ見れば偶然と区別できない。2回目で規則になる。** 片方だけ落ちる週は上の行が塗り **1**・輪郭 **0**、下の行が塗り **0**・輪郭 **1**（**同じ行での同居 0 件**＝No.161 との撃ち分け）、両方落ちる週は両行に輪郭 **1** ずつ、1セルに担体2個の週は全週で **0 件**。入れ替えても各行の粒の列は**値として完全に保存**される（`ledgerA` `ledgerB` が入れ替え前後で同一）——**瞬間に入れ替わる代償として、どちらの行がどこへ行ったかを言うのは、行の中身と行見出しだけである。** そして**入れ替えの跡は定規に残らない**ので、「先週まではこの順番だった」は画面から読めない（No.160「跡は遡って作れない」の系）。既定は未来側の担体 **0 個**、外形 **340 × 190px**（全ステート不変。ボタンが5個に増えても、ハンドルを行見出し列とトラック列の**既存の隙間 14px** に絶対配置して grid の段を増やさなかったので、No.164 のような詰め直しが要らなかった）。対照は3つ同時に壊す——**順位を状態として書く**（①② と「優先順位」の凡例と ↓ 矢印）、**落ちた理由を書く**（輪郭が **rgb(179,58,58)** に変わり、赤い点滅と `⚠ 積立が優先度が低いため実行されませんでした` のトーストが 1800ms）、**入れ替えを遡って効かせる**（入れ替え直後に週3 の `A-3-filled` `B-3-outline` が**消え**、`A-3-outline` `B-3-filled` が**新規に立つ**——消えた **2 件**・新規 **2 件**。既定では同じ押下が台帳に一切触れない）。**書き換えた側では、いま入れ替えた順番で過去がずっと動いていたことになる。** 実装が React の罠を1つ掘り当てた: `order.map` の key を index にすると、入れ替えが**要素の移動ではなく中身の差し替え**になり（DOM ノードが同じ位置に残って props だけ入れ替わる）、**「動いたのは行の `y` だけ」という主張の裏取りが崩れる**。`key` を指示の識別子に直して解いている。並列実装の副作用も1つ記録された——他標本の編集による Vite の Fast Refresh がこの標本を再マウントし、計測中に状態が初期値へ戻った（`load` も `framenavigated` も出ていないので、リロードではないと切り分けた）。',
    Component: OrderDecidesWhoFails,
  },
  {
    id: 'rule-skips-this-week',
    no: 167,
    nameJa: '今週は、来ないことになっている',
    nameEn: 'Rule Skips This Week',
    category: 'フィードバック',
    trigger: '毎週の指示と隔週の指示を別の週に始めて、週を送り続ける。1粒目では2行が同じ絵で、2粒目で間隔が分かれる（右上で「周期を名乗り、休みに印を置き、次に来る週を予告する」対照に切り替え）',
    principles: ['求めていない週には、何も置かない', '周期は担体を持たない。距離だけが言う', '三つの「無」を割らない。次の粒が立つまで区別できない'],
    ecology:
      'No.155 以降、この語彙圏の指示は**すべて毎週**だった——**周期が1でない指示を、図鑑は一度も描いていない。** 隔週の指示は、規則が求めていない週に何も置かない。輪郭ではない（求めたのに入らなかった週が輪郭＝No.157 なので、休みに輪郭を置くと輪郭が**四義**になり、No.162 が測った限界を越える）。既定の答えは**何も置かないこと**で、この標本には**輪郭が1個も出ない**——`GrainKind` 型そのものを作っていないので、構造上生成できない。第2の芯は**周期が担体を持たない**ことである。行A（毎週）と行B（隔週）は完全に対称で、トラックは両行とも **240 × 22px**（`height` 差 **0.000px**）、レールの地色は両行とも **rgb(214,214,211)**、粒は **6×6px / rgb(61,61,61) / border-width 0px / border-radius 50%** の distinct 1 値。**1粒目が立った時点では、毎週の指示と隔週の指示は完全に同じ絵**であり、隣接ペアが **0 件**なので**距離がまだ存在しない**。**2粒目が立って初めて規則が読める**——行A の粒の中心は **191 / 221 / 251 / 281 / 311 / 341 / 371px** で隣接差が全ペア **30.000px**、行B は **251 / 311 / 371px** で全ペア **60.000px**（どちらも distinct 1 値）。距離を言うラベル・凡例・強調された目盛りは **0 個**で、**周期は測るものではなく、並んだ結果として見えるだけ**である。第3の芯は**同じ絵の「無」を割らない**こと。行B の週2（指示がまだ無い）・週4（規則の休み）・週8（定規の端）は担体が 0 個なので DOM 要素そのものが無く、トラック矩形から 30px 区間を切り出して数えると子要素 **0 個**・区間 **30 × 22px** が distinct 1 値で完全一致する。**区別するのは、次の粒が立つことだけ**——週2 の無は週3 の粒で「指示が無かった」と読め（`ledgerB` **0 → 1**）、週4 の無は週5 の粒で「休みだった」と読める（**1 → 2**）。そして**定規の端では、それが永遠に起きない**: 週8 で `次の週へ` を押しても `data-current-week` は **8 → 8**、台帳も原資も履歴も不変で新規要素 **0 件**。**休みだったのか終わったのかを、画面は最後まで言わない。** 既定は禁止語（`毎週` `隔週` `周期` `2週` `休み`）**0 回**・凡例 **0 個**・赤 **0 箇所**、縦線以外に `transition-duration` ≠ `0s` / `animation-name` ≠ `none` の要素 **0 件**、未来側の担体 **0 個**、外形 **340 × 207px**（全フェーズ不変）。対照は3つ同時に壊す——**周期を名乗る**（`毎週` **2 回** / `隔週` **2 回** ＋ 凡例 **1 個**。**しかも名乗っても、その週に何が起きるかは言っていない**）、**休みの週に印を置く**（担体が4値に増える）、**次に来る週を予告する**（未来側の担体が既定 **0 個**に対し **1 個**、注記 **1 個**）。対照の外形は **340 × 227px**、予告と注記が同時に出る瞬間は **247px**。**実装が企画の規約読み落ちを3件名指しした。** いちばん重いのは、企画の台本が **`始める` の瞬間に粒が立つ前提**で書かれていたことである。この語彙圏の多数派規約（No.157 / 159 / 160 / 161 / 164）は「`次の週へ` が**出て行く週**（押す前の現在地）について定規へ書き込む」で、その帰結として**現在地の週のセルは構造上つねに空**になる。実装は規約を優先し、`始める` はフラグを立てるだけにした（No.156 と同型）。**この差し戻しが企画の主張を強くした**: 企画は「定規の端の無」を行B 固有の話として書いていたが、規約の下では**行A の週8 も空**になり、**「最後まで区別できない」は行の性質ではなく、定規の端という位置そのものの性質**だと分かった。さらに、この標本の無は No.156 が測った「今週ぶんは、まだ来ていない」の無と**同じ絵で並ぶ**——空きの意味は、この標本で **4つ**になった。実装が目視で1つ直している: 対照の休みの印を `#d6d6d3` で描くとレールの上でほぼ視認できず、共通則の淡中 `#b3b3b3` に変えた。**C1〜C7 はこの変更の前後で1項目も変わっていない**——判読性は、条件を全部通したあとに目視だけが検出した（No.162 の教訓の再演）。',
    Component: RuleSkipsThisWeek,
  },
  {
    id: 'my-doing-not-in-any-ledger',
    no: 168,
    nameJa: 'わたしが起こしたのに、どこにも無い',
    nameEn: 'My Doing, In No Ledger',
    category: 'フィードバック',
    trigger: '原資が足りる週と足りない週で `置く` を押し、週を送り続ける。押した回数と、履歴の点の数を見比べる（右上で「効いた週に履歴の点を足す」対照に切り替え）',
    principles: ['押した回数と、履歴の点の数は合わない', '受理は履歴に載せない。定規の上の対が跡になる', '遅れて効いたことを、担体は名乗らない'],
    ecology:
      'No.165 が「同じボタンが二つのことをする」を決めた時点で、**履歴に載らない押下**が生まれていた。この標本はその代償だけを取り出す。台本は8週で、押下は **3回**（週1＝原資が足りる／週2・週4＝足りずに受理）。結末は**押した回数 3・履歴の点 1・定規の塗り 3** で、**3つの数がどれも一致しない**。しかも塗りのうち2つ（週3・週6）は**読み手が何も押していない週**に立つ——受理が持ち越されて効いた週である。既定の答えは**履歴に足さないこと**。受理が効いた週送り（週3→4・週6→7）の前後で履歴の点は **1 → 1（±0）**、点は全点で `width/height/background-color/border-radius/opacity` が **distinct 1 値**（6px / 6px / `rgb(61,61,61)` / 50% / 1）。後から足せば「効いた週に押した」ことになり、追記オンリーの台帳の**時間の座標が嘘になる**（No.161 が「置くのは分かった週ではなく起きた週」と決めた、その裏返しの困難）。**跡が残るのは定規のほう**で、押した週に**輪郭**、効いた週に**塗り**という**対**になる。第2の芯は、その対が**隣接とは限らない**こと: 週4の輪郭は週5でもまだ効かず（輪郭がもう1つ増える）、効いたのは週6である。読み手が対を読む規則は「**輪郭の連なりと、その右の最初の塗り**」だけで、画面はそれを一言も説明しない。第3は**主語を言う差が担体に無い**ことで、週1の塗り（押して立った）と週3・週6の塗り（押していない週に立った）は6項目で**完全に一致**する。輪郭は効いたあとも消えず（台本終了時、輪郭 **3個**・塗り **3個**、1つの週に担体が2個立つ週は **0 件**）、週2の輪郭は立った瞬間から終わりまで `left` **202px** のまま（差 **0.000px**、`border-color` `rgb(61,61,61)` / `1px` / `solid` / `opacity` **1** も不変）。既定は禁止語（`受け付` `持ち越` `遅れ` `あとで` `足りません` `回` `件` `あなた`）**0 回**、凡例 **0 個**、赤 **0 箇所**、未来側の担体 **0 個**、`marker` 以外に `transition` / `animation` を持つ要素 **0 件**、外形 **340×177px**。**対照は履歴のほうを直しにいって、時間の座標を壊す**: 効いた週に点を後から足すので履歴は **3点**になり、`data-history-weeks` は既定の **"1"** に対して **"1,3,6"**——**押していない週に押したことになる**。遅れて効いた点だけを青くするので点は **distinct 2 値**（`rgb(61,61,61)` 1個・`rgb(58,122,179)` 2個）になり、受理を名乗るトーストは **1800ms** で消えて**跡が 0 個**になる。**実装の判断が1つ効いている**: 原資の帯に刻み（ものさし）を置かなかった——No.165 は規則の入力を見せるために刻みを置いたが、この標本の主題は台帳の不一致なので、刻みを足すと「どの規則で足りないのか」という別の問いが画面に混ざる。そして**この回で唯一、企画の数値がそのまま通った標本**でもある（原資 60・費用 50・回復 +20 で、足りる週・足りない週・持ち越しが2週かかる週が台本どおりに出た）。',
    Component: MyDoingNotInAnyLedger,
  },
  {
    id: 'i-changed-the-rule',
    no: 169,
    nameJa: '規則を変えたのは、わたしだ',
    nameEn: 'I Changed the Rule',
    category: 'ゲーム',
    trigger: '`多くする` / `少なくする` で1週に積む量を変えながら週を送る。週5では誰も押していないのに量が変わる（右上で「規則を名乗り、変更を定規に載せ、遡って作り直す」対照に切り替え）',
    principles: ['読み手が変えたら履歴に載る。外界が変えたらどこにも載らない', '規則は名乗らない。量は粒の個数だけが言う', '戻すと定規から消える。規則の履歴は操作の列にしか無い'],
    ecology:
      'No.160 は「規則が変わったことは、どちらの台帳にも載らない」と決めたが、**あれは外界が変えた場合**だった。No.166 は「規則の変更は定規に載らない。操作だけが履歴に載る」と線引きした。この標本は**同じ画面の中で両方を起こして並べる**——図鑑で初めて、**同じ出来事が、誰がやったかで載る場所を変える**ことを測る。1週に積む量は粒の個数で言い（No.151）、台本の粒は **2,2,3,3,2,1,2（合計 15個）**、履歴は **4点**。読み手が変えた3回は履歴が **1 → 2 → 3 → 4** と増え、**外界が変えた週5では 2 → 2（±0）**。外界の変更を含む週送りで新しく現れた要素は**週4を出て行く粒 3個だけ**で、**変更それ自体に起因する新規要素は 0 件**（印もトーストも注記も無い）。同じ1クリックの中で `data-rate` が **"3" → "2"** に変わるのに、**画面にはそれを言う担体が1つも無い**。第2の芯は**過去を書き換えないこと**で、週3で `多くする` を押した前後、週1・週2の粒4個の `left` / `top` の差は**すべて 0.000px**、消えた要素 **0 件**・新規 **0 件**。押した週の粒は **0個**のまま（共通則どおり、粒は `次の週へ` でしか立たない）。第3が、この標本のいちばん細かいところ: **戻すと、定規から変えた事実が消える**。週7（`多くする` で元の量に戻した週）の粒の `top` 集合は**週2 と差 0.000px で完全一致**し、その時点で履歴は **4点**——**規則の履歴は、操作の列としてしか存在しない**。しかも履歴の点は **distinct 1 値**なので、**増やしたのか減らしたのかも言わない**。既定は禁止語（`毎週` `積立量` `粒` `量` `規則` `変更` `1週あたり` `増` `減`）**0 回**、数値 **0 個**（週の目盛りを除く）、凡例 **0 個**、赤 **0 箇所**、外形 **340×164px**。**対照は3つとも「親切」の方向に壊れる**: 凡例と注記で `積立 N粒/週` と規則を名乗り、変更のたびに定規へ青い区切り印を置き（外界の変更にはトースト `規則が変わりました`、**1800ms** で消えて跡 0 件）、そして `多くする` を押した瞬間に**過去の週の粒まで新しい量で作り直す**（週1・週2 が **2個 → 3個**）。**遡って作り直した対照では、最後に元の量へ戻した時点で全週が同じ個数になり、「量が変わっていた期間」が画面から消える**——既定が定規に残した唯一の証拠を、対照は消してしまう。**実装が企画の測り方の穴を1つ名指しした**: 週5の `次の週へ` は「週4を出て行く粒を置く」と「外界が量を減らす」の**2つの出来事が重なる1クリック**で、企画の受け入れ条件「その瞬間に新規要素 0 件」は文字どおりには成立しない。実装は**外界の変更に起因する差分**を分離して測り直している。この回の差し戻しで直したことも1つ: 粒が**正方形**で作られていた（共通則の文言が「6×6px の正方形」と書いていたため）が、家風は **`border-radius: 50%` の円**（No.163〜168 / 170）で、**仕様の文言より参考実装が正だった**。',
    Component: IChangedTheRule,
  },
  {
    id: 'whose-grain-is-this',
    no: 170,
    nameJa: 'この粒を置いたのは、誰か',
    nameEn: 'Whose Grain Is This',
    category: 'フィードバック',
    trigger: '`点検` を始めて `置く` を混ぜながら週を送る。読み手・繰り返し・外界の3つの主語が同じ行に粒を置く（右上で「主語で行を割り、粒に印を付ける」対照に切り替え）',
    principles: ['主語で行を割らない。粒に印を付けない', '読めるのは並び方だけ。単発どうしは区別できない', '重なった週は、重なりが消える'],
    ecology:
  ...
  },
  {
    ...
    ecology:
      'No.170 は「重なった週は粒 1 個」と決めたが、あれは**主語の違う 2 つ**が重なった場合だった。**同じ主語が同じ週に二度起こした**場合は決まっておらず、そこで 2 つの規約が正面衝突する——No.151「量は粒の個数で言う」と No.163「同じ行の同じ場所に担体を 2 個並べない」。**同じ週の 2 回を個数で言おうとすると、週より細かい時間を定規が持っていないのに、持っているかのように描くことになる。** 既定の答えは **1 週 1 粒**である。実装は分岐で二度目を弾いていない——`addGrain(ledger, week) = ledger.includes(week) ? ledger : [...ledger, week]` の 1 行で、**集合に同じ週を 2 回入れられない**という性質そのものが答えを出す。台本の結末は **押下 8 回 / 履歴 8 点 / 粒 5 個**（週 1・2・4・5・6）で、**合っている数は押下と履歴だけ**。週 2 で `積む` を二度押した前後、粒は **1 → 2（+1）** なのに `data-press-count` は **1 → 3（+2）**。そして 1 回だけの週 1 の粒と、二度起きた週 2 の粒は、`width / height / background-color / border-width / border-radius / opacity` の 6 項目に `top`（**177px**）を加えて**完全一致**する（差 0.000px）。第 2 の芯は**粒が種類も言わない**ことで、`積む` と `点検` のボタンは**同じ `handlePress()` を指しており、引数を 1 つも取らない**——どちらを押したかを運ぶ変数がコードに存在しない。だから種類の違う 2 つが重なった週 6 の粒も、週 1 の粒と 6 項目＋`top` で**完全一致**する。台本の全 15 ステップで、1 週の粒が 2 個以上になったフレームは **0 枚**。そして第 3 が、この標本が本当に言いたいことである: **`積む` の二度目は意味があり（量が 2 倍になる）、`点検` の二度目は意味がない（起きたことは増えない）のに、定規はどちらも 1 粒にする。** 既定は片方でしか正しくない。**対照は、もう片方でしか正しくない**——週セルの中に押下ぶんだけ並べると粒は **8 個**になり押下回数と合うが、週 2 の 2 粒の中心間距離は **8.000px**、週 1 → 週 2 の粒の中心間距離は **30.000px**。**同じ「隣」が 2 つの違う時間差を指し**、読み手は 8px の隣を「週の中の前後」として読む。だが状態にその値は無い——対照がやっているのは**履歴の順序（配列の index）を、定規の上の位置に変換する**ことだけである。**順序と時刻を同じ軸に載せると、片方が必ず嘘になる。** 既定は禁止語 **0 回**・凡例 **0 個**・注記 **0 個**・赤 **0 箇所**・青 **0 箇所**・未来側の担体 **0 個**、外形は既定・対照とも **340×152px**、現在地の縦線以外に transition / animation を持つ要素 **0 個**。**企画の誤りが 1 つ出た**: 台本の週別表を足すと押下は 8 回なのに、企画の結末の要約は **9 回**と書いていた（粒 5 個のほうは表と一致していた）。実装が表のほうを忠実に実装して実測 8 を出し、要約の誤記が露見している。**表と要約を別々に書くと、検算していない数がそのまま仕様になる。**',
  ...
  },
  {
    ...
    ecology:
      'No.157 は代わりに動くものの**失敗**を扱い「代行の失敗は読み手の台帳には載らない」と決めたが、**成功をどちらに載せるか**は決めていなかった。この図鑑は No.168〜170 で「載った場所が主語を言う」を前提にしている以上、代わりに動くものは**その前提の外に居る 4 人目**である——読み手でも、繰り返しでも、外界でもない。既定の答えは **定規にだけ載せる**こと。`handleNext` の中で隔週の判定が 1 回評価されて `ledger` に書き込むだけで、`history` / `pressCount` に触る経路がコードに **0 本**ある。台本の結末は **押下 3 回 / 履歴 3 点 / 粒 4 個 + 輪郭 1 個**で、**粒 4 個のうち 3 個は読み手が押していない**。`任せる` を押した瞬間に履歴は **+1**、以後 3 回の成功で履歴は **±0**——**許可は 1 回、成功は何回でも**（`startWeek` は書き込みが 1 回・参照が週送りのたび、という非対称がそのままコードの形になっている）。代償は No.170 が測ったとおりで、代わりに立った週 2 の粒と読み手が置いた週 3 の粒は 6 項目＋`top`（**177px**）で**完全一致**する。読めるのは**列としてのふるまい**だけ——代わりの粒の `left` は **202 / 262 / 322px**、隣接差は**どちらも 60.000px** で、等間隔だけが「続いている」を言う（No.167 の距離）。粒に由来を持たせないのは隠しているからではなく、`LedgerEntry` が `{week, kind}` しか持たず、**由来を運ぶフィールドがそもそも無い**からである（既定の DOM に `data-by` 系の属性 **0 件**）。第 4 の芯が、この標本のいちばん苦い答えである: 週 7 で `取り消す` を押すと、過去 4 粒の `left / top / width / border-radius / background-color / opacity` の差は**すべて 0**、新しく立つのは現在地の**輪郭 1 個だけ**（`data-week="7" data-kind="outline"`）、そして履歴は **2 → 3**。**自分がやっていないことを取り消した跡だけが、自分の側に残る。** 実装は `handleUndo` で `ledger` を**一度も読まない**（検索もフィルタもしない）——ボタンの文言が示唆する「探して消す」を、コードは 1 行もやっていない。**対照は 3 つの壊れ方を同時に持つが、噛み合っていないことが実測で出た**: 代わりの成功を履歴に載せると点は **3 → 6** になり、**読み手が履歴を読み返すと記憶に無い押下が 3 つ並ぶ**。粒には印が付いて色が **distinct 2 値**（`rgb(58,122,179)` / `rgb(61,61,61)`）になるのに、**履歴の点は distinct 1 値のまま**——**印は定規の側にしか付けられず、順序しか持たない台帳では主語が読めない**。既定の色は `rgba(0,0,0,0)` を含む 8 値すべてがパレット内で、青・赤は **0 箇所**。外形は既定 **340×152px**、対照のトースト中だけ **340×182px**（幅は不変）。transition を持つ要素は現在地の縦線 **1 個のみ**（0.28s）。**実装が企画の穴を 2 つ埋めた**: `取り消す` の回数制限が仕様に無く（`任せる` にだけ「1 回だけ」と書いてあった）、C5 の「輪郭 1 個」を守るために同型のガードを足したこと。そして隔週の起点で、No.167 / 170 の判定関数を流用すると**任せた週そのものにも成功してしまい 1 週ずれる**ので、起点を `+1` した専用関数に分離したこと——**流用すると起点の意味が暗黙になる**という理由で、別名にしている。**配線側の目視が 1 件拾った**: 舞台説明が `「任せる」は1回だけ。` と**アラビア数字で回数を名乗っていた**（共通則が禁じているのは週ラベル以外の数字で、実装はトーストの「1 個」は自分で潰していたのに、キャプションの「1回」は残っていた）。`一度だけ` に直してある。**禁止の網は、担体の側だけでなく舞台説明の側にも掛かる。**',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '`日で見る` に切り替える。定規が 7 倍に拡大しても粒は割れず、増えるのは空白と目盛りだけ。`週で見る` で往復する（右上で「日の位置に置き直す／二度目を割って並べる／空白を塗り分ける」対照に切り替え）',
    principles: ['拡大は読み方であって、台帳ではない', '粒は最初から1週ぶんの幅を持っている', '捨てた二度目は、拡大しても戻ってこない'],
    ecology:
      'No.171 は「1 週 1 粒。順序でしか区別できない出来事は定規に載らない」と決め、**二度目を捨てた**。この標本は、読み手が**定規のほうを細かくしたとき**を撃つ。拡大は読み手にとって「もっと見える」操作であり、**捨てた情報と、持っているのに見せていない情報の差**がここで初めて画面に出る。既定の答えは、**粒は最初から「1 週ぶんの区間」である**こと。週の目盛りでは 1 週の刻みと粒幅がほぼ等しいので点に見えているだけで、日の目盛りへ拡大すると **7 倍の幅を持つ塊**として見える。実測で粒幅は **21px → 147px（7.00 倍）**、「粒幅 ÷ 1 週ぶんの幅」は週表示 **21/21 = 1.000**・日表示 **147/(21×7) = 1.000** で**表す範囲は変わっていない**。そして拡大した粒の**子要素は 0 個**——**大きくしても、中には何も現れない**。第 2 の芯は**捨てた二度目が戻らない**ことで、二度起きた週（週 4）の粒は拡大後も要素 **1 個**。対照は日表示で **2 個**に割れ、中心間 **9.00px** で並ぶ——状態に時刻のフィールドは 1 つも無いのに、**拡大したら週の中の前後が読めるようになった**ように見える（No.171 の対照と同型の嘘）。しかも対照の点は **6px 固定**で拡大に反応しない＝**点というモデルは、拡大しても大きさの情報を最初から持っていない**。第 3 は空白で、拡大して増えるのは空白だけだが、その空白には「起きなかった日」と「起きた日も分かっていない」の **2 種類**がある（No.150 の解像度版）。**既定は描き分けない**——日セルの `background-color` は **distinct 1 値**（`rgba(0,0,0,0)`＝塗らない）で、セルの存在は目盛り線だけが示す。対照は **distinct 2 値**（`rgb(214,214,211)` / `rgb(179,179,179)`）で**持っていない区別を描く**。第 4 に、拡大は**読み方**であって台帳ではない（No.159 の継承）——週 → 日 → 週 と往復したあと、5 個の粒の `left` / `width` は最初と**差 0.00px**。粒の `transition-duration` は全 scale で **0s**、`animation-name` は **none**（対照は `0.3s` で滑る＝読み方の操作なのに担体が動く）。**この標本は企画の誤りを 2 回踏んで直った。** 1 回目は日の目盛りを描いていなかったこと——数値条件は全部通っていたのに、絵は「4 個のブロックが並んでいる」だった。2 回目は企画そのもので、**「粒の実 px 幅が両表示で一致すること（差 0.00px）」という条件が、拡大を拡大でなくしていた**。刻みが変わらないので粒も大きくならず、表示範囲が狭まるぶん**レールはむしろ短く見える**——「拡大しても情報は増えない」は、**本当に拡大していないと成立しない**。条件を「実 px の一致」から「**表す範囲の比が両方で 1.000**」「**日表示の粒幅が週表示の 7.00 倍**」へ差し替えて解いた。**反証可能に書いたつもりの条件が、反証したい主張のほうを消していた。** 実装側の罠も 1 つ効いている: `PITCH_DAY` を独立した定数にすると「日表示の粒幅が 7 日ぶんの目盛り幅と一致する」が**偶然の一致**になり、どちらかを変えると壊れる。導出（`WEEK_WIDTH_IN_DAY_VIEW = PITCH_DAY * 7`）に変えたことで、一致が**式の性質**になった。もう 1 つ、`data-grain-count` にフィルタ後の配列長を出すと、日表示で画面外になった週のぶんだけカウントが落ちる——**表示範囲が狭くなることと、持っている件数が変わることは別**である。外形 **340×130px**。',
  ...
  },
  {
    ...
    category: 'フィードバック',
    trigger: '`任せる` を 1 回押し、あとは `次の週へ` を押していく。週 3 から粒が大きくなるが、UI は何も言わない（右上で「方針変更をトーストと印と履歴で名乗る」対照に切り替え）',
    principles: ['許可の範囲は宣言できない。観測するしかない', '押していない点は履歴に作らない', '変わったと言わずに、並べて残す'],
    ecology:
      'No.172 は「代わりにやったことの成功は**定規にだけ**載る。履歴に載るのは `任せる` の 1 点だけ」と決め、No.169 は「読み手が規則を変えたら履歴に載り、外界が変えたらどこにも載らない」と決めた。この標本が撃つのは**4 人目**——**代わりに動くものが、規則そのものを変えたとき**である。既定の答えは 172 をそのまま貫くこと: **履歴には載せない**。規則変更の前後で履歴の点は **1 → 1（±0）**、対照は **+1**（全走行では **2 → 4** で +2）。代償は企画が名指ししたとおり高い——**成功は 1 週ぶんだが、規則は以後の全週に効く**。この標本は、その代償を払ったうえで読み手が気づける最小の形を探した。第 1 の芯は、**許可の範囲は宣言できない**こと。読み手が押したのは `任せる` の 1 つだけで、それが「積むことを任せた」のか「量の決め方まで任せた」のかを画面は一度も言っていない（No.166「上の行が先に取る、とは書いていない」の代行版）。実測で、`任せる` を押してから最初の粒が出るまで**許可の範囲を示す要素は 0 個**、`data-rule-notices` は全状態で **0**、既定の DOM 全文に「変更」の語が **0 件**。**範囲を示すのは、任せたあとに定規に現れる粒の並びだけ**である。第 2 の芯は**気づき方**で、粒の大きさが 1 週に積む量を表す（No.151 の語彙）。実測で粒の `background-color` **rgb(61,61,61)**・`border-color` **rgb(61,61,61)**・`border-style` **none**・`border-width` **0px**・`opacity` **1** がすべて **distinct 1 値**——**大きさ以外は完全に同じ**で、幅だけが **6px → 12px**（週 2→3 の比 **2.00**）に変わる。単独の粒から規則の値を復元する手がかりは画面に **0 個**なので、**読み手は隣り合う 2 週を見比べて初めて気づける**。**気づくのに 2 週かかる規則変更を、UI は止められない**——止めようとすると通知が要り、通知は担体を増やす。第 3 は取り消しで、`元に戻す` は読み手の操作だから履歴に **+1（1 → 2）**、そして**過去の粒は 1px も動かない**（過去 5 個の left/top/width/height の最大差 **0.000px**）。**変えられていた期間は、定規の粒の並びにしか残らない。** 実装は 172 よりさらに徹底しており、`handleUndo` は `ledger` を**一度も読み書きしない**（フラグを立てるだけ）。**対照の 4 つの壊れ方は、どれも「親切」の顔をしている。** トーストで名乗り、粒に赤い印を付け、色を分け（`background-color` が **distinct 2 値**: rgb(61,61,61) / rgb(58,122,179)）、履歴に点を積む。いちばん質が悪いのは 4 つめではなく、**通知があると読み手は範囲を確かめなくなる**ことである——「変更は管理されている」という誤った安心を作るが、通知は**変更が起きた後**にしか出ない。**実装が企画の穴を 2 つ埋めた**: 粒の大きさを面積比例（`side ∝ √amount`）で書くと実測比が **1.414** になり、C4 が要求する **2.00** から外れる（線形に修正）。規則変更の境界を `week > changeWeek` にすると変更週そのものが旧規則のままになる（`>=` に修正）。**量を大きさで言う標本は、「大きさ」が一辺なのか面積なのかを企画が決めていないと、実装が測れない形で分岐する。** 外形 **340×156px**。',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '`次の週へ` で週 8 まで送り、週番号を押して当時の目盛りに戻す。粒は 1px も動かない（右上で「過去の粒を今の規則へ変換して並べる」対照に切り替え）',
    principles: ['過去は書き換わらない。書き換わるのは読み方のほう', '粒は当時の規則を名乗らない', 'どちらの目盛りでも、全部の週は読めない'],
    ecology:
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '週番号でいちばん上の行の目盛りを合わせ、`下に置く` で行を増やす。粒は 1 つの大きさのまま、行ごとに目盛りの間隔だけが違う（右上で「行ごとに粒を変換する／行が規則を名乗る／一致した週を赤枠で囲む」対照に切り替え）',
    principles: ['並べても、粒は1つの大きさしか持たない', '読めることと、比べられることは別', '行は増やせるが、読める週は増えない'],
    ecology:
      'No.176 は「どちらの目盛りでも、全部の週は読めない」を代償として引き受けた。代償を知った読み手が次にやることは決まっている——**両方を並べる**。この標本はその手を最後まで通す。**企画は、苗床に書いてあった前提を 1 つ捨てた**: 種には「同じ粒が二つの大きさで同時に在る」と書いてあったが、176 が「粒の高さは置かれた週の値だけで決まり、目盛りを一度も参照しない」と決めた時点でそれは誤りになった。**並べても粒は 1 つの大きさしか持たない**——実測で、3 行（20px / 5px / 20px）に並べた週 1〜8 の粒は行間の `width` / `height` 差が**すべて 0.00px**、違うのは目盛り線の間隔だけ（上段 **20.00px** / 下段 **5.00px**）。捨てた前提のほうは**対照 1** に回した。第 2 の芯がこの標本の答えである。**並べると全週が読めるようになる。ただし、どの週も片方の行でしか読めない**——今の目盛りで読めるのは週 4〜8 の **5 週**、当時の目盛りでは週 1〜3 の **3 週**、和は **8 週（全週）**、そして**両方で読める週は 0 週**。だから **読めることと、比べられることは別**になる: 2 つの週を比べられるのは同じ行で両方とも読めるときだけで、`C(5,2) + C(3,2) = 13` 組。**全 28 組のうち 15 組は、何行並べても比べられない**（`data-comparable-pairs` は行ごとの読める週の集合から導出しており、直書きではない）。第 3 に、**行は増やせるが読める週は増えない**。この図鑑にはまだ規則が 2 つしか無いので、3 行目は必ず既出の読み方の複製になる——3 行にしても `data-readable-union` は **8 のまま（増分 0）**で、増えるのは高さだけ（**187.92 → 259.92 → 331.92px、+72.00px/行**）。**読み方の数は、規則の数を超えられない。** 担体は増えていない: rail-wrap 内の class 名の集合は 1 行時と 3 行時で **7 種のまま完全一致**（No.163「行を複製する。担体の種類は増やさない」の手）、行見出しは全行 `定規` の **distinct 1 値**で、**どの行が「今」かを画面は名乗らない**（`data-rule-attrs` **0**、禁止語 **0 件**）。読み方の操作なので担体は動かない——transition を持つ要素 **0 個**、`下に置く` → `しまう` の往復後も上段 8 粒の `left` / `top` / `width` / `height` の差は**すべて 0.00px**。**対照は、苗床の前提をそのまま実装したもの**である: 行ごとに粒を変換すると同じ週の粒が二つの大きさで在り、**どの行でも全週が読めてしまう**（`data-readable-both` が **0 → 8**）——**「読めない週がある」という事実そのものが画面から消える**（No.176 の対照と同型）。さらに行が `今の規則` / `当時の規則` と名乗り（行見出し distinct **2 値**）、行の増減が **0.3s** で滑る。**この回は企画の誤りが 1 件、実測で出た。** 対照 3（両行で高さが一致した週を赤枠で囲む）は `data-coincidence-weeks` が**実測 0 件**で、一度も発火しない——企画は「規則が変わっていない週では必ず一致が出る」と書いたが、**2 つの規則が 4 倍離れている限り、変換後の高さが一致することはない**。**一致が起きるのは、規則どうしが近いときだけである。** 実装が目視で拾った罠も 1 件: 対照の行見出し `今の規則` が `LABEL_COL = 34px` の列で **`今の...` に潰れて読めなかった**（折り返しに変更して解消）。**行見出しの幅は、既定の 2 文字に合わせて決まっていた。**',
  ...
  },
  {
    ...
    category: 'フィードバック',
    trigger: '`任せる` を押して `次の週へ` を送り、粒が倍になったところで `狭める` を押す。押した瞬間は何も起きず、次の週の粒が基準に戻るだけ（右上で「段をバッジとトーストと赤い印と 3 値の履歴で名乗る」対照に切り替え）',
    principles: ['制約は方向だけを押させる。語彙を持たせない', '点の列から、段は復元できない', '狭めたせいの空きと、もともと何も無い空きは、同じ空きである'],
    ecology:
      'No.175 は「許可の範囲は宣言できない。観測するしかない」と決めた。ではその範囲を**狭めようとしたら**どうなるのか。第 1 の芯は、**制約の語彙を画面に一度も出さない**こと。読み手が押せるのは `狭める` という**方向だけ**で、「何を」狭めるかは選べない——選ばせた瞬間、No.160 / 166 / 175 が三度封じた「規則を名乗る」が制約の側から漏れる。実測で `data-scope-attrs` は **0**（DOM 全体を走査して属性名を `scope|limit|permission|level|rule` でフィルタした違反 **0 件**）、既定の DOM 全文に禁止語 **0 件**、そして `狭める` / `ゆるめる` のラベルは段 0 / 1 / 2 のどこでも**同一文字列**（distinct **1 値**ずつ）——**同じボタンが、段ごとに違う意味を持つ**（No.165 の 3 値版）。第 2 の芯がこの標本のいちばんの主張で、**点の列から段は復元できない**。台本 A（`任せる` → `狭める` → `狭める`）と台本 B（`任せる` → `狭める` → `ゆるめる`）を走らせると、`data-history-dots` は**両方 3 で一致**し、履歴の全点の `class` / `data-*` / 矩形の JSON も**完全一致**する。それでいて段は **2** と **0** である。**同じ履歴で、段が違う。** 第 3 は**守られたことが観測できない**こと: 段 1 のとき、代わりに動くものが量を変えたかった週と、もともと基準で積む週の粒は `width` / `height` **6px / 6px（差 0.00px）**・`background-color` **rgb(61,61,61)**・border・opacity まで**完全一致**する。止められたことを描くには、**代わりに動くものの意図**という、既定が持っていない情報が要る（No.150 の権限版）。第 4 に、**狭めたせいの空きと、もともと何も無い空きは同じ空き**である。段 2 の週は粒が立たないが、No.157 が「代行が試みて失敗した週は輪郭で置く」と決めたのに対し、ここでは**試みそのものが起きない**ので輪郭も立たない——空いた週セルとまだ到達していない週セルは、ともに子要素 **0 個**、`background-color` は **rgba(0,0,0,0)** の **distinct 1 値**（＝塗らない）。**自分が狭めたせいなのか、もともと打てなかったのかは、画面のどこにも書かれていない。** そして効かない押下は履歴に載らない（No.159 の継承）: 段 2 で `狭める` を 3 回押すと `data-press-count` は **3 → 6** に増えるのに `data-history-dots` は **3 → 3** のまま——**押した回数と点の数は合わない**（No.168 の継承）。気づくのに要する週は `data-weeks-to-notice` **2**（量が倍になる境界と同じ定数から導出）。**対照は 4 つとも「親切」の顔をしている**: 段のバッジ（`制限: N段`）とトーストで名乗り、止められた週に赤い印を出し、履歴の点を 3 値の濃さに塗り分け、段 2 の空きを「制限のせい」として塗り分ける（空きセル背景 distinct **2 値**）。**いちばん質が悪いのは 3 つめ**で、点に段を持たせた瞬間、履歴は「読み手が押した列」から**「権限の状態の台帳」**に変わる（No.169 / 173 の決めが壊れる）。**企画の自己矛盾が 1 件、実装に見つかった**: 企画は検証値を `data-scope-level` に出せと書いたが、**その名前自体が同じ企画の C1 が禁じるパターン（`scope` にも `level` にも）に当たる**——書けば C1 を自分で破る。実装は `data-narrow-depth` に改名して解いた。**企画が決めていなかった点も 1 つ効いている**: 倍化の起点を No.175 と同じ `startWeek + 2` にすると収録台本（任せる → 次へ ×2）で倍の週がまだ立たず、絵が成立しない——`startWeek + 1` に読み替えた。実装が踏んだ罠は、既定側の空き週に `is-empty-by-depth` のような識別クラスを足しかけたこと。**足していれば、芯 4 がその場で壊れていた。** 外形 **340×156px**。',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '週番号で目盛りを選び、`閉じて開く` を押す。点だけが消えて、目盛りは残る。選び直して元の値へ戻すと、一度も触っていない画面と同じ絵になる（右上で「復元しましたと名乗り、選択中を塗り、`既定に戻す` を置き、出どころで目盛りの濃さを変える」対照に切り替え）',
    principles: ['残るのは規則、消えるのは操作', '既定になった読み方は、選ばれた跡を持たない', '「元」を持っているのは画面ではなく、読み手の記憶だけ'],
    ecology:
  ...
  },
  {
    ...
    ecology:
      'No.179 が「残るのは規則、消えるのは操作」と決めた直後に来る問いを撃つ。**この目盛りは、誰が決めたのか。** 主語は3つある（今日の自分・過去の自分・代わりに動くもの）が、No.169 は「読み手が変えたら履歴に載る／外界が変えたらどこにも載らない」と**2つ**しか分けていない。既定の答えは強い否定形で、**出どころは規則に書かない。書けるのは操作の跡だけで、跡は再訪で消える**。実測で、読み手が目盛りを変えると履歴の点は **0 → 1**、`閉じて開く` で **1 → 0**、代わりに動くものが変えても **0 → 0**（増えない）。この標本の中心は、**代行が変えた直後の画面と、読み手が自分で同じ値にして再訪した画面が、全要素の JSON で完全一致する**ことである（`tagName` / `className` / `textContent` / computed style / `data-*` / 矩形を集めて比較し、**差分 0 件**）。**同じ規則に、一方は代行が、一方は読み手が到達しているのに、画面はそれを区別しない**——だから**主語は3つあるのに、画面が分けられるのは2つ**（点が在るか、無いか）になる。**過去の自分は、画面の上では外界と同じ側に落ちる。** ここで No.170「主語は**並び方**だけが推定させる」が、この図鑑で初めて**移せない**ことが分かる。あの答えは**粒が複数あって並びを持つ**から成立していた。**規則は1つしか無く、並びを持たない。** この標本は 170 を移す代わりに、**並びを持つ側（履歴の点）に主語を預け、預けた先が消えることで答えを失う**という形で解いた——**主語は規則の属性ではなく、操作の跡の属性である。** 担体は1つも足していない（増えたのは操作が1つ＝`代わりに動く` だけで、既存の操作と同じ見た目を持つ）。外界の変更を**動きで描かない**のも決めのひとつで、代行の変更の前後 20 フレーム × 4 担体 = **520 サンプル**で computed `transition-duration` は **`0s` 以外 0 件**、`animation-name` は **`none` 以外 0 件**（CSS に宣言を書いていないので構造的に保証される）。**出発点が画面の外にあるものを動かすと、とっくに起きていたことが「いま起きたこと」になる。** 粒は全工程で **0.00px**（`left` / `top` / `width` / `height` の最大差分）で、動くのは目盛りの間隔（**20.00 ⇄ 5.00px**）だけ。**企画の条件が1つ、文字どおりには満たせなかった。** C6 は「4つの状態の JSON が distinct 2 値」を要求したが、状態には**規則の値**という別の軸（5px か 20px か）が混ざるので、DOM 全体を機械的に比較すると必ず **3 群**に割れる。実装は「点が在るか無いか」を導出値として1つの属性に出し、**その属性だけを C6 の測定軸に絞った**（DOM 全体の distinct **3** も併記されている）。**原則は正しく、条件の書き方が原則より強すぎた**——No.179 が同型の過剰条件を踏んだのに続く2例目である。**対照は、壊れていることを実演するために嘘の跡を作らなければならなかった。** 出どころのバッジを3種（`あなたが設定` / `自動で調整されました` / `前回のまま`）出すには「過去の自分」に相当する点が要るが、そんな点は**どの台帳にも無い**ので、対照は初期状態から `前回のまま` の点を**操作なしででっち上げて**いる。バッジは初期状態でも `前回のまま` と名乗る——**バッジは必ず何かを名乗るので、「分からない」が言えなくなる。** トーストは 1800ms で **0個**になり（見ていなかった読み手には何も残らない）、履歴の点の色は **distinct 3 値**（`rgb(140,140,140)` / `rgb(61,61,61)` / `rgb(179,58,58)`）＝**履歴が出どころの台帳に変わる**（No.169 の分けが壊れる）。外形は既定 **340×185px** / 対照 **340×209px**。',
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: '自分の目盛りを選んでから `送る` を押す。下段（受け手）の目盛りと、両方で読める週の数を見る（右上で「読み方も一緒に渡す」対照に切り替え）',
    principles: ['渡せるのは粒だけ。読み方は担体を持たないので渡らない', '渡した跡は送り手の履歴にだけ載る。受け手には「最初からそうだった」に見える', '渡されたものに印を付けない'],
    ecology:
      'No.181 は、No.176「あの週を、今の目盛りで読んでいる」を**他人**に向けた標本である。自分の画面を他人に見せたい——**粒は同じでも、読み方は一緒に渡らない**。No.98「他人は主役にならない」と No.72「よその手が入る」は**他人の操作**を扱ったが、これは**他人の読み方**であり、**操作は台帳に載せられるが、読み方は載せる先が無い**。既定の答えは、**渡せるのは粒だけ**。実測で、`送る` の前後で下段（受け手）の目盛りの間隔は **20.00px → 20.00px（差 0.00px）**——対照は上段の値へ**上書き**される（20.00 → 5.00px）。粒の高さは規則で変わらない（No.176 の継承）ので、同じ週の粒は上段と下段で**8週ぶんすべて差 0.00px**。**違うのは目盛りの間隔だけで、その結果「読める週の集合」が上下で違う**：開いた直後は上段（5px）が **3週**（①②③）、下段（20px）が **5週**（④⑤⑥⑦⑧）、**両方で読める週は 0 週**。週6を押して自分の目盛りを受け手と同じ値にすると上段 **5週** / 下段 **5週** / 両方 **5週**になるが、これは**読み方が渡ったのではなく、たまたま揃っただけ**である（渡す操作は一度も受け手の規則に触っていない）。台帳の分け方も継承する——`送る` で**送り手の履歴は +1**、**受け手の履歴は ±0**。受け手にとって、その目盛りは**最初からそうだった**ように見える（No.179 の C3 が、自分の再訪ではなく他人に対して起きる）。そして受け手の側には**渡されたものを示す担体を1つも置かない**：下段の全粒の `className` / `background-color` / `border` / `opacity` が **distinct 1 値**で、渡された粒と元からの粒は画面の上で区別できない（No.170「行を割らない・印を付けない」の継承）。**渡した本人にしか、それが渡したものだと分からない。** 渡す動きを**移動で描かない**のも決めのひとつで、送る前後 20 フレームで下段の粒の `transition-duration` は **`0s`**、`top` の distinct は **1 値**（中間値 **0 件**＝いきなり最終位置に在る）。対照は **`0.3s`** で降ってきて、`top` の distinct は **18 値**——**出発点は相手の画面、つまりこの画面の外にあるので、移動として描くと「いま起きた」になる。** **企画が自分の言葉で自分の条件を割った。** 企画は操作ボタンを一貫して「**渡す**」と呼んでいたが、同じ企画の C7 が禁止語に「**渡**」の一文字を入れていた——可視のラベルにその字を書いた瞬間、必ず 1 件ヒットする。属性名の自己言及（No.176 / 178 / 179）と同型だが、**今度は画面に出る文字で起きた**。実装はボタンの表示を「**送る**」に変えて解いている（操作の中身は変えていない）。診断用のルート属性 `data-sender-history` が禁止語 `sender` を含んでいた件も同時に見つかり、属性名のほうを改名して根本から 0 件にした。**対照の設計にも、企画の穴が1つ出た。** 企画の台本は下段の初期の粒を常に 0 個としていたが、それだと渡された粒が**必ず全部**になるので、どう塗り分けても distinct 1 値にしかならず、対照の「色分けで区別する」が**構造的に成立しない**。実装は対照側にだけ「受け手が元から持っていた粒 4 週ぶん」を置いて解いた。**実物を見て直した点が1つある。** 受け入れ条件はすべて通っていたのに、**読める週の丸印が上段にしか出ていなかった**ので、この標本のいちばんの主張（読める週の集合が上下で違う／両方で読める週 0 週）が**画面から確かめられなかった**。No.177 と同じ担体（週番号の丸印）を下段にもう 1 列足して解いている（担体の種類は増えず、増えたのは行の個数だけ）。外形は **340×313.84px**（既定・対照とも）。',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '`小さいものを落とす` `まとめて見る` `新しいほうだけ` を順に押す。読める週が 7 → 5 → 3 → 1 と減るが、画面はいくつ効いているかを何も言わない。`閉じて開く` で再訪する（右上で「効いている数を名乗り、チップで個別に外せて、落ちた週を塗り分ける」対照に切り替え）',
    principles: ['個数を名乗らない。効いていることは、効いた結果でしか読めない', '落ちた週と、もともと無い週は、同じ空きである', '規則は残り、何枚掛かっていたかだけが消える'],
    ecology:
      'No.182 は「**規則のゼロは存在しないので、やめる操作は置けない**」と決めた。その帰結が未決のまま残っていた——**捨てられない規則は、積める。** この標本は定規（週 1〜8、高さは決め打ちの `{1:48, 2:44, 3:40, 4:なし, 5:36, 6:14, 7:12, 8:55}`）に、足すことしかできない規則を3つ置く（`小さいものを落とす` は 20px 未満、`まとめて見る` は隣接ペアの後ろ、`新しいほうだけ` は週 5 未満）。第1の芯は**個数を名乗らない**ことで、既定の可視テキストに禁止語（`読み方` `規則` `フィルタ` `条件` `適用` `件` `個` `有効`）の出現は **0 件**、出現する数字はキャプションの「3つのボタン」と週番号だけ＝**効いている数を表す数字は 1 つも無い**。読み手が知る手段は、定規の上で読める週が減っていくことだけである——実測で **7 → 5 → 3 → 1**。対照は `効いている読み方: 3` のカウンタを出す（禁止語 **1 件**）。第2の芯は**外す操作が無い**ことで、既定の個別除去の操作は **0 個**、対照はチップの × が **3 個**。第3の芯がこの標本の主張そのもので、**規則で落ちた週と、もともと粒が無い週が、定規の上で完全に同じ空きになる**（No.178「狭めたせいの空きと、もともと何も無い空きは、同じ空きである」の継承）。実測で既定の空きセル 6 件は `background-color` / `border` / `opacity` / `className` が **distinct 1 値**、対照は **distinct 2 値**（規則落ちは `rgb(211,211,208)` ＋ `is-filtered`、週 4 の自然な空きだけ塗られない）。**これが「不在の偽装」の実体である**——読み手は見えなくなるたびに「データが無い」と読む。第4に、規則を足すのは出来事ではない。生き残った粒の 20 フレーム最大差分は **0px**（A 単独追加・B 追加・C 追加の3通りとも）、消える週の担体も `transition-duration` **`0s`** / `animation-name` **`none`**。対照は `0.3s` で、同じ 20 フレームの `opacity` が `1 → 0.96 → 0.89 → … → 0` と**中間値を並べる**。そして最後に、履歴の点は足すたびに **+1**（0 → 3）だが `閉じて開く` で **0** になり、**そのとき定規の絵は1要素も変わらない**（再訪の前後で全要素 JSON の差分 **0 件**、読める週も 1 のまま）——**規則は残り、何枚掛かっていたかだけが消える。** **実装が企画の言葉を2か所で割った。** ひとつ、企画は「A/B/C の効果が互いに独立になるよう値を選ぶ」と書いたが、**可換性と線形独立性は別のことで、両立しない**。押下順に依存しないこと（可換性）は `isVisible(week, aOn, bOn, cOn)` という**押下順を引数に取らない純関数**で構造的に達成したが、B は「まだ見えている週のペア」を畳む仕様なので、**B の効果量は A が何を落とした後かに依存する**。これは企画の文言そのものが持つ性質で、値選びでは回避できない。ふたつ、企画の期待値表は「なし = 8 週」を前提に書かれていたが、**芯3（自然な空きの実演）には最初から粒の無い週が要る**ので、実測の基準は **7** になった——**企画は自分の芯と自分の期待値表を、暗黙に矛盾させていた**（芯を優先した）。みっつ、企画は芯2で「読み手にできるのは全部を選び直すことだけ」と書いたが、**その操作はこの標本の台本に一度も定義されていない**（No.182 の語彙をそのまま流用した跡）。実装は作らなかったので、既定はモード切替以外に**規則を戻す手段を1つも持たない**——芯2「捨てられない規則は、積める」を、企画の文言より強く体現する形になった。**実装が踏んだ罠も2つある。** B を「2週の粒を1つの統合表示に描き直す」方式で作ると、統合後の座標が元の2週どちらとも一致せず、**粒が1pxも動かないという条件と原理的に両立しない**——「後ろの週だけ消し、前は触らない」方式に変えて解いた。対照の消える粒を DOM ごとアンマウントすると transition が発火しないので（No.177 が踏んだ罠と同型）、要素は常に残して `is-collapsed` で `opacity` だけを切り替えている。**目視で直った点もある。** 初版は B を押しても粒が消えるだけで、読み手が消滅の理由を読み取れなかった。破線の `pair-frame` を足して「このペアは今1つの枠」を描いている——**担体は、それが載る場所が描かれていないと読めない**が、ここでは**規則が作った場所**を描く必要があった。外形は **340×213px**。',
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: '`送る` → `返る` → `受け取る` の順に押す。上段（自分）と下段（戻り）が鏡像になり、受け取ると上段が瞬時に相手の並びになる。`閉じて開く` で再訪する（右上で「相手の並びだと名乗り、印を付け、滑らせ、『並びは自分のまま』を選ばせる」対照に切り替え）',
    principles: ['並びだけを捨てる操作は置けない。並びが担体を持たないから', '返ってきたものに印を付けない', '原因は再訪で消え、結果だけが残る'],
    ecology:
      'No.181 は「**渡せるのは粒だけ。読み方は担体を持たないので渡らない**」と決めたが、それは**行き**の話だった。**帰りは決めていない**——帰りには、相手の手を経た**結果**が付いてくる。週 1〜8 の高さを単調増加の決め打ち表 `[14, 22, 29, 36, 43, 50, 57, 62]` にしたので、相手の規則「大きい順」は**週の完全な逆順**と一致し、しかも恒等順列との**一致する位置が 1 つも無い**置換になる。上段は右肩上がり、下段は右肩下がりの**完全な鏡像**で、**どちらも規則どおりであって優劣が無い**ことが数値を読まなくても伝わる。第1の芯は、**並びだけを捨てる操作を置けない**ことである。既定にその操作は **0 個**——「並びは自分のまま、粒だけ取る」は、**並びが担体を持たないので選択肢として書けない**（No.181 の結論が、ここでは「選べない」という形で出る）。対照は **1 個**置くが、押すと**読み手が最後に見ていた並びではなく初期値へ戻る**（実測で不一致 **8 週 / 8 週**）——**「自分のまま」に相当する並びを、画面はどこにも保持していない。** 第2の芯は**返ってきたものに印を付けない**ことで、受け取った粒の `className` / `background-color` / `border` / `opacity` はすべて **distinct 1 値**。対照は後半4週だけ `rgb(179,58,58)` に色分けし（**distinct 2 値**）、`相手の並びで返ってきました` のバッジを **1 個**出す。既定の可視テキストには他人を指す語が **0 件**（操作ボタンの 3 ラベルを除く。**企画の語をそのままボタンに書くと必ずヒットする**という No.181 が踏んだ罠を避けるため、キャプションは「返」「受け取」を含まない言い換えにしてある）。第3は**出発点が画面の外にあるものを動きで描かない**ことで、受け取りの瞬間の `left` は前後 40 フレームで **distinct 2 値**（`8px` と `218px` のみ、中間値 **0 件**）。対照は **distinct 19 値**で 0.3s かけて滑る——**とっくに起きていたことが、いま起きたことになる。** そして粒の高さの集合は受け取りの前後で**完全一致**（8週とも差 **0px**）——**変わるのは並びだけである。** 第4がこの標本のいちばん苦いところで、履歴の点は `送る` で **+1**、`返る` で **±0**（他人の操作は載らない。No.98 の継承）、`受け取る` で **+1**、そして `閉じて開く` で **2 → 0** になるのに、**並びはそのまま残る**（再訪の前後で差分 **0 件**）。**原因が消えて、結果が残る**——再訪した読み手にとって、その並びは**最初からそうだった**ように見える（No.181 の結論が、今度は**自分自身に対して**起きる）。**企画の条件が1つ、実装不能だった。** C7 は「受け取ったあとの画面と、自分で同じ並びを作った画面が完全一致すること」を要求したが、**この標本には上段の並びを `受け取る` 以外で変える操作が1つも無い**（芯1の裏返し：並びは担体を持たないので「この並びにする」という直接操作を書けない）。比較対象の片方が**到達不可能**である。**条件が原則より強すぎると、実装できる形が存在しなくなる**（No.180 の C6 に続く2例目）。代わりに、上段の粒が由来を示す state を一切持たないこと（`topOrder` は `number[]` のみ）をソースと DOM の両方で確認している。**実装の判断も3つ効いている。** React の `key` を**週番号で固定**し、並びに応じて `left` だけを再計算した——インデックスを key にすると並びが変わるたびに DOM が作り直され、対照の `transition: left` が中間値を作らず（同じノードが移動しないと transition は起きない）、週ラベルの対応も壊れる。受け取ると**下段を空に戻す**——残すと、受け取り後の画面が「最初から相手の並びを持っていた画面」より要素が1つ多くなり、芯2に反する。対照の色分けは「受け取ると上段の 8 週が全部相手発になり、区別できる部分集合が無くなる」ので、対照専用の概念として後半4週だけを塗っている（No.181 の C5 と同型の課題への同じ解き方）。**計測の側の罠も1つ出た。** 対照は位置に `transition: left 0.3s` を掛けているので、**クリック直後にフレームを待たず `getComputedStyle` を読むと補間開始前の値が返り、「ボタンが効いていない」ように見える**（診断用の `data-*` 属性は即座に正しい）。この標本の位置系の実測は 0.3s 以上待ってから読む必要がある。**目視で直った点もある。** 下段を地色のまま空にすると「まだ何も無い場所」なのか「そもそも表示されていない」のかが紛らわしかったので、下段のトラックに薄いストライプを敷き、**空でも枠として存在する場所**だと常に読めるようにした。外形は既定 **340×255.92px** / 対照 **340×285.92px**。',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '`小さいものを落とす` → `まとめて見る` の順に押すと2本に畳まれる。押す順を逆にしても同じ絵になる。対照で `入れ替える` を押すと同じ状態から3本が出る（右上で「適用順を名乗り、入れ替えさせる」対照に切り替え）',
    principles: ['順序は担体を持たない。だから選ばせない', '押した順は、効く順ではない', '順序を選ばせると、跡が消えたあとも効き続ける'],
    ecology:
      'この回で唯一、**苗床に無かった種**である（183 が「規則は積める」と決め、184 が「他人の規則が返ってくる」と決めた時点で、**積んだ規則のあいだの順序**が未決のまま残った）。週 1〜8 の高さは決め打ちの `[8, 15, 25, 8, 30, 30, 5, 5]`。規則は2つ——`小さいものを落とす`（18px 未満を外す）と `まとめて見る`（隣接ペア (1,2)(3,4)(5,6)(7,8) を**高さの合算**で畳む）——で、この2つは**非可換**である。**A → B** は先に小さい粒を落とすので合算にその分が入らず、残るのは **2 本**（高さの集合 **{25, 60}**）。**B → A** は先に合算して大きくなった粒がしきい値を越えるので、残るのは **3 本**（**{23, 33, 60}**）。**同じ台帳・同じ2つの規則・順序が違うだけで、粒の個数も高さも変わる。** 第1の芯は、**順序を固定し、名乗らず、選ばせない**ことである。既定に順序を入れ替える操作は **0 個**、可視テキストに順序を表す語（`順` `先` `あと` `次に` `→`）は **0 件**、数字も **0 件**。対照は順序語 **2 件**（`適用順` と `→`）・数字 **2 件**（`1` `2`）・入れ替えの操作 **1 個**。**順序を固定することが、読み手が覚えておかなくてよい唯一の形である**——選べない規則は、忘れても困らない。第2の芯がこの標本のいちばん効く実測で、**押した順は、効く順ではない**。A を先に押した場合と B を先に押した場合で、既定の全要素 JSON の差分は **0 件**（粒は `[{left:228, top:178, width:44, height:25}, {left:288, top:143, width:44, height:60}]` で完全一致）。第3に、規則の on/off は出来事ではない——既定の生存粒は 20 フレーム最大差分 **0.000px**、`transition-duration` **`0s`** / `animation-name` **`none`**。対照は `0.3s` で、同じ 20 フレームに `8 → 6.09 → 3.08 → 1.95 → 1.03 → 0.34 → 0` と**中間値が 5 フレーム**出る。第4に、履歴の点は A の on で **+1**、B の on で **+1**（0 → 2）、`閉じて開く` で **0**。既定はこのとき**粒の差分 0 件**——消えるのは点だけである。**そして最後に、対照のいちばん苦い壊れ方が来る。** 順序を入れ替えてから `閉じて開く` すると、**履歴の点は 0 個になるのに、順序は残る**（No.179「残るのは規則、消えるのは操作」がそのまま効く）。再訪した画面は初期順序の画面と**一致しない**（差分 **2 件**。p0 が 0 → 23px、p1 が 25 → 33px）。**原因が消えて、結果が残る**——しかも履歴の点が 0 個なので、**読み手には原因が1つも見えない。** **実装が企画の条件を1つ、正直に割った。** C3（押した順 ≠ 効く順）は、本実装では A/B を独立した boolean で持ち、描画時に `computeShape(aOn, bOn, AB)`（第3引数は固定の文字列） と**固定文字列の順序で1回だけ合成する**ので、**押下順を運ぶ経路が構造的に存在しない**。つまり C3 の一致は「2つの実行が偶然揃った」のではなく**自動的に成立する**（ほぼトートロジー）。主張の証明としては最も強い形だが、**「押下順を一度記録してから固定順序で上書きする」ような実装に対する回帰テストとしての感度は無い**——**条件が測っているのは結果であって、結果を生む経路ではない。** **実装が決めたことも3つある。** 合算 0 の組は既定では**描かない**（0px の箱を置かない。No.182「規則のゼロは存在しない」への類推）。対照だけは 0.3s の transition を成立させるため 4 組ぶんのスロットを常時マウントし、高さ 0 を一時的に経由する——**対照専用の装置で、既定にこの発想は無い**。そして**週番号のティックを廃止した**——先行標本（179 / 182）は週選択の操作があったので数字ティックを置いていたが、この標本は on/off 以外の操作が無く、数字ティックがあると C1 の「数字の通し番号 0 件」が**順序の数字か週番号かで曖昧になる**。可視テキストから週番号そのものを無くして、曖昧さを構造的に消している。**React の key についての知見も1つ出た。** 個別週のシェイプ（`w1`〜`w8`）と畳んだ組のシェイプ（`p0`〜`p3`）は**完全に別の key 空間**なので、B の on/off（シェイプが変わる瞬間）には transition が掛からず一瞬で切り替わる。**常時スロットをマウントするパターンでも、シェイプを跨ぐ補間はできない。** **目視で直った点がこの標本の要になった。** 初版は畳んだ組の幅を単週と同じ 14px のままにして高さだけ合算値にしていたので、スクリーンショットで見ると「2週ぶんの合算」ではなく**「1週の値が大きくなっただけ」に読めた**——企画が最重視した「畳まれた粒が2週ぶんだと一目で読めるか」を満たしていなかった。幅を **44px**（隣接2週の器をまたぐ幅）に直し、真ん中に 1px の継ぎ目を足し、トラック背景に週境界の淡い縦線を敷いて、**粒の幅をマス目と比べられる**ようにしている。外形は既定 **336×170px** / 対照 **336×228px**。',
    Component: RuleOrderChangesIt,
  },
  {
    id: 'grain-made-by-the-reading',
    no: 186,
    nameJa: 'その粒は、読み方が作った',
    nameEn: 'Grain Made by the Reading',
    category: 'アナリティクス',
    trigger: '`まとめて見る` を押して4本の合算にする。**合算の粒を押す**（外れずに、その組だけ畳みが解ける）。出てきた単週の粒を押す（こちらは外れる）。`閉じて開く` で再訪し、もう一度同じ合算を押す（右上で「合算を赤く塗って個数を名乗り、押すと合算そのものを消す」対照に切り替え）',
    principles: ['作られた粒に出どころの印を付けない', '作られた粒は操作の受け手になれない。台帳への入口になる', '同じ定規の上に、戻るものと戻らないものが混じる'],
    ecology: 'No.185 の規則 B（`まとめて見る`）は隣り合う2週を畳んで高さを合算するが、**その合算値は台帳に一度も書かれていない**。No.176 は「過去は書き換わらない。書き換わるのは読み方のほう」と決めたが、**読み方が新しい粒を作ってよい**とは決めていなかった。この標本はそこを撃つ。週 1〜8 の高さは決め打ちの `{1:10, 2:14, 3:20, 4:8, 5:16, 6:22, 7:12, 8:9}`、合算は `{24, 28, 38, 21}` で、**どの合算もそれを構成するどちらの単週より高い**（「1週の値が大きくなっただけ」に読まれないための値選び。No.185 が目視で踏んだ失敗の継承）。第1の芯は**作られた粒に出どころの印を付けない**ことで、単週と合算が同時に見える混在状態で `background-color` `border-color` `border-style` `border-width` `opacity` `className` が**すべて distinct 1 値**（`rgb(61,61,61)` / `rgb(61,61,61)` / `none` / `0px` / `1` / `mz-grain-made-by-the-reading-grain`）。**幅だけが distinct 2 値**（`14` / `44`）だが、**幅は「何週ぶんか」という台帳についての事実**を言っているのであって、出どころを言っていない。ただし正直に書いておくと、**読み手は幅の差から「これは単週ではない」ことまでは読める**——読めないのは「単週の値がたまたま大きいのか、読み方が合算したのか」の区別のほうである。対照は合算を `rgb(179,58,58)` に塗り（`background-color` distinct 2 値）、`合算 N件` のバッジで個数を名乗る（禁止語 **3 件** / 数字 **2 件**。既定は**どちらも 0 件**）。第2の芯がこの標本の中心で、**作られた粒は操作の受け手になれない。台帳への入口になる。** 単週の粒を押すと台帳の行が消えるが、**合算の粒を押しても外れない**——代わりに**その組だけ畳みが解けて、中の2つの単週粒が出てくる**。実測で定規の上の粒は押下の前後で **4 → 5（+1）**、対照は **4 → 3（−1）**。**取り消す対象が無いのではなく、取り消す対象は畳みの下に在る。** そして畳みを解くのは**規則を外すことではない**——解いた後も残りの合算は **3 件**が畳まれたままで、規則は生きている（No.183「捨てられない規則は積める」の継承）。`disabled` は使っていない（「いまは押せない」と読まれるうえ、No.116 が別の意味に使っている語彙）。第3の芯は、**同じ定規の上に、戻るものと戻らないものが混じる**ことである。合算は読み方が作ったので畳みを解けば消えて元の2粒に戻るが、単週を外すのは台帳の変更なので**戻らない**——外してから畳み直して解き直す往復の後、既定の生存粒は **1 個**（週2のみ。週1は戻らない）。対照は同じ往復で **2 個**に戻る。第4に、規則の on も畳みの解きも外しも**出来事ではない**——既定の生存粒は 20 フレーム最大差分 **0.000px**、`transition-duration` **`0s`** / `animation-name` **`none`**。対照は `0.3s` で、同じ 20 フレームに**中間値 17 個**（オーバーシュートして `26.3 → 24.36` で収束する）。第5に、`閉じて開く` で履歴の点は **1 → 0** になるが、定規の絵の差分は **0 件**——**合算は台帳に書かれていないのに、規則が残っているので同じ値で再計算される。****企画が自己矛盾していた。** 芯3 の実測文は「その組を**畳み直し**→また解く」と書いて**畳み直す操作の存在を前提**にしていたのに、「企画が決めていないこと」の節は同じ論点を**未決の選択肢**として扱い、「置かないなら C5 の測り方を工夫すること」という逃げ道まで用意していた。**本文の実測例が操作の存在を前提にし、決め事の節がその不在を許容する**——実装は専用ボタンを置かず、`閉じて開く` を畳み直しの代役に充てて両立させた（専用ボタンを足すと「これは畳める特別な粒だ」という**出どころの標識**になりかねない、という理由も併せて報告されている）。**実装が決めたことも3つある。** 再訪後は**畳まれ直す**（解くのは台帳の変更ではなく読み方の変更なので、履歴の点と同じ寿命にする）。当たり判定は**粒本体のみ**（下のマス目では反応しない）。対照の「押すと消えて、解くと戻る」は、`まとめて見る` のトグル自体が台帳と読み方の両方の記録を空にリセットするという**1箇所の実装ミス**に寄せてある——おかげで C3 の対照（合算が消える）と C5 の対照（消したはずの2週が戻ってくる）が**同じ原因から自然に出る**。**罠も2つ出た。** C1 が `className` を測る（No.185 の C1 は測っていなかった）ので、継ぎ目を `is-merged` のような修飾クラスで表現できず、**inline style の `backgroundImage` だけ**で描いている。対照の 0.3s は単週と合算で key 空間が別なので発火せず、個別8枠＋合算4枠を**常時マウント**して `height` を 0 との間で往復させた（No.185 が名指しした「key 空間を跨ぐ補間はできない」の再演）。**そして、親のレビューが目視で2つ直した。** ひとつ、既定のキャプションが「粒を押すと、外れるか、畳みが解ける」と**芯2の結論を文章で先に言っていた**——押して発見させることに意味がある標本で、答えを先に書くと動きで言う必要が消える。`まとめて見る。粒は押せる。` に直した。ふたつ、**履歴の列が点0個のとき完全に空白で、そこが列だと読めなかった**（No.116 がレールを敷いて直した失敗の再演）。外形は既定 **336×170px** / 対照 **336×195px**。',
    Component: GrainMadeByTheReading,
  },
  {
    id: 'both-hands-at-once',
    no: 187,
    nameJa: '二人が同時に触っている',
    nameEn: 'Both Hands at Once',
    category: 'ナビゲーション',
    trigger: '何も押さずに眺める（予告も動きもなく、どこかの週が下がる）。`上げる` を押す（自分の押下だけが 0.18s の動きになる）。伸びている途中に相手の手が同じ週へ入る回を待つ。`閉じて開く` で再訪する（右上で「相手を名乗り、色を付け、滑らせ、衝突で自分の動きを打ち切る」対照に切り替え）',
    principles: ['自分の変化は動く。他人の変化は動かない', '動き終われば、出どころは画面に残らない', '押下は必ず履歴の点になる。相殺されても積む'],
    ecology: 'No.184 は「渡す → 返る → 受け取る」の**逐次**だったので**受け取りの瞬間**があった。**同時には、その瞬間が無い**——いつ相手の手が入ったかを指せない（No.89「見ていないあいだに終わったこと」の常時版）。舞台は週 1〜8 の定規。自分は `上げる` で週を巡回しながら **+8px**、相手は **1.8 秒ごと**に決め打ちの置換 `[2,4,8,1,6,3,7,5]` を消費して **−8px** する（乱数は使わない）。第1の芯は**自分の変化は動き、他人の変化は動かない**ことで、既定は押下中だけ `transition-duration` を `0.18s` にし（それ以外は `0s`）、相手の tick はその state に一切触れない。実測で自分の +8 は 20 フレーム中**中間値 10 件**、相手の −8 は distinct **2 値**（`38 → 30px`）・中間値 **0 件**・`0s`。対照は相手の変化を `0.3s` で滑らせ（中間値 **6 件** / distinct **19 値**）——**とっくに起きていたことが、いま起きたことになる**（No.28）。**この「動くかどうか」は出どころの印ではない。** No.28 と No.113 の既存の決めから自動的に出てくる帰結であって、出どころを名乗るために足した担体ではない。**そしてこの標本のいちばんの収穫は、実装が CSS の側に見つけた。** 自分の +8 が伸びている途中に相手の −8 が同じ週へ入ると、**同じ `height` の transition が進行中に新しい目標値を受け取り、ブラウザが今の中間値から滑らかに向き先を変える**（transition の retargeting）。実測のフレーム列は `50 → 51.9 → 53.5 → 54.9 → 56.0 → 56.95`（ピーク。目標の 58 に**届かない**）`→ 54.75 → 53.7 → … → 50`。**「伸びながら、より低い位置へ着地する」という主題を、JS が何もせずに CSS の標準挙動だけで描いている。** 不連続な飛びは 1 フレームも無い。実装は最初「相手の tick が来たら自分のアニメを止める」を書きかけて、**それが企画の挙げた対照の壊れ方4そのものだった**ことに気づいて引き返している。対照はまさにそれをやる——衝突の瞬間だけ `transition-duration` を `0s` に強制し、`50 → … → 57.64px` の直後に **1 フレームで 50px へ飛ぶ**。**自分の操作が効いたことが、1 フレームも画面に出ない。** 第2の芯は**印を付けない**ことで、既定は 8 粒すべての 6 項目が distinct **1 値**、「この粒は誰が触ったか」を覚える state を既定側が**持っていない**（対照専用の集合だけが覚える）。対照は相手が触った週だけ `#b33a3a` に塗る（distinct 2 値）。第3の芯は、**押下は必ず履歴の点になる。相殺されても積む。** 5 回押して既定の点は **5 個**、うち **2 回**（週2・週4）は高さの純変化が **0**（`data-net="0"`）。**「押したのに効いていない」のではなく、「効いたが、同時に別の力も効いた」。** 対照は同じ台本で **4 個**——衝突した回が省かれ、**押したのに盤面にも履歴にも残らない**（二重の消失）。第4に、既定の可視テキストに禁止語 **0 件**・数字 **0 件**（週番号のティックも置かない。No.185 と同じ理由）、対照は禁止語 **5 件**（`同時` `競合` `相手` `さん` `編集中`）・数字 **8 件**。第5に、`閉じて開く` で点は **5 → 0**、8 粒の高さの差分は **0 件**——**再訪した読み手にとって、その高さは最初からそうだったように見える。****実装が決めたことも4つある。** 週の選び方は**巡回固定**にした（選択マーカーを置くと、それ自体が「これから触る場所」を名乗る新しい担体になる）。相手を**止める手段は置かない**（止められるなら「同時」ではない）。画面外判定も付けない（No.61 に倣い、タイマー駆動という主題に条件分岐を持ち込まない）。高さは 0〜96 でクランプするが、本番の台本では発火しない安全網である。**企画が決めきれていなかった点も1つ出た。** 「同じタイミングで下げる」が**同じ周期の中のどこか**なのか**実時間で近接**なのかを決めていなかった。既定は純変化をライブ計算するのでどちらでも成立するが、**対照の「相殺されたら積まない」は判定を遅らせる設計**になるので、遅延窓より相手の tick が遠いと相殺を検出できない——最初の台本（押してから 1.8s 受動的に待つ）は、この理由で C5 の対照だけが落ちた。**対照の壊れ方のほうが、企画の曖昧さに敏感だった。** 計測の罠も2つ。tick の到達時刻を「検出できた時刻 + 1800ms」で予測すると**検出のためのフレーム収集コストが基準時刻に混入して 300〜500ms ずれる**ので、ブラウザ内で変化を検出した瞬間の `Date.now()` を直接返す形に直した。`capture.html` は StrictMode なので effect が二重実行され、`setInterval` の後始末を書かないとタイマーが二重に走る（5 回押して点がちょうど 5 個であることで裏取りしている）。**そして、親のレビューが目視で3つ直した。** ひとつ、既定のキャプションが `触れたところは伸びる。触れていないところも、変わっている。` と**芯1の結論そのものを文章で先に言っていた**。ふたつ、**履歴の列が点0個のとき読めなかった**（No.116 の再演）。みっつ、履歴の行に `自分` というラベルが置かれていた——**この標本には相手の操作を記録する対の行が無いので、そのラベルは「自分／他」の区別を暗に持ち込む新しい担体**になる。No.184 / 185 は相手役の実体が無いので同じラベルでも問題にならなかった。**第二の主体が実在する標本では、ラベルの要否を個別に判断する必要がある。**外形は既定 **336×208px** / 対照 **336×247px**。',
    Component: BothHandsAtOnce,
  },
  {
    id: 'undo-without-return',
    no: 188,
    nameJa: '取り消しても、絵は戻らない',
    nameEn: 'Undo Without Return',
    category: 'ナビゲーション',
    trigger: '`置く` を3回押して自分の粒を置き、`盤面が動く` を押す（他人の粒が2本、中割りなしで現れる。履歴の点は増えない）。`◀ 戻る` を3回押す——履歴の点が 0 個になっても、盤面には粒が残る（右上で「盤面ごと巻き戻して『元に戻しました』と言う」対照に切り替え）',
    principles: ['取り消しは台帳から操作を取り除くことであって、盤面を過去へ戻すことではない', '巻き戻しを動きで描かない', '戻り切っても最初の絵ではない。そしてそれを名乗らない'],
    ecology: 'この回で唯一、**苗床に無かった種**である（No.186 が「操作の受け手になれない担体が並ぶ」と決め、No.187 が「他人の操作は画面に瞬間を持たない」と決めた時点で、**取り消しの行き先**が未決のまま残った）。既定の答えはひとつの文に尽きる——**取り消しは、台帳から自分の操作を取り除くことであって、盤面を過去へ戻すことではない。**ふだんこの2つは一致するので区別が要らない。**他人の操作が間に挟まると、一致しなくなる。**第1の芯は**取り消しが自分の操作だけを消す**ことで、実装は盤面を `ownGrains` と `foreignGrains` の別々の state に分け、既定の `handleBack` に **`foreignGrains` へ触れる行がコード上に存在しない**（No.184 と同じ「触る経路が構造的に無い」形の保証）。実測で `◀ 戻る` の前後、他人の粒の `left` / `top` / `height` の最大差分は **0px**、個数は **2 → 2**。対照はスナップショットへ丸ごと復元するので **2 → 0 個**——**他人の仕事が消える。**第2に、**巻き戻しを動きで描かない**。既定は取り消し中の 20 フレームが distinct **2 値**（中間値 **0 件**）、`transition-duration` **`0s`** / `animation-name` **`none`**（No.113「再演は動きを1msも変えない」／No.28「出発点が画面の外にあるものは動きで描けない」の継承）。対照は `0.3s` で distinct **7 値**——**過去の出来事が「いま起きたこと」になる。**第3は**印を付けない**ことで、自分の粒と他人の粒の6項目が **distinct 1 値**。**取り消せる粒と取り消せない粒が、同じ顔で同じ列に並ぶ。**対照は `className` / `background-color` / `border-color` / `border-style` が distinct 2 値（`rgb(61,61,61)` 実線 / `rgb(214,214,211)` ＋ `rgb(179,58,58)` 破線）——ただし `border-width` と `opacity` は対照でも 1 値のままで、**対照が見分けに使っているのは色と線種だけ**である（実装の正直な報告）。第4に、**履歴の点の個数と盤面の粒の個数が一致しない**。既定は `置く`×3 ＋ `盤面が動く` の後で点 **3 個** / 粒 **5 本**、対照は **5 / 5**（他人の操作も積むので一致する）——対照はそのぶん**読み手が他人の操作を取り消せてしまう**。そして第5の、この標本のいちばん苦いところ。自分の操作を全部取り消すと履歴の点は **0 個**になるが、盤面は初期状態と**一致しない**（差分 **2 件**。他人の粒が残っている）。**既定はその不一致をどこでも名乗らない**——禁止語（`戻` `元` `取り消` `完了` `失敗` `できません` ＋ レビューで足した `粒` `点` `残る` `一致`）の可視テキストへの出現は**ボタンラベルを除いて全 10 語とも 0 件**、トースト・警告 **0 個**。対照はトーストを **1 個**出して `元に戻しました` と言い、全部戻すと盤面は **0 粒**で初期状態と**一致してしまう**——**それは「元」ではなく、他人の仕事を消した結果である。嘘を言っている。**`閉じて開く` で点は **1 → 0**、盤面の差分 **0 件**、その後 `◀ 戻る` は `disabled`（No.116 の継承）——**再訪した読み手は、自分が置いた粒すら取り消せない。****企画の条件が、家の語彙と正面から衝突した。** C5 は既定の可視テキストに `戻` の出現 0 件を要求したが、COMMON.md と企画書自身が `◀ 戻る`（No.116 の継承）を舞台の語彙として指定している。**取り消しのボタンに「戻る」を使わずにこの標本は作れない。** 実装は「禁止語の検査は状態説明文を対象とし、操作ボタンのラベルは対象外」と読み替えて解いた——**条件の適用範囲（ボタンラベルを含むか）を企画が明文化していなかった**、という形の穴である。**実装が決めたことも4つある。** 他人の操作の中身は「置く」にした（「外す」だと自分の粒が対象になり得て、他人が自分の担体に触れることになる）。1クリックで **2 個**まとめて置く（1個だと点3個・粒4本になり、不一致は出るが企画が例示した 3 / 5 にならない）。`◀ 戻る` は**複数回押せる**（1回きりだと芯「戻り切っても最初ではない」を実演できない）。そして**対照に由来の色分けを足した**——これは企画の対照4項目に無い**5つ目の壊れ方**で、「印を付ければ安全」というどんぶり実装をそのまま演じさせ、**印を付けても巻き戻しは他人の粒を消す**ことを対照に語らせている。**罠も2つ。** 対照の逆再生は、消える粒を state 配列から即座に取り除くと DOM ノードが unmount されて `transition` が一切効かない（同一ノードの値変化でしか発火しない。No.177 / 183 が踏んだのと同型）——対象週を 300ms だけ `height: 0` で描画し続けてから配列を差し替えて解いた。トーストを `top: 8px; right: 8px` に置いたら**モード切替の `対照` ボタンと重なって隠れた**——数値条件とは無関係なので、目視でしか見つからない。**そして、親のレビューが目視で2つ直した。** ひとつ、既定のキャプションが `置いた粒だけを消す。盤面に残るものもある` と**結論を先に言っていた**。ふたつ、`盤面 N粒・履歴 N点` の注記が**不一致を数字で名乗っていた**——芯が「説明しない」と決めているのに、C5 の禁止語リストにその語（`粒` `点`）が入っていなかったので、**C5 を全部通したまま主張が死んでいた**。注記は既定から JSX ごと消し、測定に要る値は不可視の `data-grains-total` / `data-history-count` に移した（**測れるが、読めない**）。注記を消したぶん外形は **340×201px → 340×180px** に縮んでいる。**実装が書き直しの途中で同じ罠を踏み直したことも報告されている**——中間案のキャプションに `戻るで、ひとつ前へ` と書いて、ボタンラベル以外の場所に `戻` を持ち込んだ。**禁止語の検査は1回書いて終わりではなく、書き直すたびに走らせる回帰テストとして使う必要がある。** 外形は既定・対照とも **340×180px**。',
    Component: UndoWithoutReturn,
  },
]
