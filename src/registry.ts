...
import TriedAndCouldNot from './specimens/tried-and-could-not'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    no: 147,
    nameJa: 'その約束を、まだ知らない',
    nameEn: "The Promise You Don't Know Yet",
    category: 'ゲーム',
    trigger:
      '空きの点にポインタを乗せる／占有チップに乗せる（同じ種類のものが同時に濃くなる）。`使う` / `足す` で原資の右端が動き、`次の週へ` で現在地が進む（右上で「初回だけ吹き出しで案内し、点を左から順に湧かせる」対照に切り替え）',
    principles: [
      '初見と100回目で、絵を1pxも変えない',
      '教えない。引ける状態にしておくだけ',
      '濃さは触っているあいだだけ。事実の担体にしない',
    ],
    ecology:
      'No.146 の実装者は実物を見て正直にこう書いた——**「読めるのは、空きの点＝その週まで原資が届いている、という約束を既に知っている読み手だけ」**。この標本はその限界を正面から撃つ。ただし**消すのではなく、動かす**。No.120〜122 が撃った「画面が答えを持っていない」は全部**事実**についての知らないだったが、ここで画面が知らないのは**読み手**であり、しかも**観測できない**。第1の芯は、観測できないので**観測しない**ことである。既定は初見でも100回目でもまったく同じ絵を出す——`week` / `runwayEnd` / `hoverGroup` の3つの state はどれも「何回目の訪問か」を持たず、**既定側のコードには「これは初回か」を尋ねる分岐が1つも無い**。実測では同じ7手の操作列を2周し、各ステップの DOM スナップショット（要素数・`class`・`data-*`・矩形）が**全ステップで完全一致**した。対照は吹き出しが **1 → 0 個**になり、2周目は **0 個**のまま——**同じ画面が、読み手によって別のものになる**。第2の芯は、教える代わりに**確かめられるようにする**こと。定義を先に見せる（＝出来事にする。No.114「動かした時点で起きたことになる」に抵触）のではなく、**読み手が触ったときにだけ担体が自分の群を名乗る**。空きの点に乗せると空きの点**だけ**が5個同時に濃くなり、占有チップに乗せると占有チップ**だけ**が濃くなる。**事実は1つも言っていない**——数も意味も良し悪しも言わず、「これらは同じ種類だ」を読み手自身の操作で確かめられる状態にしてあるだけである。No.95 の「囲む・塗る・指す」のどれでもない**濃淡の同期**という動詞を、95 の語彙集合に足さずに、既存の点の `class` だけで実装している。第3は担体を増やさないことで、ホバーで増える要素は **0 個**、変わるのは `background-color` / `border-color` / `box-shadow` だけ——`box-shadow` は描画上の滲みで `getBoundingClientRect` を広げないので、実測でホバー中の矩形差は空き・占有とも **0.00px**、`animation-name` は **`none`** のままになる。濃さを No.74 の「薄さ＝確度」と衝突させないため、変化は**触っているあいだだけ**で、離せば `transition`（`animation` ではない）で完全に戻る——触っていないフレームの `background-color` は **distinct 1 値**（`rgba(0,0,0,0)`）で、ホバーして離した後の computed style は触る前と**完全一致**する。そして既定はロード後2秒間に自動で変化する要素が **0 個**＝**教えるための出来事を1つも起こしていない**。対照は初回に空きの点が **5 個**、左から順に湧く（120ms 時点で 1/5、700ms で 5/5）——1回目に見た読み手には案内だが、**2回目以降の読み手には出来事の予告に見える**。**この標本は限界を解消しない。限界の位置を1段動かすだけである。** ホバーで分かるのは「これらは同じ群だ」までで、「原資」という語もその意味も読み手は得ない。146 が抱えていた限界はここでも消えていない——変わったのは、読み手が**自分がまだ知らない群がある**ことに、**自分の操作で気づける**ようになった点だけである。**限界の中身は同じで、限界に気づく経路が1つ増えた。** 実装が企画の決めていなかったことを3つ決めた。ひとつ、占有チップに No.146 の「届かない予定」の意味（`runwayEnd` に連動する `opacity`）を**継がせない**——この標本の主題はホバーの群分けなので、2つの群を意味的に直交させたほうが芯が立つ。ふたつ、対照の「1回だけ」を保持する `seenOnboarding` を `resetAll` の**対象外**にした（モードを往復しても再表示されない＝「見逃した読み手には二度と出ない」を対照側で正しく再現する）。みっつ、履歴トラックを**置かなかった**——この標本の主張にも C1〜C6 にも要らない。計測の側の罠も1つ出た。**C1 の各クリック後に 260ms の落ち着き待ちを入れないと、ボタン自身の `translateY(-1px)` の遷移が測定に混ざって、標本の状態とは無関係なサブピクセルの不一致が出る**——「2周が一致すること」を測るときは、測る対象ではない要素の遷移が終わるまで待つ必要がある。',
