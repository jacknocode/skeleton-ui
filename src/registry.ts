...
import PresenceEcho from './specimens/presence-echo'
import StaleRefresh from './specimens/stale-refresh'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    nameEn: 'Undo Unravel',
    category: '入力',
    trigger: '「実行する」→ 猶予のあいだに「取り消す」',
    principles: ['残り時間を長さで持つ', '逆再生で戻す', '確定は跳ねない'],
    ecology:
      '実行してしまったあとにも、まだ戻せる時間がある——その残りを数字で数えず、札の縁を縫っている糸の長さで持たせた標本。実行の瞬間、札は1px沈んでいったん戻る。これは受領であって結果ではない。そこから糸が5秒かけて一周ほどけていくが、ここだけは絶対にイージングを付けない。等速でないと「あとどれだけ戻せるか」が読めなくなるからで、緩急を付けないことが設計になっている珍しい箇所。ほどけ切ると糸端がひと呼吸だけ張り、その80ms後に札が跳ねずに沈んで実線に落ちる——確定は祝わず、動きが止まることだけで伝える。取り消すと、糸は残った位置から往路と同じイージングで縫い戻り、札は元の位置へ吸い戻される。ここでオーバーシュートを付けないのが芯で、跳ねた瞬間それは「元に戻った」ではなく「新しく生まれた」に見える。猶予中の札は薄くなるが高さは変えない。レイアウトが動くと、戻る先が分からなくなる。',
  ...
  },
  {
    ...
    category: 'アナリティクス',
    trigger: '「広告を出す」「採用する」→「週を進める」',
    principles: ['押下と反映を別の拍にする', '効く量は先に・効く時は後に', '呼吸＝まだ効いていない'],
    ecology:
      '押したのにグラフが動かない——それを故障ではなく予告に変える標本。施策を押すと線は一切動かず、代わりに時間軸の「効き始める週」へ薄い影の帯が置かれる。押下が空振りにならないのは、必ず何かが置かれるからで、受領と結果を別の時刻に分けることがこの標本の芯になっている。影の上端の高さはそのまま「どれくらい効くか」なので、量は先に読めて、待つのは「いつ」だけになる。待っているあいだ影はゆっくり呼吸し続ける——呼吸している＝まだ効いていない。週を進めて現在位置が影に届くと、その影の呼吸だけがぴたりと止まり、線が少し弾みながら持ち上がって点が灯り、役目を終えた影は溶けて消える。複数の施策が同じ週に重なっても同時には光らせず、260msずつずらして1つずつ実体化させる。同時に光らせると、どの施策がどの持ち上がりを生んだのかの対応が消えるからで、因果を読ませたい図では「一斉」は情報を捨てる動きになる。',
  ...
  },
  {
    ...
    category: '入力',
    trigger: '「+120 を送る」「+120 を送る（弾かれる）」',
    principles: ['速さの正体を隠さない', '来た道の逆再生', '震えるのは戻り切ってから'],
    ecology:
      '押した瞬間に数字が動く。judgment を待たないその速さが、楽観的更新の値打ちそのもの——だがそれは「まだ通っていない結果を先に見せている」という嘘の上に立っている。この標本は、その嘘に姿を与える。増えたぶんはわずかに浮き、斜線で塗られ、1.4秒周期で浅く呼吸する。この3点が揃って初めて「まだ実体ではない」が伝わり、1つでも欠けると単なる増加に見える。承認されれば浮きが落ちて実塗りに変わり、呼吸が止まって1px沈む。否定されると、帯は伸びたときと寸分違わぬ経路を減速だけのイージングで逆再生して戻り、数字も同じ時間で逆回しに落ちる。別の動き——フェードアウトや縮んで消える——で戻すと、それは巻き戻しではなく「何か別のことが起きた」に見えてしまう。震えるのは戻り切ったあとに一度だけ。戻りながら震えると、失敗が伸びの一部に混ざって読めなくなる。承認待ちは積み上げられ、それぞれ独立に判定される。',
  ...
  },
  {
    ...
    nameEn: 'Scroll Baton',
    category: 'ナビゲーション',
    trigger: '枠の中を縦にスクロールする（戻すと巻き戻る）',
    principles: ['再生ヘッドはユーザー', '時間ではなく距離のイージング', '主役に transition をかけない'],
    ecology:
      '再生ヘッドを図鑑ではなくユーザーが握っている、唯一の標本。点が棒に伸び、棒が丸まって輪になる——その進み具合はスクロール量そのもので、指を止めればその形のまま止まり、戻せばそのまま巻き戻る。ここでは図鑑の看板であるオーバーシュートが原理的に使えない。行き過ぎて戻る動きを逆再生すると「戻ってから行き過ぎる」になり、予備動作が事後動作に化けるからで、巻き戻せる動きに予備動作を仕込んではいけない。主役には transition も一切かけない（時間で補間すると、指を止めた瞬間に主役だけが動き続けて「スクロール量＝再生ヘッド」の契約が破れる）。では何で気持ちよさを作るのかというと、時間ではなく距離にイージングをかける——スクロール量に easeInOutCubic を通してからモーフに渡す。左右対称の曲線なので、逆走しても同じ手触りで返ってくる。跳ねの代わりに置いた報酬が、パネルの中央で止まったときだけ主役の下に現れる1本の線で、これも時間ではなく位置の関数なので行きでも戻りでも同じ条件で出入りする。主役・キャプションの濃さ・右端のつまみは、ぜんぶ同じ1つの数から導いている。',
  ...
  },
  {
    ...
    no: 71,
    nameJa: '他人の操作のこだま',
    nameEn: 'Presence Echo',
    category: 'フィードバック',
    trigger: '「自分が変える」「他人が変える」「目を離す」',
    principles: ['跳ね＝自分がやった印', '他人には必ず予備動作', '残光は時間ではなく視線で引く'],
    ecology:
      '図鑑のほかの標本は全部「自分ひとりと、システム」の2者しか想定していない。画面を動かしているのが自分以外の誰かであるとき、それをどう語るかがこの標本の中身で、芯は「誰がやったかを、色やアバターではなく緩急で語る」ことにある。自分の操作は押した瞬間に動き、跳ねて確定する——自分は予告を知っているので予備動作が要らない。他人の操作は逆で、まず気配（溝を滑ってくる細い印と、行に差す淡い墨）が先に来て、そこから180ミリ秒だけ何も起きない間があり、それから値が跳ねずに滑って入れ替わる。この空白がこの標本のいちばん大事な部品で、詰めると「押したら効いた」に戻って主題が消える。だから空白は CSS の delay ではなく JS のタイマーで持たせてある（グローバルCSSが duration を潰す reduced-motion でも、気配→間→変化 の順序だけは残る）。跳ねを他人に付けてはいけないのは、跳ねが「自分がやった」の印だから。付けた瞬間、他人の変更が自分の操作に見える。目を離している間に起きた変更には気配が来ない——受け取っていないのだから当然で、代わりに濃い残光が引かずに留まり、ヴェールが晴れて目が戻った瞬間から薄れ始める。留守中の変化は、見られるまで待っている。',
    Component: PresenceEcho,
  },
  {
    id: 'stale-refresh',
    no: 72,
    nameJa: '古くなった数字',
    nameEn: 'Stale Refresh',
    category: 'アナリティクス',
    trigger: '放っておく（7秒で退色しきる）/「更新（値が動く）」「更新（変化なし）」',
    principles: ['鮮度は濃度で持つ', '更新と変化を別の拍で言う', '変わらないものは動かさない'],
    ecology:
      '誰も何もしていないのに古くなっていく、という変化を受け持つ標本。鮮度を「3分前」という時刻ラベルで説明せず、濃度そのもので持つ——数字は時間とともに薄れ、輪郭がわずかに緩み、下線が痩せる。退色は線形で進む（緩急を付けると「何かが起きた」に見えてしまい、時間が均一に流れていることそのものが内容なので、ここに緩急はいらない）。動かすのは opacity と blur と scaleY だけで、幅や字間のようなレイアウトを動かす手は使わない。古くなることが「値が変わった」に見えてしまうからで、薄さの床も 0.42 で止める（消えるのは「古い」ではなく「無い」）。更新は2拍に割る。第1拍は全タイルが同時に濃さを取り戻す動きで、これが「届いた」を言う——ここで時間差を付けると「変わった」に混ざる。140ミリ秒の沈黙をはさんで、第2拍でようやく変わった数字だけが名乗る。変わらなかった数字は第2拍で一切動かない。全部動くと、どれが変わったのか読めなくなる。「更新（変化なし）」のボタンが残してあるのは、第1拍だけが起きて第2拍が起きないことを見せるためで、この2つを別の動きで言い分けられていることの証明になっている。動きを減らす設定では退色を途中の濃さで凍らせる（そのまま潰すと、開いた瞬間に薄れ切った壊れた画面に見える）。',
    Component: StaleRefresh,
  },
]
