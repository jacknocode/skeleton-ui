...
export const specimens: Specimen[] = [
  ...
  /* ---------- No.84〜86「置き換わるとき、何を引き継ぐか」 ---------- */
  {
    ...
    ecology:
      '図鑑83種は全部ひとつの部品の中で閉じていて、画面が入れ替わる場面が1つも無かった。この標本だけ、一覧の1枚が詳細の見出しへ**そのまま昇格する**。同一性を担保しているのは形でも色でもラベルでもなく、**一度も消えなかったという事実**ひとつきり。6枚のカードは React ツリーに常時マウントされたままで、詳細へ移っても生まれ直す要素が1つも無い。位置と寸法は state で書き換えて瞬時にワープさせ、その飛びを FLIP が滑らかに見せる——クリック直前の矩形を getBoundingClientRect で取り、再描画後に最終矩形を測って差分を transform で打ち消し、次フレームで 420ms・cubic-bezier(0.22, 1, 0.36, 1) をかけて 0 へ戻す。主役の CSS には opacity の遷移ルールを1行も書いていない（実測、遷移中42フレームすべてで opacity = 1.000）。ここでぷるんを使わないのは、行き過ぎが「着いた」の祝いだから。祝ってしまうと主題が引き継ぎではなく到着に化ける。上部の「引き継ぐ／引き継がない」を倒すと、同じ主役ノードに FLIP の代わりに 420ms のクロスフェードがかかる。最終的な位置も寸法も1pxも変わらないのに、opacity が一度 0.000 まで落ちるだけで「同じものが動いた」が「別物に差し替わった」に化ける——**位置と寸法という事実は変えず、担保の手段だけを入れ替えた対照**。No.85 では同じクロスフェードが正しい語彙になるので、技法そのものに善悪は無く、何を引き継ぐかが真偽を決める。周りの5枚は translate を一度も使わない（実測 0px）。opacity 1→0（220ms）と scale(0.96) だけで退く。退場が移動を持つと主役の移動と混ざり、どれが昇格したのか読めなくなる（No.79「動かない棒」の逆版）。戻りは往路と同じ尺・同じ緩急で逆再生する。No.67 の「戻り道は往路と変える」は**取り消し**の語彙で、ここは可逆な移動だから逆再生が正しい——同じ図鑑の中で則が反転する場所として置いてある。実物を見て企画に足りていなかったのは、**「器が先、中身はあと」をカードの内側だけの話にしていたこと**だった。初稿は詳細の本文パネルと戻るボタンを遅延なしで出していて、開始140ms のフレームに「退場中の一覧」「移動中の主役」「詳細の調度品」の3層が同時に写る。3つが同時に読めると、引き継がれた1枚がどれだったか分からない。調度品を主役の着地後（300ms）から出すよう直し、退場中カードが 0.05 を切る 233ms と調度品が現れ始める 367ms の間に 134ms の空白を作った。閉じるときは同じ問題が逆向きに起きる（調度品が消えきる前に一覧が戻ってくると、修正前は5フレーム重なっていた）ので、調度品は遅らせず120msで先に消し、一覧の復帰を130ms遅らせて追いかけさせている。**引き継ぎは主役1枚の設計では足りず、画面に出ている全部の到着順を決めることだった**。',
  ...
  },
  {
    ...
    ecology:
      '待ち終わった瞬間の受け渡しを扱う標本。図鑑の「読み込み中」は No.11（ローディング）しか無く、あれは待たせる時間の演出で、待ちが明けるところは誰も見ていなかった。実際に体験を壊すのはそこにある。骨（グレーの棒）はプレースホルダではなく**実体の型**として設計してある：行の高さ44px、数値カラムの幅64px を骨と身が同じ定数から出し、両者を同じ行要素の中に絶対配置で重ねる。行そのものは「読み込む」を押しても一度もアンマウントされない——寸法をDOM計測で合わせにいくのではなく、構造の側からズレを起こせなくしてある。実測で骨→身の行上端の変化は全5行 0.00px、高さの差も 0.00px。ここが1pxでも狂うと、置き換わった瞬間に行が飛んで、読んでいた場所を失う。骨は一斉に消えない。5行それぞれ別の時刻に届く（到着間隔の型は 140 / 90 / 260 / 70 / 180ms で、最大÷最小＝3.79）。等間隔で消すと機械の目盛りに見え、「通信が返ってきた」に読めない——No.74 で帯の左端と右端の尺を変えたのと同じ理由で、非対称さが「向こう側で個別に起きている」の唯一の説明になる。骨→身はクロスフェード（骨が160msで消え、身が2px下から200msで現れる、同時開始）。図鑑では No.84 がクロスフェードを禁じ手として使っているが、ここでは正しい：骨と身は別物で、引き継ぐのは枠だけだから。**同じ技法が、何を引き継ぐかによって真偽を変える**。骨の待ちは呼吸（opacity 0.5⇄0.85、1.4s）だけで語り、斜めに光が走る shimmer は採用していない。光が走ると「進捗している」に読めるが、実際には何も進んでいない（No.61 / No.67 の「待ちを長さで語らない」の系）。この標本のいちばんの発見は、速さのほうが壊れることだった。60msで返ってくる回線で骨を律儀に出すと、実測28〜104msで骨が現れて消える——速いのに不安定に見える。だから閾値を2つ持たせた：120ms 以内に届くと見込める間はそもそも骨を出さず空欄から直接身へ置き換え（実測、速い回線＋閾値ありで骨の出現は0回）、一度出した骨は最低400ms 表示する（実測の下限401.6ms）。速いのにチラつくくらいなら、律儀に待たせたほうが安定して見える。上部の「閾値なし」は両方を外した対照で、同じ到着タイミングのまま骨がちらつく。閾値はどちらも JS のタイマーが持っている：動きを控える設定では CSS の animation-duration が潰れるので、到着のスケジュールを CSS の尺に載せると、その人の画面だけ受け渡しの順序が変わってしまう（No.67 で見つけた「残り時間は装飾ではなく情報」の、受け渡し版）。',
  ...
  },
  {
    ...
    ecology:
      'No.84 と No.85 が「引き継ぐ」側なら、これは引き継がない側。同じ見た目の5行に、性質の違う2つの変化を並べてある。「並べ替え」は同じ5件の順序が変わるだけなので**行は消えずに移動する**（FLIP、opacity は実測で全区間 1.000、尺は移動距離に比例して 260〜420ms、緩急は減速のみ）。「次のページ」は別の5件が来るので**行は1pxも動かない**（実測、枠の Δy = 0.000px）。中身だけが入れ替わる——旧い中身は 140ms で下へ3px沈みながら消え、新しい中身は40ms遅れて 200ms で上から3px現れる。この標本の則は一行で言える：**引き継ぐなら消さない、引き継がないなら動かさない**。難所は 84 の裏返しで、「動かしてはいけない」ではなく「同じに見せてはいけない」。入れ替えを縦のスライドで書いた瞬間、それは移動の語彙を借りることになり、別のデータが来たのに「同じ行が並び替わった」に読めてしまう。だから同一性の軸（移動＝縦の大きな変位）と入れ替えの軸（不透明度＋3pxの微小変位）を**直交させて**ある。3px という数字は「動いていない」と読める上限で、実測もちょうど 3.000px。「語彙を混ぜる」トグルは、その嘘をわざと踏む対照になっている：並べ替えをクロスフェード（opacity 最小 0.000）に、入れ替えをスライド（Δy = 40.000px）に差し替えると、どちらも**間違い方が逆**になる。並べ替えは自分の追っていた行を見失い、入れ替えは別のデータであることが伝わらない。いちばん細かくて決定的な一手は、席番号をどちらに属させるかだった。行に持たせると数字が行と一緒に動いてしまい、「何位から何位へ動いたか」が読めなくなる。順位は席の名前なので席に置く——実測で席番号の Δy は並べ替え・入れ替えともに 0.000px、動くのは中身だけ。実装で分かったのは、**同一性の担い手が React の key ではなかった**こと。枠（席）が「一度も消えない」ことを構造で保証するには key を席に固定するしかなく、そうすると key はもう行の同一性を運べない。行の同一性は perm（新しい席に元はどの席の持ち主が来たか、の対応表）というデータ層の値として運び、その対応から FLIP の差分を引いている。ページ送りでは perm を作らず新しい ID でデータごと作り直す——この**「運ぶか、運ばないか」の一手だけ**が2つの語彙を分けている。key は同一性の宣言ではなく、再マウントを避けるための道具でしかない。',
