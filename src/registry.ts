...
import DuplicatePlace from './specimens/duplicate-place'
...
export const specimens: Specimen[] = [
  ...
  },
  {
    ...
    category: 'ナビゲーション',
    trigger: '間取り図をドラッグして動かし、倍率のつまみを掴む。「閉じる」→「ひらく」で戻る（右上で「中心の座標で持つ」の対照に切り替え）',
    principles: ['行の代わりになるのは名前であって座標ではない', '「近い」は画面の物差しで決まる', '持てないときは、持てないと言う'],
    ecology:
      'No.90〜101 の12種は、ずっと1つの前提の上に立っていた——**現在地は「行」に載っている**。No.97 が「座標ではなく行の同一性で持て」と言えたのは台帳に行という不変量があったからで、地図・キャンバス・波形のような連続空間には、それに当たるものが1つも無い。素朴な答え（ビューポート中心の座標＋倍率）は、No.97 が撃った座標を2次元へ広げただけ——それが対照モード。壊れていない間は対照も完璧に働く（何も起きずに閉じて開くと、既定・対照とも中心の誤差 **0.0px**）。壊れるのは、見ていたものが動いたとき。実測で、既定は復元後も配電盤の**画面座標**が保存時と同じ（誤差 0.0000076px）で、中心は (500,150)→(590,220) と他人が動かした +90/+70 のぶんだけ追従する。対照は中心を誤差 **0.0px** で守り、その結果、配電盤は cx=302px——幅300px の枠の**外**へ出る。**座標は完璧に保存されているのに、見ていたものが画面に居ない。** No.97 の「正確であることと正しいことは別」を、行の無い側で再演した形。この標本の答えは、現在地を**近傍の名前のあるもの（アンカー）＋そこからの相対＋倍率**で持つこと。**行の代わりになるのは名前であって座標ではない。** そして芯は「近い」の定義にある。距離だけで選ぶと、世界の物差しの順位と画面の物差しの順位が一致してしまい（どれにも同じ倍率がかかるので順位が変わらない）、**倍率が変わっても選択が変わらない**——画面に数pxでしか写っていない、読み手が識別できないものがアンカーになりうる。実装で定義し直した: **アンカーの資格は「名前が読める大きさで写っていること」**。`canReadName(f, zoom)` という1つの述語が、ラベルを描くかどうかと、アンカーになれるかどうかの**両方**を決める。実測でも倍率 0.6 では配電盤（40×28）の見かけ短辺が 16.8px（閾値 22px 未満）で名前が出ず、アンカーは給湯室。倍率 2.2 では 61.6px になって名前が出て、いちばん近いのでアンカーになる。世界距離は配電盤 18.68 / 給湯室 61.77 で**終始配電盤のほうが近い**——距離だけを見ていたら、この乗り換えは起きなかった。**読めない名前は、名前ではない。** ズームの不動点も同じ思想で、保つのは幾何の中心ではなく**意味の中心**（アンカー）にした。倍率 0.9→2.2 を30ステップで動かす間、アンカーの画面座標の最大偏差は **0.0000px**、フレーム間ジャンプは 0.0001px（No.100「連続な変化には連続な補正」の空間版）。**掴んでいる間はアンカーを選び直さない**（30ステップ中 30/30 同一）。選び直すのは離した瞬間だけで、2.2→0.6 では release 時点の見かけ短辺が配電盤 16.80px・給湯室 24.00px となり、**資格の喪失**で給湯室へ乗り換わる。他人が別のものを動かしたときは中心も画面座標も動かない（誤差 0.0px、帯0個）——**他人は主役にならない**（No.98 の継承）。第3の芯は**持てないことを認める**こと。名前のあるものが1つも資格を持たない空白地帯を見ていた場合、現在地は保存しない。開くと帯が1個出て（`▸` は **0個**——行ける先が無いなら押せない導線を出さない）、ビューはホーム (150,90) から始まる。対照はこの1点でだけ勝つ（保存座標へ誤差 0.0px で復元する）。**対価は正直に払う。** アンカーが削除されていたときは、帯が出て**3秒放置しても中心は1pxも動かず**、押すと尺ゼロで保存座標へ着地し（毎フレーム記録で現れた中心値は2つだけ＝中間状態なし）、着いた先に「ここに在りました」の跡が1個残る。文言は必ず「**おおよその場所へ**」と名乗る——**座標は現在地にはなれないが、消えたものの跡としてなら使える。ただし近似だと名乗る。** 実装が企画の誤りを1つ暴いた。企画は「倍率を**上げる**とアンカーが乗り換わる」と書いていたが、アンカーの画面座標を固定したまま倍率を上げる限り、他候補の画面距離は倍率について増える一方で、追いつきようがない。総当たり（8什器 × 開始オフセット、128,520 通り）で 0.9→2.2 方向の乗り換えは **0件**、逆方向は **29,838件**。**乗り換えが起きるのはズームを下げる側**で、しかも理由は距離ではなく資格の喪失だった——企画が「近い」の定義に置いた資格が、そのまま乗り換えの引き金になっている。目視でも1つ直している。初稿は配電盤が小さすぎてラベルが「配…」としか出ず、**この標本の主張そのもの（行の代わりは名前）が画面に出ていなかった**。抽象的な px 閾値をやめて資格を「名前が読める大きさ」に定義し直し、ラベル表示と資格判定を1つの述語に束ねて解決した。',
  ...
  },
  {
    ...
    no: 103,
    nameJa: '同じ行が2か所にある',
    nameEn: 'Duplicate Place',
    category: 'ナビゲーション',
    trigger: '同じ項目が2つのグループに出ている台帳で読みかけを決め、「閉じる」→「ひらく」／グループ分けを切り替える（右上で「行 id だけで持つ」の対照に切り替え）',
    principles: ['行の同一性は場所の一意性ではない', '同一性と現在地は別の担体で描く', '一意なら黙って指し直し、候補が複数なら選ばずに提示する'],
    ecology:
      'No.97（戻り先が変わっている）は「現在地は座標ではなく行の同一性で持て」と結論したが、そこには書いていない前提がもう1つあった——**行 id が、画面上の場所を一意に決めること**。グループ分け・タグ・ツリーでは同じ項目が複数の場所に現れ、id だけでは「どこで読んでいたか」が決まらない。実測でそのまま出る。担当C の下の「見積りの確認」を読みかけにして（枠内 y=68px、`scrollTop=256`）閉じてひらくと、対照（＝No.97 の答えのまま id だけで持つ）は `scrollTop=196` で**担当B の下の同じ行**に着く。着地行の itemId は保存値と `===` で一致する。**同じ行に着くのに、違う場所に着く。** No.97 の対照は台帳が変わって初めて壊れたが、この標本の対照は**何も起きていなくても壊れる**——同じ行が2か所にあるという事実そのものが、id では足りないことの証拠になっている。既定は itemId ＋ **経路**（どのグループの下で読んでいたか）で持ち、両方が一致する出現へ戻る（`scrollTop` 256→256、上端 y=68px 誤差0、帯0個）。第2の芯は**担体**。同じ行が2か所にあるとき、担体を1つで済ませると必ず嘘になる——片方だけ強く点けると「もう片方は別の行だ」と読め、両方強く点けると「現在地が2つある」（No.95 が別の意味で使った言い方）と読める。だから**同一性の印**（左端の細い線＋薄い塗り。その項目の全出現に付く。実測2個）と**現在地**（行を囲む。実測1個）を別の担体に分け、しかも**両モード共通**にした。対照でも印は出る——だからこそ「印は2か所に出ているのに、囲みが着いたのは自分が読んでいたほうではない」が1画面に並ぶ。第3は**指し直しの基準**。経路が消えたとき（グループ分けの切替）、候補が1つなら黙って指し直す（期日別では「見積りの確認」は今週に1か所だけ。帯0個、上端 y=68px、毎フレーム記録で現れた `scrollTop` は2つだけ＝尺ゼロで中間状態なし）。候補が2つ以上なら**選ばずに提示する**（タグ別では 見積 と 至急 の2か所。帯1個・`▸` 2個で、押すまで `scrollTop` は3秒放置しても 196 のまま。押すと選んだ側へ尺ゼロで着地）。**迷う理由が無いときに聞くのはただの手間で、迷う理由があるときに機械が選ぶのは No.94 の「頼まれていない移動」になる。** 単一出現の行（請求書の発行）では既定と対照の実測が完全に一致する（`scrollTop=34`・着地 index=2・帯0個）——**主張が効くのは複数出現のときだけで、余計な差を作らない**。実装が企画に無かった罠を1つ暴いた。**見出しの直後の行は、上端を枠内 y=68px まで押し下げられない。** `FRAME_ALIGN_Y(68) > GROUP_H(26)` なので必要な `scrollTop` が負になり、ブラウザが0で頭打ちにする。No.101 が末尾で見つけた頭打ち（一番下の行は引き上げきれない）の**ちょうど鏡写し**で、対策も鏡写しになった——先頭に定数から導出した余白（`TOP_SPACER_H = FRAME_ALIGN_Y − GROUP_H = 42`）を常設している。**同じ物差しを使う限り、入れ物の両端で同じ限界が出る。** もう1つ、**目で見るまで出なかった誤り**がある。受け入れ条件は全項目通っていたのに、**2つの出現が1画面に入らなかった**（間隔196px・可視高204px・行高34pxで、どちらを枠内 y=68px に置いてももう片方が枠外へ出る）。直したのは実装ではなく**台帳の設計**で、グループの所属を組み替えて間隔を60pxにした。**主題が1画面に入るかどうかは、実装ではなくデータの寸法で決まる。**',
    Component: DuplicatePlace,
  },
  {
    id: 'no-place-yet',
    no: 104,
    ...
    nameEn: 'No Place Yet',
    category: 'ナビゲーション',
    trigger: '「ひらく」を押した直後に ↓ / Enter / Esc を押す（右上で「先頭に置く」の対照に切り替え）',
    principles: ['描くのは「無い」ではなく、次にどこから始まるか', '出発地の無い移動は、移動として描かない', '言葉が要るのは次の一歩が無いときだけ'],
    ecology:
      '**No.101 はここを踏んでいた。** あれは開いた直後の現在地を「台帳の頭」に置いたが、それは「先頭に**居る**」であって「まだ**居ない**」ではない。この2つを撃ち分けていなかった。ところが「無い」は担体の不在でしか描けないので、担体を足して描こうとすると必ず別の事実と潰れる——読み込み中も、空の台帳も、まだ選んでいないも、画面の上では同じ「何も点いていない」になる。この標本の答えは、**描くのは「無い」ではなく、次にどこから始まるか**。現在地とは別の担体（**入口の印**）を1つだけ足して、3つの状態を撃ち分ける（実測: 読み込み中は骨8本・入口0・現在地0、空は文言1個・入口0・現在地0、一覧は行8・**入口1・現在地0**）。対照（＝開いた直後に先頭行へ現在地を置く。No.101 の実装そのもの）は、開いた瞬間から現在地が1個あり、`Enter` **1回で行が開く**——読み手は何も選んでいないのに決定できてしまう。既定は `Enter` 1回では現在地が立つだけ（`open=false`）で、2回目に開く。**対価は1手**で、そこは正直に払う。芯は `Esc` にある。既定も対照も `Esc` の直後は現在地が **0個**で、**「無い」の見た目は完全に同じ**。違うのは次の一歩だけで、既定は入口の印が直前に居た行（id=5）に付き、`↓` を押すと **id=5** に立つ（隣の6ではない——**始まるのであって、動くのではない**）。対照は入口を持たないので `↓` で **id=0** へ戻り、直前に居た場所が画面のどこにも残っていない。**「まだ選んでいない」と「選ぶのをやめた」は、無さでは区別できない。区別できるのは次の一歩の場所だけ。** 第3は動きの側。現在地が無い状態から最初の1つを選ぶのは**移動ではない**（出発地が無い）。No.92 の答え（飛べるのは隣まで／経路を見せる）は出発地がある前提だったので、そのままでは使えない。担体は**その場で立ち上がる**（`scale(0.88)→1`、120ms。毎フレーム記録で `translate` 成分は全フレーム 0、現れたあとの `top` は `[82.7]` の1値だけ＝位置の中間状態が無い。`transform-origin: top center` で上端を数学的に不変にしている）。第4に、**言葉が要るのは次の一歩が無いときだけ**。未選択の画面には「未選択」「選択されていません」の語が1つも無い（実測で禁止語0件）。言葉が出るのは空の台帳のときだけ（1個）——**次の一歩が在るなら印で言えるが、無いなら言葉で言うしかない**。検収で1つ直した。初稿の入口の印は静止した中空の輪で、**「予告」に見えなかった**。予告の担体はこの図鑑に既に語彙がある——No.59「予告と発効」の**「予告の紙は端が揺れ続け、発効の瞬間ぴたりと止まる」**。印を 1.8s の呼吸（透明度 0.4↔1、ぷるんは使わない。跳ねは出来事の語彙で、これは待っている状態の語彙）にし、現在地が立った瞬間に印そのものが消える形にした（実測: 呼吸の `animation-name` が付くのは入口の印だけで、他の7行と現在地の行は `none`）。**予告は揺れ続け、確定すると消える。**',
